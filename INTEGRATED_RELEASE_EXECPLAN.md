...
- [x] (2025-11-29 15:10Z) Captured the final release review outcomes: no new code deltas required, CI remains the gating signal (no fresh local test execution beyond the recorded suites), and governance/runbook touchpoints are logged for the rollout handoff.
...
- Cryptanalysis commissioning guidance is codified in `docs/SECURITY_REVIEWS.md` with explicit PQ parameter scopes, deliverable expectations, and finding templates, giving auditors and pool operators a single source for miner identity and pool cryptography reviews.
- Formal verification artifacts for MASP balance and PoW fork-choice live under `circuits/formal/` and `consensus/spec/formal/` with TLC/Apalache instructions; they align with the canonical parameters in `consensus/spec/consensus_protocol.md` and the transaction witness layout, reducing drift between specs and code.
- Adversarial harnesses now cover PQ handshake tampering and ciphertext integrity (e.g., `network/tests/adversarial.rs`), and the CI `security-adversarial` job wires them together with the circuit fuzzers and wallet address fuzzing; a local run of the handshake tampering test passed, while the monolithic `security_pipeline` suite was aborted mid-bootstrap due to the workspace compilation footprint and remains delegated to CI for full coverage.
- Validation outcomes and lessons: the `docs/SECURITY_REVIEWS.md` additions rendered without broken anchors, the MASP balance and PoW fork-choice specs executed under their TLC/Apalache harnesses with canonical parameters intact, and the PQ handshake tampering test remained green even as the CI-consolidated `security_pipeline` reminded us to keep heavyweight adversarial coverage in automation rather than developer loops.
- Reviewed the transaction circuit fixtures and CI coverage: the `gen_fixtures.rs` helper now deterministically emits proving/verifying keys, a valid proof, and a deliberately tampered `invalid_balance.json` from a mixed-asset witness, while the `circuits-proof` and `security-adversarial` CI stages exercise the transaction/block circuit suites, prover smoke tests, and fuzz/adversarial harnesses; the latest `PROPTEST_MAX_CASES=64` run kept the circuit fuzz harness green even as broader workspace flakes persisted, so the adversarial CI gate remains the reliable circuit guardrail.
- Final review outcomes: the single-binary plan is accepted as written with rollout governance and runbook notes captured in `governance/VERSIONING.md` and `runbooks/runtime_upgrade.md`; no additional local tests were run for this review cycle, with CI expected to re-affirm the recorded suites. Retrospective: keep ExecPlan, governance, and runbook updates in lockstep during final approvals so operational owners inherit the same state as engineering.
