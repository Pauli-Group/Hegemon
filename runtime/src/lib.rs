...
impl system::Config for Runtime {
    type BaseCallFilter = HegemonCallFilter;
...
pub struct HegemonCallFilter;

impl frame_support::traits::Contains<RuntimeCall> for HegemonCallFilter {
    fn contains(call: &RuntimeCall) -> bool {
        matches!(call, RuntimeCall::Kernel(_) | RuntimeCall::Timestamp(_))
    }
}
    ...
    for RuntimeStablecoinPolicyProvider
{
    fn policy(
        ...
        Some(pallet_shielded_pool::StablecoinPolicySnapshot {
            ...
            oracle_max_age: policy.oracle_max_age as BlockNumber,
    ...
    for RuntimeOracleCommitmentProvider
{
    fn latest_commitment(
        ...
        let policy = manifest::protocol_manifest()
            .stablecoin_policies
            .into_iter()
            .find(|entry| entry.oracle_feed == *feed_id)?;
        Some(pallet_shielded_pool::OracleCommitmentSnapshot {
            commitment: policy.oracle_commitment,
    ...
    for RuntimeAttestationCommitmentProvider
{
    fn commitment(
        ...
        let policy = manifest::protocol_manifest()
            .stablecoin_policies
            .into_iter()
            .find(|entry| entry.attestation_id == *commitment_id)?;
        Some(pallet_shielded_pool::AttestationCommitmentSnapshot {
            commitment: policy.attestation_commitment,
            disputed: policy.attestation_disputed,
