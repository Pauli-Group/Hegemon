...
fn test_arith_lookups() {
    ...
    prover
        .verify_all_tables_with_public_values(&batch_stark_proof, &common, Some(public_values))
        .unwrap();
...
fn get_test_circuit_proof() -> TestCircuitProofData {
    ...
    let batch_stark_proof = prover
        .prove_all_tables(&traces, &common, witness_multiplicities)
        .unwrap();
    let public_values = PublicAir::<F, 1>::trace_to_public_values(&traces.public_trace);
