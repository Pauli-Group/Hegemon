    ...
    SC::Challenge: PrimeCharacteristicRing,
{
    ...
    // Generate all challenges (alpha, zeta, PCS challenges).
    ...
    let init_first_point = pcs.first_point(&init_trace_domain);
    let init_next_point = init_trace_domain.next_point(init_first_point).ok_or_else(|| {
        VerificationError::InvalidProofShape(
            "Init trace domain does not provide next point".to_string(),
        )
    })?;
    let generator = init_next_point * init_first_point.inverse();
    let generator_target = circuit.alloc_const(generator, "trace_subgroup_gen");
    let zeta_next = circuit.alloc_mul(zeta, generator_target, "zeta_next");
    ...
    pcs.verify_circuit(
        circuit,
        &challenge_targets[2..], // PCS challenges (after alpha, zeta)
        &coms_to_verify,
        opening_proof,
        pcs_params,
    )?;
...
///
/// This includes:
/// - Base STARK challenges (alpha, zeta)
    ...
    SC::Challenge: PrimeCharacteristicRing,
{
    ...
    let CommitmentTargets {
        trace_targets,
        quotient_chunks_targets,
        random_commit,
        ..
    } = &proof_targets.commitments_targets;
    let trace_comm_targets = trace_targets.to_observation_targets();
    let quotient_comm_targets = quotient_chunks_targets.to_observation_targets();
    let random_comm_targets = random_commit.as_ref().map(|c| c.to_observation_targets());
    let preprocessed_commit_targets = preprocessed_commit.as_ref().map(|c| c.to_observation_targets());

    let degree_bits = proof_targets.degree_bits;
    let degree_bits_minus_zk = degree_bits.saturating_sub(config.is_zk());
    let degree_bits_target = circuit.alloc_const(
        SC::Challenge::from_usize(degree_bits),
        "degree_bits",
    );
    let degree_bits_minus_zk_target = circuit.alloc_const(
        SC::Challenge::from_usize(degree_bits_minus_zk),
        "degree_bits_minus_zk",
    );
    ...
    // Return flat vector: [alpha, zeta, ...pcs_challenges]
    let mut all_challenges = vec![alpha, zeta];
