    ...
    SC::Pcs: PcsGeneration<SC, <SC::Pcs as Pcs<SC::Challenge, SC::Challenger>>::Proof>,
{
    ...
    let num_challenges = 2 // alpha and zeta (zeta_next is derived)
        ...
        + CanObserve<FriMmcs::Commitment>,
{
    fn generate_challenges(
        ...
        let (commit_pow_bits, query_pow_bits, log_height_max) = match *params {
            [commit_pow_bits, query_pow_bits, log_height_max] => {
                (commit_pow_bits, query_pow_bits, log_height_max)
            }
            [query_pow_bits, log_height_max] => (0, query_pow_bits, log_height_max),
            _ => {
                return Err(GenerationError::InvalidParameterCount(params.len(), 3));
            }
        };
        opening_proof
