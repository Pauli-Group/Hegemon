    ...
    SymbolicExpression<SC::Challenge>: From<SymbolicExpression<Val<SC>>>,
{
    ...
    pub fn verify_all_tables(
        ...
        self.verify_all_tables_with_public_values(proof, common, None)
    }

    /// Verify the unified batch STARK proof against all tables with explicit public values.
    pub fn verify_all_tables_with_public_values(
        &self,
        proof: &BatchStarkProof<SC>,
        common: &CommonData<SC>,
        public_values: Option<Vec<Val<SC>>>,
    ) -> Result<(), BatchStarkProverError> {
        ...
        EF: Field + BasedVectorSpace<Val<SC>>,
    {
        ...
        let public_values =
            PublicAir::<Val<SC>, D>::trace_to_public_values(&traces.public_trace);
    ...
    fn verify<const D: usize>(
        ...
        if let Some(values) = public_values {
            pvs[PrimitiveTable::Public as usize] = values;
        }
...
mod tests {
    ...
    fn test_babybear_batch_stark_base_field() {
        ...
        let public_values = PublicAir::<BabyBear, 1>::trace_to_public_values(&traces.public_trace);
        assert!(
            prover
                .verify_all_tables_with_public_values(&proof, &common, Some(public_values))
                .is_ok()
        );
    }
    ...
    fn test_extension_field_batch_stark() {
        ...
        let public_values = PublicAir::<BabyBear, 4>::trace_to_public_values(&traces.public_trace);
        prover
            .verify_all_tables_with_public_values(&proof, &common, Some(public_values))
            .unwrap();
    }
    ...
    fn test_koalabear_batch_stark_base_field() {
        ...
        assert!(proof.w_binomial.is_none());
        let public_values =
            PublicAir::<KoalaBear, 1>::trace_to_public_values(&traces.public_trace);
        prover
            .verify_all_tables_with_public_values(&proof, &common, Some(public_values))
            .unwrap();
    }
    ...
    fn test_koalabear_batch_stark_extension_field_d8() {
        ...
        assert_eq!(proof.w_binomial, Some(expected_w));
        let public_values =
            PublicAir::<KoalaBear, 8>::trace_to_public_values(&traces.public_trace);
        prover
            .verify_all_tables_with_public_values(&proof, &common, Some(public_values))
            .unwrap();
    }
