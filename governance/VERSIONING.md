# Version release coordination and activation playbook
...
In the current release line, the only accepted transaction-proof binding resolves to `SmallwoodCandidate`. The historical backend wire discriminant is reserved as a rejecting tombstone: no version maps to it and release coordination must not schedule it as a compatibility or rollback path.
...
- **VersionProposal** – the ZIP-style release artifact that specifies:
...
3. **Scheduling** – Once a release line adopts the proposal, it is added to `VersionSchedule` by calling `register(VersionProposal { ... })`. This step bakes the activation height into that release line’s consensus rules: every PoW node running the same release computes the same `VersionSchedule` state at a given height, so block templates generated by miners after `activates_at` must include the new binding. The activation height is announced at least 2 epochs ahead so wallets can ship new proving keys and pools can propagate updated binaries.
...
1. **Release artifact distribution** – Every adopted `VersionProposal` (plus attached verifying key digests) is published in the release repository and mirrored as a signed bulletin sent to mining pools and infrastructure providers. Pools treat this similarly to a template change announcement.
...
4. **Conflict handling** – If two proposals attempt to occupy the same height with incompatible bindings, release coordinators must linearize them into one canonical schedule before `register` is called. Consensus cannot resolve that conflict by itself; nodes only see the schedule compiled into the release they run. Should a miner somehow compile an outdated `VersionSchedule`, full nodes reject their blocks with `ConsensusError::UnsupportedVersion`, and pools drop the miner until they upgrade.
...
1. Release coordinators publish the proving/verifying key artifacts (or URIs + SHA-256 digests) referenced in the proposal.
...
2. Push the proposal through review/release coordination and merge the new bindings/keys.
