...
- `formal-core`: aggregates parallel `checker`, `lean`, `vectors`, and `policy` invocations of `bash scripts/check_formal_core.sh`; the required aggregate fails unless every shard passes. Together they build the pinned Lean proof kernel, check every generated Lean-to-Rust conformance vector, and validate the formal claims, blueprint, inventory, independent vectors, and native backend release posture.
- `core-tests`: aggregates `base`, transaction library/integration, wallet base plus three proof-heavy multisig workflows, and both node feature-profile invocations of `./scripts/check-core.sh`; the required aggregate fails unless all nine shipping-path Rust test shards pass. The wallet tx-leaf E2E case remains owned once by `native-path-tests`.
- `security-adversarial`: runs `HEGEMON_REDTEAM_MODE=ci bash scripts/run_proving_redteam.sh` for hostile proving regressions. The required `native-backend-security` job owns review-package parity so CI does not rebuild and verify the same package twice; `HEGEMON_REDTEAM_MODE=full` retains the parity campaign for standalone release hardening.
