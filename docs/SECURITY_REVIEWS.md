...
- The mandatory release-facing formal gate is:
  ...
  It builds the pinned Lean proof kernel under `formal/lean`, validates `config/formal-security-claims.json`, validates the blueprint DAG in `config/formal-security-blueprint.json`, checks the formal inventory, verifies independent bridge message/replay vectors in `testdata/formal_core_vectors/`, and reruns the existing native backend reference vectors in `testdata/native_backend_vectors/`.
- `formal/lean/Hegemon/Bridge/Replay.lean` contains the first actual Lean theorem, `Hegemon.Bridge.accept_prevents_duplicate`, proving duplicate inbound replay rejection for the executable replay-state transition. Treat it as real but narrow evidence: it does not prove BLAKE3 replay-key derivation, bridge light-client validity, external-chain covenant behavior, or Rust implementation equivalence.
