...
- `formal/lean/Hegemon/Native/BlockArtifactBindingAdmission.lean` contains real Lean block-artifact binding theorems for decoded tx-leaf/action nullifier, commitment, ciphertext-hash, version, and ciphertext payload-hash agreement, plus candidate artifact DA-root, tx statement-commitment, and verified recursive state-root agreement with rejection precedence. `formal/lean/Hegemon/Native/GenerateBlockArtifactBindingAdmissionVectors.lean` emits examples checked against the production helpers called during native recursive artifact verification, and native regressions reject binding mismatches in the specified order. Treat this as real but narrow evidence: it does not prove SCALE decoder correctness, ciphertext hash security or implementation equivalence, DA-root hash security or implementation equivalence, statement-commitment hash security or implementation equivalence, recursive proof cryptographic soundness, native tx-leaf proof soundness, or complete native-node equivalence.
