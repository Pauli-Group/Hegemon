...
- **Discovery and address exchange**
  ...
  - Peers exchange compact address lists on connect and on a periodic rotating refresh. Learned admission caps retained endpoints globally and per IP before persistence or propagation. Opportunistic dials rotate across the admitted pool when capacity remains, with endpoint and in-flight IP deduplication, endpoint plus shared-IP retry backoff, disconnect cooldown, concurrency bounds, and connect/handshake timeouts. Cached startup reconnect targets are also deduplicated by IP, with configured seeds preferred over stale cached ports; explicit operator endpoints remain endpoint-addressed.【F:network/src/service.rs】【F:network/src/peer_manager.rs】
