...
That note now exists for the active integrated witness-free candidate statement. The current no-grinding candidate profile is `rho = 2`, `nb_opened_evals = 3`, `beta = 3`, `decs_nb_evals = 16384`, `decs_nb_opened_evals = 26`, `decs_eta = 3`, zero grinding bits, and it clears the term-wise `128-bit` bar for the live integrated bridge geometry. The integrated backend is now past the old scalar fallback: the packed semantic relation is real, the exact serialized proof envelope for the current Rust candidate now projects to `112716` bytes, and the passing release roundtrip emits `112724` proof bytes. That is about `3.14x` smaller than the shipped `354081`-byte tx proof while still staying below the `524288`-byte native `tx_leaf` cap. After the latest runtime surgery in the Rust engine, the focused warmed release roundtrip is also down to about `2.1s` directly and about `2.1s` through the wrapped `tx_leaf` seam.
