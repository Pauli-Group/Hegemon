...
That note recorded an earlier integrated witness-free candidate statement with `rho = 2`, `nb_opened_evals = 3`, `beta = 3`, `decs_nb_evals = 16384`, `decs_nb_opened_evals = 26`, `decs_eta = 3`, and zero grinding bits. It is historical benchmark evidence, not the active security profile. The active deployed profile and its exact aggregate bound are defined in `tx_proof_smallwood_no_grinding_soundness.md` and checked across Lean and Rust. At the time of this benchmark, the packed semantic relation emitted a `112716`-byte serialized proof envelope and a `112724`-byte release proof, about `3.14x` smaller than the then-shipped `354081`-byte proof and below the `524288`-byte native `tx_leaf` cap; the warmed release roundtrip was about `2.1s` directly and through the envelope.
