//! SHAKE256-512 hash plumbing for the prospective strict Hegemon Binius profile.
//!
...
type Shake256Fixed512 = XofFixedWrapper<Shake256, U64>;

const LEAF_DOMAIN: &[u8] = b"hegemon.proof.merkle-leaf.shake256-512.v1";
const NODE_DOMAIN: &[u8] = b"hegemon.proof.merkle-node.shake256-512.v1";
const TRANSCRIPT_DOMAIN: &[u8] = b"hegemon.proof.fiat-shamir.shake256-512.v1";
...
pub const STRICT_HASH_PROFILE_ID: &str =
    "hegemon.binius64.basefold.prospective-strict-hash.shake256-512.v1";
...
macro_rules! domain_digest {
    ($name:ident, $domain:ident) => {
        #[derive(Clone, Debug)]
        pub struct $name(Shake256Fixed512);

        impl Default for $name {
            fn default() -> Self {
                let mut inner = Shake256Fixed512::default();
        ...
        impl FixedOutput for $name {
            fn finalize_into(self, out: &mut Output<Self>) {
                let mut inner_out = Output::<Shake256Fixed512>::default();
...
/// Two-to-one SHAKE256-512 Merkle compression with an independent node domain.
...
impl CompressionFunction<Output<StrictLeafDigest>, 2> for StrictNodeCompression {
    fn compress(&self, input: [Output<StrictLeafDigest>; 2]) -> Output<StrictLeafDigest> {
        let mut hash = Shake256Fixed512::default();
...
/// Binius hash-suite adapter for 64-byte SHAKE256 commitments.
#[derive(Clone, Debug, Default)]
...
mod tests {
    ...
    fn reference_shake(parts: &[&[u8]]) -> [u8; 64] {
        let mut hash = Shake256::default();
        for part in parts {
    ...
    #[test]
    fn leaf_digest_matches_external_python_hashlib_vector_and_is_64_bytes() {
        let message = b"strict-leaf-kat";
        let got = StrictLeafDigest::digest(message);
        // Generated once with Python 3 hashlib/OpenSSL:
        // hashlib.shake_256(LEAF_DOMAIN + b"strict-leaf-kat").digest(64)
        let expected = [
            0xac, 0x23, 0x1e, 0xe1, 0x6f, 0xdc, 0xd1, 0xb9, 0x94, 0x79, 0x2c, 0x08, 0x59, 0x4e,
            0x32, 0x4b, 0x95, 0x8a, 0xce, 0x23, 0x74, 0xa3, 0x33, 0x47, 0x14, 0xea, 0x39, 0x7a,
            0x53, 0xe8, 0x54, 0x0a, 0x46, 0xcb, 0xa6, 0x50, 0x90, 0xb1, 0x8c, 0x8b, 0xb4, 0x5c,
            0xee, 0x98, 0x36, 0x28, 0x48, 0x2f, 0x39, 0x60, 0x23, 0x8e, 0x07, 0x59, 0x99, 0xc9,
            0xd9, 0x42, 0x6f, 0xa5, 0x4a, 0xc8, 0x78, 0x05,
        ];
