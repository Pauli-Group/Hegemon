...
mod tests {
    ...
    #[test]
    fn challenger_rollover_matches_external_two_digest_vector() {
        type StrictChallenger = HasherChallenger<StrictTranscriptDigest>;

        let mut challenger = StrictChallenger::default();
        let mut prefix = [0u8; 16];
        challenger.sampler().copy_to_slice(&mut prefix);
        challenger
            .observer()
            .put_slice(b"hegemon-strict-transcript-kat");
        let mut got = [0u8; 64];
        challenger.sampler().copy_to_slice(&mut got);

        // Independently generated with Python hashlib. The first 50 bytes are
        // SHAKE256-400(domain || initial_digest || LE64(16) || observation).
        // Rollover feeds that digest into a reset, re-domain-separated instance;
        // the final 14 bytes are the prefix of SHAKE256-400(domain || first_digest).
        let expected = [
            0xb1, 0xd9, 0x08, 0x0d, 0xac, 0x14, 0xb2, 0xa6, 0x9e, 0x04, 0x23, 0xbe, 0xb1, 0xf7,
            0xbd, 0x32, 0x77, 0x2e, 0x5b, 0x3d, 0xf5, 0x57, 0xc7, 0xe4, 0xf5, 0x7c, 0x2c, 0xd2,
            0x81, 0x5f, 0xb8, 0xa4, 0x3f, 0xb5, 0xf2, 0x13, 0x42, 0x86, 0x1d, 0xa8, 0x63, 0x78,
            0x57, 0x12, 0x43, 0x49, 0x6a, 0x15, 0x69, 0xad, 0x07, 0x3a, 0x6f, 0xf8, 0xba, 0x1b,
            0x5e, 0xa8, 0xdb, 0x8a, 0x2f, 0x4f, 0x42, 0xfd,
        ];
