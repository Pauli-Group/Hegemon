...
const BUFFER_POOL_PATCH_SHA256: &str =
...
const TRANSCRIPT_DOMAIN: &[u8] = b"hegemon.full-m4.shake400-repeated-zk.v1\0";
...
const SOURCE_BUNDLE: &[(&[u8], &[u8])] = &[
    ...
    (
        ...
        include_bytes!("../../strict-hash-profile-shake400/src/lib.rs"),
    ),
    (
        b"scalar/src/lib.rs",
        include_bytes!(
    ...
    (
        b"buffer-pool-u50.patch",
        include_bytes!("../patches/buffer-pool-u50.patch"),
    ),
];
...
fn prove_artifact(out: &Path) -> Result<(), String> {
    ...
    let proof_collision_bits = 400.0 / 3.0;
    ...
    require(
        composed_bits >= 128.0,
        "composed PQ128 arithmetic screen fell below 128 bits",
    )?;
    require(
        composed_bits >= 128.0,
    ...
    let proof_lengths: Vec<usize> = canonical_records
        ...
        .collect();
    let manifest = json!({
        "schema": "hegemon.full-m4.shake400-repeated-zk-baseline.v1",
        "status": "full-m4-zk-wrapper-pq128-screen-not-production-authorized",
        "relation": {
        ...
        "backend": {
            ...
            "buffer_pool_patch_sha256": BUFFER_POOL_PATCH_SHA256,
        ...
        "security": {
            ...
            "composed_arithmetic_screen_bits": composed_bits,
            "zk_wrapper_basis": "pinned upstream ZK wrapper plus grouped precommit/private Phase-A relation; sequential copies use independent honest-prover coins",
            "complete_zk_established": false,
            "security_claim_established": false,
...
fn verify_artifact(artifact: &Path) -> Result<(), String> {
    ...
    println!(
        ...
        serde_json::to_string_pretty(&json!({
            "schema": "hegemon.full-m4.shake400-repeated-zk-restart-verification.v1",
            "verified": true,
...
fn source_bundle_digest() -> [u8; 32] {
    ...
    Sha2Digest::update(
        &mut hash,
        b"hegemon.full-m4.shake400-baseline.source-bundle.v1\0",
    );
