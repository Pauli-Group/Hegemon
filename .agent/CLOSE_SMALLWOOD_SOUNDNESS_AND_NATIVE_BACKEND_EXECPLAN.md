...
- [x] (2026-07-16) Committed, pushed, and opened ready pull request #202.
- [ ] (2026-07-16) Remove measured CI duplication, hold the complete blocking pipeline below 30 minutes, regenerate the deterministic review package, and finish PR #202 green.
...
- Observation: the native backend test helper generated a fresh roughly 90-second proof for each arbitrary seed even when tests only needed one immutable valid artifact; the same 51-test binary consumed about 30 minutes in two separate CI jobs.
  Evidence: PR #202 job logs completed successive fixture-backed tests at roughly 90-second intervals and reported 1,808 seconds for `superneo_hegemon`.

- Observation: importing one ArkLib classical extractor definition caused the path-scoped formal-crypto job to build thousands of unrelated `ToMathlib.Probability` modules and hit its 60-minute timeout.
  Evidence: the cancelled job was still building `ToMathlib.Probability.ProbabilityMassFunction.TotalVariation` at build item 2,641 when GitHub stopped it.

- Observation: the native timing smoke generated 144 complete transaction-leaf artifacts in every pull request, while the `superneo-bench` unit test regenerated the same review-vector bundle already regenerated by the package verifier.
  Evidence: the full timing profile took 1,561 seconds locally; the bounded 16-sample-per-class profile took 246 seconds and rejects lower counts. Changing the unit test to verify the checked-in bundle reduced it from 232 seconds to 14 seconds while package generation parity remained in the package gate.
...
- Decision: enforce a 30-minute CI wall-clock budget by reusing immutable cryptographic fixtures and parallelizing independent existing gates behind the same required aggregate check names.
  Rationale: repeated proof generation and serial independent checks add no assurance. Aggregators preserve fail-closed branch protection and release dependencies while removing latency rather than coverage.
  Date/Author: 2026-07-16 / Codex

- Decision: remove ArkLib from the sanity package and state the exact adaptive standard-QROM target locally.
  Rationale: the imported ArkLib definition was a classical straight-line baseline, not Hegemon's required QROM theorem, and its broad build consumed an hour without strengthening the claimed boundary.
  Date/Author: 2026-07-16 / Codex

- Decision: keep the 64-sample-per-class timing profile as the default research run, but enforce a fail-closed 16-sample-per-class, four-warmup profile in blocking CI.
  Rationale: the harness is explicitly a gross timing-regression screen, not a constant-time proof. The bounded profile preserves interleaving, Welch and relative-delta checks, and rejects attempts to lower its sample floor while removing roughly ten minutes from every pull request.
  Date/Author: 2026-07-16 / Codex
...
- `HEGEMON_REDTEAM_MODE=ci PROPTEST_CASES=64 bash scripts/run_proving_redteam.sh`: every parser, semantic, SmallWood, recursive-block, receipt-root, prover-configuration, and network campaign passed in 319.62 seconds. Review-package regeneration is owned once by `native-backend-security` rather than repeated in the CI red-team lane.
- `bash scripts/check-core.sh test-base`: passed in 286.49 seconds, including all three serialized P2P integration tests in 10.97 seconds after the readiness-flake repair.
- `cargo test -p superneo-bench`: all nine tests passed in 16.48 seconds after review-vector generation was moved exclusively to the package parity gate.
- `HEGEMON_TIMING_SAMPLE_COUNT=16 HEGEMON_TIMING_WARMUP_COUNT=4 cargo run -p native-backend-timing --release`: passed in 245.69 seconds; a sample count below 16 fails before proof generation.
...
### Milestone 6: Bound CI convergence without reducing assurance

Use PR #202 step timings to remove repeated cryptographic fixture construction, repeated review-vector generation, and repeated verification of byte-identical packages. Split independent Rust and formal-core stages across runners, retain `core-tests` and `formal-core` as fail-closed required aggregates, and keep release build dependent on those aggregates. Preserve the full timing research profile while enforcing a bounded PR profile with a hard minimum. Replace the broad research-library import with the narrow standard-QROM target actually required by this branch. Acceptance requires every check green and the complete PR pipeline, including release build, to finish within 30 minutes of workflow start.
