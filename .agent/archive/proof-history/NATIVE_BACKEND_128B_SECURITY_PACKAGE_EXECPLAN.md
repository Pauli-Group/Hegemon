...
Historical note: this plan completed the package scaffolding pass around `goldilocks_128b_rewrite`. The later structural commitment rewrite in [`.agent/STRUCTURAL_COMMITMENT_GEOMETRY_EXECPLAN.md`](/Users/pldd/Projects/Reflexivity/Hegemon/.agent/STRUCTURAL_COMMITMENT_GEOMETRY_EXECPLAN.md) superseded that family as the active line.
...
- [x] (2026-03-27 23:16Z) Completed milestone 6: added the audit/break-it bundle under [audits/native-backend-128b](/Users/pldd/Projects/Reflexivity/Hegemon/audits/native-backend-128b), added [package_native_backend_review.sh](/Users/pldd/Projects/Reflexivity/Hegemon/scripts/package_native_backend_review.sh) and [verify_native_backend_review_package.sh](/Users/pldd/Projects/Reflexivity/Hegemon/scripts/verify_native_backend_review_package.sh), and established the first reproducible tarball path. That package was later superseded by the redteam-fix rebuild below.
- [x] (2026-03-27 23:27Z) Completed milestone 7: archived the first closure benchmark and reran the focused backend/consensus/node validation set. That first closure snapshot was later superseded by the redteam-fix rerun below.
- [x] (2026-03-27 23:58Z) Redteam forced three real fixes: the claim model now uses the full active parameter object instead of manifest-label shortcuts, the reference verifier now uses its own transcript/opening/fold checks instead of production verification helpers, and the timing harness now uses `64` samples plus relative-delta gates instead of the old permissive `abs(t) < 10` screen.
- [x] (2026-03-28 00:12Z) Regenerated the fixed vector bundle, reran the cross-verifier agreement tests, archived a fresh current-tree release benchmark at [native_tx_leaf_receipt_root_security_package_redteamfix_20260327.json](/Users/pldd/Projects/Reflexivity/Hegemon/.agent/benchmarks/native_tx_leaf_receipt_root_security_package_redteamfix_20260327.json), and rebuilt the external-review package with the checksum then current at that point.
- [x] (2026-03-28 01:41Z) Reworked the claim model one level deeper instead of leaving `commitment_binding_bits` as a naked label: added `max_commitment_message_ring_elems` to the bound surface, added `commitment_codomain_bits`, `commitment_same_seed_search_bits`, and `commitment_random_matrix_bits` to `NativeSecurityClaim`, fixed parameter-fingerprint/spec-digest drift so the explicit claim inputs actually affect artifact identity, regenerated the fixed vectors, and rebuilt the review package with checksum `6536a67bdf3d1dee1363b15b8acbd22d3fe4b3263dbfb3ecc4e405562ae95678`.
...
The plan is complete. The repo now has one exact protocol document at [docs/crypto/native_backend_spec.md](/Users/pldd/Projects/Reflexivity/Hegemon/docs/crypto/native_backend_spec.md), one manifest-owned `spec_label`, one code-derived `spec_digest`, one explicit `NativeSecurityClaim`, one fixed vector bundle, one independent verifier, local and CI fuzz-smoke discipline, one timing harness, and one reproducible external-review package. The post-redteam line was stricter than the first closure snapshot: it charged a `7`-bit composition loss, used the explicit `commitment_assumption_bits` input instead of a manifest-label shortcut, used `64` timing samples plus relative-delta gates, and recorded `code_fingerprint.json` with a composite `worktree_fingerprint` instead of pretending every tarball came from a clean commit. That package scaffolding remains in force. The active family, however, is now owned by [`.agent/STRUCTURAL_COMMITMENT_GEOMETRY_EXECPLAN.md`](/Users/pldd/Projects/Reflexivity/Hegemon/.agent/STRUCTURAL_COMMITMENT_GEOMETRY_EXECPLAN.md), which replaced `goldilocks_128b_rewrite` with the geometry-derived `goldilocks_128b_structural_commitment` line. The closure verdict from this plan remains `KEEP`, but only for the security-package machinery, not for the old active family label.

## Context and Orientation

The current native backend lives in [circuits/superneo-backend-lattice/src/lib.rs](/Users/pldd/Projects/Reflexivity/Hegemon/circuits/superneo-backend-lattice/src/lib.rs). That file owns the backend manifest, parameter object, commitment and opening logic, transcript challenge schedule, fold verification, and the current `NativeSecurityClaim`. The active family is now `goldilocks_128b_structural_commitment`. It is a candidate, not a final package.
...
The current design and methods state is captured in [DESIGN.md](/Users/pldd/Projects/Reflexivity/Hegemon/DESIGN.md) and [METHODS.md](/Users/pldd/Projects/Reflexivity/Hegemon/METHODS.md). Both documents now say, in plain language, that the active family is `goldilocks_128b_structural_commitment` and that the backend is still an in-repo approximation. This plan must keep those documents synchronized with the actual implementation and must not let either file overclaim.
