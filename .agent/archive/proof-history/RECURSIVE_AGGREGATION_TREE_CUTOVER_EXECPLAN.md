...
- [x] (2026-03-13 01:03Z) Added explicit coordinator logging when a recursive stage produces a final `PreparedBundle` (`Prepared recursive proven batch is ready` with `tx_count`, `proof_mode`, `artifact_hash`, and `build_ms`). This gives the next strict throughput run a direct prepared-batch latency signal instead of only the wrapper-side wait loop.
- [x] (2026-03-13 04:25Z) Fixed the throughput matrix harness so it can actually test `1/2/4/8` local provers on the recursive lane. `scripts/throughput_sidecar_aggregation_tmux.sh` now caps local aggregation workers by first-level leaf-job count instead of `leaf_fanin`, derives conservative strict/job/TTL budgets from `(tx_count, leaf_fanin, merge_fanin, prover_workers)` when operators do not pin them explicitly, and exports `HEGEMON_PROVER_WORK_PACKAGE_TTL_MS` alongside `HEGEMON_BATCH_JOB_TIMEOUT_MS`. `scripts/throughput_scaling_matrix.sh` now lets those derived budgets apply by default instead of forcing a fixed `7200s` / `7200000ms` envelope onto every point.
- [x] (2026-03-13 05:01Z) Restored `HEGEMON_AGG_PROVER_THREADS` handling to the V5 leaf/merge outer-prove path in `circuits/aggregation/src/v5.rs` and re-profiled the live-tree leaf benchmark on `hegemon-prover`. `cargo check -p aggregation-circuit` passes locally. The remote `aggregation_v5_leaf_fanin8_cold_warm_profile` run now uses a multi-core test process, but cold `v5_leaf_outer_prove` still measured `713392 ms` (total cold leaf `796944 ms`), so the dropped thread-pool hook was not the main throughput blocker.
- [x] (2026-03-13 05:01Z) Added env-tunable aggregation table packing in `circuits/aggregation/src/lib.rs` and `circuits/aggregation/src/v5.rs` via `HEGEMON_AGG_WITNESS_LANES`, `HEGEMON_AGG_ADD_LANES`, and `HEGEMON_AGG_MUL_LANES`, then rechecked both `cargo check -p aggregation-circuit` and `LIBCLANG_PATH=/Library/Developer/CommandLineTools/usr/lib DYLD_LIBRARY_PATH=/Library/Developer/CommandLineTools/usr/lib cargo check -p hegemon-node`. This keeps the next proving experiment additive: if the corrected matrix still stalls on `outer_prove`, the next remote benchmark can test denser witness packing without another code pass.
- [x] (2026-03-13 18:32Z) Revalidated the strict local throughput lane on the current tree with the operator-style snapshot flow: a clean `tx32/pw1` run now reuses a funded snapshot, admits all 32 proofless sidecar transfers, and reaches `Strict mode: waiting for local proven batch candidate...` with live coordinator status `queued_jobs=7`, `inflight_jobs=1`, `prepared_bundles=0`.
- [x] (2026-03-13 18:32Z) Identified the remaining cold-start bottleneck with a local sample on the strict lane and on the ignored aggregation leaf benchmark: the cold recursive leaf spends its wall-clock in `aggregation_circuit::build_common_data_parallel` / `commit_preprocessing` (Merkle/MMCS preprocessing), not in scheduler glue or proofless admission.
- [x] (2026-03-13 18:32Z) Fixed two remaining throughput-harness blind spots. `scripts/throughput_sidecar_aggregation_tmux.sh` now accepts aggregation packing knobs end to end, and `scripts/throughput_scaling_matrix.sh` now keeps worker prewarm disabled only during snapshot creation while enabling worker prewarm by default for real runs and auto-prewarming the target `tx_count` shape before strict measurement.
- [x] (2026-03-13 19:05Z) Added a leaf-only worker-prewarm mode in `circuits/aggregation/src/v5.rs` plus harness plumbing in `scripts/throughput_sidecar_aggregation_tmux.sh` / `scripts/throughput_scaling_matrix.sh`. Throughput-matrix runs now default to `HEGEMON_AGG_PREWARM_INCLUDE_MERGE=0`, which avoids doing a full representative leaf outer proof during startup just to derive merge-prewarm inputs.
- [x] (2026-03-13 19:05Z) Rebuilt the release node and reran the strict `tx32/pw1` lane on the leaf-only prewarm path. Startup now reaches RPC in roughly 90-110 seconds instead of sitting in full worker prewarm for many minutes, but the live coordinator still settles at `queued_jobs=7`, `inflight_jobs=1`, `prepared_bundles=0` after send, so the first live leaf outer proof remains the prepared-bundle bottleneck on this host.
- [x] (2026-03-13 23:12Z) Generalized V5 aggregation and the prover coordinator from a hard-coded two-level tree to arbitrary-depth fixed-fan-in recursion. `circuits/aggregation/src/v5.rs` and `consensus/src/aggregation/v5.rs` now allow merge-of-merge children, while `node/src/substrate/prover_coordinator.rs` plans full level-by-level trees, publishes merge stages as soon as each subtree is ready, and verifies subtree-scoped merge payloads instead of assuming one root merge over all leaves.
- [x] (2026-03-13 22:30Z) Added primitive-trace size profiling to the V5 leaf/merge path in `circuits/aggregation/src/v5.rs`, rebuilt `aggregation-circuit`, and reran clean remote benchmarks on `hegemon-prover`. The cold `tx_count=1` non-root leaf measured `witness/add/mul = 15202544 / 9218899 / 6073661` rows with `v5_leaf_outer_prove = 104108 ms` and total `147001 ms`; the cold `tx_count=2` root leaf measured `30404798 / 18437798 / 12147322` rows with `v5_leaf_outer_prove = 208460 ms` and total `296968 ms`; and the cold `merge(2 x leaf_fanin=1)` node measured `19616022 / 11935172 / 7859506` rows with `v5_merge_outer_prove = 120996 ms` and total `176997 ms`.
- [x] (2026-03-13 22:30Z) Re-ran the clean one-child leaf under `HEGEMON_AGG_PROVER_THREADS=8 RAYON_NUM_THREADS=8` and confirmed that naive thread throttling makes the outer prover worse, not better: the same leaf was still stuck in `v5_leaf_outer_prove` after roughly `179 s` in the outer-prove window, compared with `104108 ms` on the default unconstrained pool.
- [x] (2026-03-13 23:58Z) Re-ran the key leaf-vs-merge profiles directly on `hegemon-prover` using the current tree code and validated the warm-path shape choice: `aggregation_v5_leaf_partial_cold_warm_profile` with `HEGEMON_AGG_LEAF_FANIN=2 HEGEMON_AGG_ACTIVE_CHILDREN=2` measured `cold_ms=296913 warm_ms=227329`, while `aggregation_v5_merge_cold_warm_profile` with `HEGEMON_AGG_LEAF_FANIN=1 HEGEMON_AGG_MERGE_FANIN=2` measured `cold_ms=176068 warm_ms=133886`. That makes the production cutover explicit: singleton leaves plus binary merges are materially cheaper than proving multi-child leaves directly, so the default live lane now moves from the stale `4/8` and `8/8` split to a consistent `1/2` across circuit, coordinator, verifier, and import helpers.
- [x] (2026-03-13 23:58Z) Fixed two benchmark blind spots exposed by the remote rerun. `measure_recursive_prepared_latency` in `node/src/substrate/prover_coordinator.rs` now logs `stage_queue` details so recursive progress is diagnosable instead of a bare shape hash, and it now respects shell-provided `HEGEMON_AGG_*` thread knobs instead of overwriting them unconditionally inside the process. Remote `hegemon-prover` runs showed why that matters: Rayon thread caps must be exported before process start, otherwise a nominal `workers=1` run can still burn ~18 CPU cores via already-initialized global pools.
- [ ] (2026-03-13 18:32Z) Complete a fresh `tx32/tx64 × 1,2,4,8,16` strict matrix on the corrected warm-worker path, then rerun targeted outer-proof experiments against the remaining cold common-data build.
...
- Observation: the remote prover data justifies a full small-fan-in cutover, not more tuning around wide leaves.
  Evidence: on `hegemon-prover`, a `tx_count=2` root leaf measured `30404798 / 18437798 / 12147322` witness/add/mul rows with `296913 ms` cold and `227329 ms` warm, while the binary alternative `merge(2 x singleton leaf)` measured only `19616022 / 11935172 / 7859506` rows with `176068 ms` cold and `133886 ms` warm. The repo also had split defaults (`4/8` in the circuit/coordinator and `8/8` in verifier/import helpers), so the cutover work now aligns the live default lane to `leaf_fanin=1`, `merge_fanin=2`.
...
- Observation: strict throughput runs lacked one critical measurement point: prepared-batch readiness itself.
  Evidence: existing logs showed proofless submission and wrapper-side waiting, but there was no direct info-level log at the moment a recursive stage inserted a final `PreparedBundle` into the coordinator. Adding that log in `node/src/substrate/prover_coordinator.rs` now exposes the exact ready event and its `build_ms`.

- Observation: the scaling matrix was still under-driving local parallelism even after the recursion lane and chainspec fixes.
  Evidence: `scripts/throughput_sidecar_aggregation_tmux.sh` silently clamped `HEGEMON_TP_PROVER_WORKERS` to `HEGEMON_TP_AGG_LEAF_FANIN`. With the current defaults that meant the advertised `tx32/pw8` and `tx64/pw8` points never used more than `4` local prover workers even though those candidates have `8` and `16` first-level leaf jobs.

- Observation: the fixed `7200s` strict wait / `7200000ms` job budget used by the matrix is below the observed wall-clock needed for the `pw1` recursive points and can also expire the recursive assembly state before the last leaf returns.
  Evidence: on `hegemon-prover`, the live `tx32/pw1` run reported a cold leaf at `916977 ms` and a warm leaf at `711485 ms`. With `leaf_fanin=4`, `tx32` needs `8` leaves and `tx64` needs `16`, so a single-worker run exceeds the old fixed budget before merge/finalize. The coordinator ties package expiry to the same timeout budget (`HEGEMON_PROVER_WORK_PACKAGE_TTL_MS` defaults to `HEGEMON_BATCH_JOB_TIMEOUT_MS`), so the old matrix envelope could drop the recursive assembly before the final stage completed.

- Observation: the V5 path had dropped the old outer-prover thread-pool hook, but restoring it does not materially change the live-tree leaf bottleneck.
  Evidence: after wiring `HEGEMON_AGG_PROVER_THREADS` back into `circuits/aggregation/src/v5.rs`, the remote `aggregation_v5_leaf_fanin8_cold_warm_profile` on `~/Hegemon-scaling-current` used a multi-core test process yet still reported `aggregation_profile stage=v5_leaf_outer_prove tx_count=4 outer_prove_ms=713392 total_ms=796944`, which is effectively the same outer-prove cost seen in the live node logs.

- Observation: the next plausible proving lever is row packing, not another missing scheduler or thread-pool knob.
  Evidence: `BatchStarkProver::prove` in `spikes/recursion/vendor/plonky3-recursion/circuit-prover/src/batch_stark_prover.rs` still contains `TODO: Consider parallelizing AIR construction and trace-to-matrix conversions`, while the recursive aggregation path hard-coded `TablePacking::new(4, 4, 1)` for both cache construction and outer proving. Exposing those lane counts via env is low-risk because verification keys are derived from the proof shape, not from a fixed runtime constant.

- Observation: the clean local strict lane is no longer failing at pool admission or wrapper syntax; it now stalls exactly where the architecture says it should if cold recursion is still too expensive.
  Evidence: on the refreshed easy chainspec and current binaries, `scripts/throughput_sidecar_aggregation_tmux.sh` with `tx_count=32`, `proof_mode=aggregation`, and one local prover worker admitted all 32 proofless sidecar transfers, then reported `Strict mode: waiting for local proven batch candidate...` while `prover_getStagePlanStatus` stayed at `queued_jobs=7`, `inflight_jobs=1`, `prepared_bundles=0`.

- Observation: the dominant cold-start cost on this host is aggregation common-data preprocessing, not the leaf outer proof itself.
  Evidence: a local `sample` of both the ignored `aggregation_v5_leaf_fanin8_cold_warm_profile` benchmark and the prewarm-enabled node run showed `aggregation_circuit::v5::prove_leaf_aggregation -> get_or_build_aggregation_common_data -> build_common_data_parallel -> commit_preprocessing`, with the test process reaching `~37 GiB` footprint during the cold common-data build.

- Observation: the throughput matrix itself was still understating the recursive architecture by disabling worker prewarm for the real measurement runs.
  Evidence: before the latest script change, `scripts/throughput_scaling_matrix.sh` exported `HEGEMON_AGG_DISABLE_WORKER_PREWARM=1` everywhere, so strict `tx32/tx64` points were charging thread-local cache/common-data setup to the first live proofless batch instead of prewarming the target shape before send.

- Observation: full worker prewarm was itself overpaying startup by running a representative leaf proof all the way through the outer batch prover just to build merge-prewarm inputs.
  Evidence: sampled stacks on the release node with blocking worker prewarm showed `hegemon-prover-worker-0 -> prewarm_thread_local_aggregation_cache_from_env -> prove_leaf_aggregation -> prove_outer_tables`, while the main thread was still blocked waiting for worker prewarm before RPC startup.

- Observation: leaf-only prewarm is a worthwhile operational default for the throughput matrix, but it does not remove the first live leaf bottleneck.
  Evidence: after adding `HEGEMON_AGG_PREWARM_INCLUDE_MERGE=0` to the matrix path and rebuilding release binaries, the same strict `tx32/pw1` lane reached RPC/startup in roughly `~1.5` minutes instead of many minutes, then admitted all 32 transfers and still sat at `queued_jobs=7`, `inflight_jobs=1`, `prepared_bundles=0` during strict wait.

- Observation: the current `active_children > 1` leaf path buys essentially no batching win at the outer-proof layer; it just scales the primitive trace almost linearly and then pays roughly double the outer batch-STARK cost.
  Evidence: on a clean `hegemon-prover`, a non-root singleton leaf measured `witness/add/mul = 15.20M / 9.22M / 6.07M` rows with `v5_leaf_outer_prove = 104108 ms`, while a two-child root leaf measured `30.40M / 18.44M / 12.15M` rows with `v5_leaf_outer_prove = 208460 ms`. The two-child trace is almost exactly `2x` the primitive rows and the outer-prove wall clock.

- Observation: replacing the bad multi-child leaf with a two-level `singleton leaves + merge` shape helps only partially, because the merge node is itself still a very large outer proof.
  Evidence: `merge(2 x leaf_fanin=1)` on the same clean host measured `witness/add/mul = 19.62M / 11.94M / 7.86M` rows with `v5_merge_outer_prove = 120996 ms` and total `176997 ms` for the merge node alone. That is smaller than the two-child root leaf, but not enough to make a two-level tree “cheap”.

- Observation: the default unconstrained rayon pool is materially better than an 8-thread cap for the current outer prover on this host.
  Evidence: with `HEGEMON_AGG_PROVER_THREADS=8 RAYON_NUM_THREADS=8`, the same one-child non-root leaf had not completed `v5_leaf_outer_prove` after roughly `179 s` in the outer-prove window, versus `104108 ms` on the default pool.
...
- Decision: make the throughput harness derive its default worker cap and timeout/TTL budget from the actual recursive leaf plan instead of pinning a fixed `leaf_fanin` cap and a fixed two-hour envelope.
  Rationale: the user-visible goal is to show prepared-bundle latency improving with more parallel provers. Silently flattening `pw8` to `pw4` invalidates that experiment, and a fixed `7200s` budget is smaller than the observed `pw1` leaf wall time for `tx32/64`, which can cause strict runs to fail by harness policy rather than by proving throughput.
  Date/Author: 2026-03-13 / Codex

- Decision: expose aggregation table packing as env-tunable for the next proving experiment instead of immediately hard-switching away from `4/4/1`.
  Rationale: the corrected matrix run is already in flight and should not be invalidated by an unmeasured proving-layout change. Making `HEGEMON_AGG_WITNESS_LANES` / `HEGEMON_AGG_ADD_LANES` / `HEGEMON_AGG_MUL_LANES` configurable preserves the current lane while keeping the next outer-prove experiment one environment change away.
  Date/Author: 2026-03-13 / Codex

- Decision: keep worker prewarm disabled during funded-snapshot creation, but enable it by default for real throughput-matrix runs and auto-prewarm the target `tx_count` shape when the operator did not pin `HEGEMON_SCALE_AGG_PREWARM_MAX_TXS`.
  Rationale: snapshot creation only mines and syncs notes, so worker prewarm there is wasted time. Real strict runs, however, are meant to measure prepared-bundle latency after the recursive worker caches are ready. The clean `tx32/pw1` run and local samples showed that leaving worker prewarm off charges the dominant common-data build to the first live batch and hides the architecture’s actual steady-state scaling behavior.
  Date/Author: 2026-03-13 / Codex

- Decision: make the throughput matrix default to leaf-only worker prewarm (`HEGEMON_AGG_PREWARM_INCLUDE_MERGE=0`) while leaving full prewarm available as an explicit override.
  Rationale: the measured startup bottleneck on this host was the representative leaf outer proof executed during prewarm, not just common-data setup. Leaf-only prewarm still removes the first-leaf cache/common-data cold build while keeping startup low enough to make strict matrix runs materially more tractable.
  Date/Author: 2026-03-13 / Codex

- Decision: treat the current `active_children > 1` leaf path as a structural dead end on the two-level tree and stop spending time on thread-pool fiddling as the primary fix.
  Rationale: the clean-host measurements show that a two-child leaf is almost exactly `2x` the singleton primitive trace and `~2x` the outer-prove wall clock, while a two-level singleton-leaf/merge tree still leaves the merge node with a `120996 ms` outer proof on `19.62M` witness rows. The viable next implementation step is deeper small-fan-in recursion (`merge-of-merge`) or a materially cheaper outer proof backend, not another local thread cap.
  Date/Author: 2026-03-13 / Codex

- Decision: replace the coordinator’s `all leaves, then exactly one merge` assembly shortcut with a real level-by-level merge scheduler.
  Rationale: without subtree-ready publication, external or local provers cannot overlap independent merge work, so added provers do not translate into throughput even when the recursion payload format already supports merge-of-merge proofs.
  Date/Author: 2026-03-13 / Codex
...
The next harness-level result is also clearer than before this pass. The scaling scripts no longer silently suppress prover parallelism above `leaf_fanin`, and the default recursive timeout budget now scales with the number of first-level leaf jobs so `tx32/64` single-worker points are not forced to fail at the two-hour mark. That does not make the leaf outer proof cheaper, but it removes two benchmark artifacts that would otherwise mask whether throughput really scales with added provers.

The current codebase is finally structurally aligned with the intended throughput model. V5 recursion is no longer capped at `leaf` or `merge-over-leaf`; merge nodes can verify either leaf proofs or lower merge proofs, and the coordinator publishes the first ready merge subtree as soon as its child group finishes instead of waiting for the whole leaf frontier. A deterministic unit test now exercises that exact behavior on a `33`-tx tree and proves the first level-1 merge package is published before the final leaf and root merge are ready.

The next prover-layer result is narrower too. V5 now honors `HEGEMON_AGG_PROVER_THREADS` again, but a direct remote leaf profile on the same tree used by the throughput matrix still spent roughly `713s` in `v5_leaf_outer_prove` for a cold 4-child leaf. That means the current throughput blocker is still the outer batch proof itself, not a missing thread-pool hook around it.

The next code path is now ready for a more aggressive proving experiment without another structural refactor. Aggregation table packing is configurable from the environment, and the local node still compiles with that change. If the corrected matrix proves that the architecture is still stalled on `outer_prove`, the next benchmark can test denser witness packing on the same recursive lane immediately.

The newest local-throughput result changed the diagnosis again. The corrected strict `tx32/pw1` harness now proves that proofless sidecar admission, funded snapshots, and stage scheduling are all working on the current tree: the run reaches `Strict mode: waiting for local proven batch candidate...` with 32 accepted transfers and exactly 8 published leaf jobs. Live coordinator status still sits at `7 queued / 1 inflight / 0 prepared`, so the remaining throughput blocker is inside the first recursive leaf build itself rather than in the scheduler.

The newest sampling result narrowed that cold build further. On this macOS host, the cold recursive leaf is spending its wall-clock inside `build_common_data_parallel` / `commit_preprocessing` (Merkle/MMCS preprocessing) rather than in `v5_leaf_outer_prove`. That means worker prewarm is not optional for meaningful throughput experiments: without it, the matrix is mostly timing cache/common-data bootstrap, not prepared-bundle latency. The latest harness change therefore flips the measurement default to the warm-worker path while still skipping prewarm during snapshot creation. The next useful milestone is to finish the full worker-count matrix on that corrected path and then decide whether the next code change belongs in common-data preprocessing, row packing, or both.

The newest prewarm refinement made that warm-worker path less punitive. The matrix now defaults to leaf-only prewarm, which avoids a representative leaf outer proof during startup and gets the strict `tx32/pw1` lane to RPC/send much sooner on this laptop. That is a real throughput-harness improvement, but it does not change the core prepared-bundle result yet: once the 32 proofless transfers are admitted, the system still blocks on one live leaf outer proof at a time. So the remaining engineering problem is still the outer leaf prove itself, not startup glue.
...
- A “leaf aggregation proof” is a recursive proof that verifies up to `HEGEMON_AGG_LEAF_FANIN` transaction STARK proofs. In this branch the default fan-in is `4`, and incomplete leaves are padded deterministically.
...
Plan update note (2026-03-13 02:58Z / Codex): Isolated a major steady-state leaf bottleneck in the vendored recursion runner: `prove_leaf_aggregation` was deep-cloning the compiled circuit and then cloning the full op list again inside `CircuitRunner::run()`. Refactored the runner to borrow immutable circuit metadata and execute against borrowed ops. Direct release leaf profiling on the `fan_in=4`, recursion `2/2` lane showed the post-cache `runner + set_targets` gap drop from ~14.7s to ~1.1s and `v5_leaf_runner_run` drop from ~20.2s to ~10.0s on local hardware.
Plan update note (2026-03-13 03:22Z / Codex): Found that worker prewarm was warming the wrong aggregation cache key. V5 prewarm built its sample transaction proof with production tx-proof params, while live recursive leaf jobs use `TransactionProofParams::recursion()`. Patched aggregation prewarm to build recursion-shaped sample proofs and added an explicit `aggregation v5 thread-local cache prewarm complete` log.
Plan update note (2026-03-13 03:44Z / Codex): Rebuilt `target/release/hegemon-node` and reran the strict proofless harness. Evidence in `/tmp/hegemon-throughput-optfix.log` now shows all four worker threads prewarming the recursive leaf cache before the first transfer: duplicated `cache_verify_inner`, `cache_circuit_build_*`, and `cache_common_lookup_build_start` lines appear during startup rather than on the first live batch. This confirms the prewarm-key mismatch is fixed.
Plan update note (2026-03-13 03:50Z / Codex): Updated `scripts/throughput_sidecar_aggregation_tmux.sh` so `HEGEMON_TP_PREWARM_ONLY=1` in aggregation mode forces `HEGEMON_AGG_PREWARM_BLOCKING=1` and logs `Aggregation cache startup: worker_prewarm_blocking=1`. This stops the harness from mixing worker-cache startup with strict prepared-batch latency, which previously made the first proofless send absorb the full recursive leaf cold-build cost.
Plan update note (2026-03-13 05:42Z / Codex): Updated the throughput harness to make the scaling matrix runnable instead of manual. `scripts/throughput_sidecar_aggregation_tmux.sh` now supports `HEGEMON_TP_SKIP_BUILD=1` to reuse existing release binaries, and auto-raises `AGG_MERGE_FANIN` when `TX_COUNT` exceeds the current two-level recursive capacity unless the operator explicitly pinned an insufficient value. Added `scripts/throughput_scaling_matrix.sh` to execute the strict aggregation matrix over tx counts and prover-worker counts, emit per-run artifact JSONs, and summarize `strict_wait_ms`, `inclusion_total_ms`, and `effective_tps` into a TSV.
Plan update note (2026-03-13 06:23Z / Codex): Reworked the matrix path so it no longer benchmarks setup churn. `scripts/throughput_sidecar_aggregation_tmux.sh` now supports `HEGEMON_TP_NODE_BASE_PATH`, `HEGEMON_TP_REUSE_EXISTING_STATE`, and `HEGEMON_TP_PREPARE_SNAPSHOT_ONLY`, and reused-state runs keep the prover coordinator alive by starting with mining enabled then issuing `hegemon_stopMining` immediately after RPC startup. `scripts/throughput_scaling_matrix.sh` now prepares one funded snapshot per `tx_count`, restores that snapshot for each prover-count run, and auto-generates an easy local chainspec (`pow_bits = 0x1e00ffff`) so the matrix measures proving and inclusion rather than repeated 35/67-block funding setup.
Plan update note (2026-03-13 04:25Z / Codex): Inspected the live `hegemon-prover` `tx32/pw1` run and found two remaining benchmark-envelope bugs after the recursion cutover. The wrapper was still capping local prover workers to `leaf_fanin`, so the advertised `pw8` points could never use more than `4` workers on the default lane. It also forced a fixed `7200s` / `7200000ms` strict/job budget even though the live run had already shown `~917s` cold and `~711s` warm leaf times, making the `pw1` points time out or expire package TTLs before the last leaf/merge returned. Patched `scripts/throughput_sidecar_aggregation_tmux.sh` to cap by first-level leaf-job count, derive conservative aggregation budgets from `(tx_count, leaf_fanin, merge_fanin, prover_workers)`, and pass `HEGEMON_PROVER_WORK_PACKAGE_TTL_MS`; updated `scripts/throughput_scaling_matrix.sh` to stop overriding those derived budgets unless the operator explicitly sets `HEGEMON_SCALE_STRICT_PREPARE_TIMEOUT_SECS` or `HEGEMON_SCALE_BATCH_JOB_TIMEOUT_MS`.
Plan update note (2026-03-13 05:01Z / Codex): Restored the old `HEGEMON_AGG_PROVER_THREADS` Rayon-pool hook to the V5 leaf/merge outer-prove path so the knob is no longer ignored on the recursive lane. Re-profiled `aggregation_v5_leaf_fanin8_cold_warm_profile` on `~/Hegemon-scaling-current` with `HEGEMON_AGG_PROVER_THREADS=4`; the test process used multiple cores, but cold `v5_leaf_outer_prove` still took `713392 ms`, confirming that the current live-tree bottleneck is the outer batch proof itself rather than missing thread-pool wiring.
Plan update note (2026-03-13 05:01Z / Codex): Added `HEGEMON_AGG_WITNESS_LANES`, `HEGEMON_AGG_ADD_LANES`, and `HEGEMON_AGG_MUL_LANES` as aggregation-prover env knobs, replacing the remaining hard-coded `TablePacking::new(4, 4, 1)` sites in both the legacy and V5 proving paths. Verified `cargo check -p aggregation-circuit` and `cargo check -p hegemon-node` (with the documented macOS `libclang` env). I did not benchmark alternate packing yet because the corrected remote `tx32/pw1` matrix run is already live and should finish without introducing a second variable.
