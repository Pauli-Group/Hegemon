...
The earlier pre-remediation formal-core, red-team, core, and isolated-node runs remain historical evidence only. The corrected tree has now passed focused Lean elaboration, the 142-test formal checker suite, and the complete 14-stage formal-core gate, including claims, blueprint, active-goal, exact generated-vector, native-reference-vector, and release-posture checks. Package, red-team, core, and compatibility gates are being rerun from the committed source and regenerated artifact states. The pre-existing testnet process has not been touched by this branch work. No changed path is under `consensus/`, `network/`, `node/`, runtime serialization, chain-spec, or genesis code. The checked-in review package still describes the earlier source state and must be regenerated from the final clean source commit before publication. The package hash, security-scan result, final commit ids, and the draft PR URL remain pending.
