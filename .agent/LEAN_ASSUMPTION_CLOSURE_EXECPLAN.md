...
The closure score is now authenticated in layers: Rust pins the required track inventory and all 25 supporting theorem identities, including every component of the two compound native arithmetic tracks and the bounded reducer classification; an independent test ratchets those exact sets; the claims ledger must include each closed-track theorem; lexical source provenance still rejects generated/local/private lookalikes; the active-goal ledger binds the complete Lean source tree; and the public active-goal command runs aggregate Lean elaboration plus the axiom policy before reporting success. Rust also pins a domain-separated digest over the Lean toolchain, fixed identity query, and all 25 fully elaborated proposition types, so same-name proposition weakening cannot be hidden by renewing the editable source digest alone. This closes the demonstrated invalid-unimported-theorem exploit, open-track-deletion denominator exploit, representative-theorem compound-track deletion exploit, and same-name `True` weakening exploit. The typed all-16 SmallWood profile regression is cardinality-checked, requires exactly one named field delta, verifies that one non-ignored test executed, and is independently red-teamed; its Lean theorem is explicitly restricted to well-formed serialized inputs. All blueprint target reviews remain pending independent review. Their digests bind the node metadata and referenced implementation/evidence bytes for drift detection, but they are not signatures or proposition acceptance. The native review package contains the complete committed Git source tree except its recursive archive/checksum outputs, applies its compressed/member/expanded limits before the bounded work, rejects extra non-source files, and must match the clean tracked checkout file set and bytes before internal evidence is used. The verifier copies that source outside attacker-controlled package ancestors, rebuilds every generated artifact, and compares all bytes. Intentional proposition changes still require human review of the explicit Rust policy update; they are not represented as cryptographically impossible.
