...
- [x] (2026-06-07T05:52:56Z) Ran local validation: `bash scripts/check_lean_formal.sh`, focused FlyClient generated-vector conformance, `cargo test -p consensus-light-client long_range_proof_verifies_mmr_openings_and_samples -- --nocapture`, `jq empty config/formal-security-claims.json config/formal-security-blueprint.json`, `git diff --check`, `bash scripts/check_formal_core.sh`, `cargo fmt --all -- --check`, and `cargo build -p hegemon-node --bin hegemon-node --no-default-features --release`; all passed locally. The full formal-core gate built 109 Lean jobs, checked FlyClient sampler conformance, and reported `claims = 26`, `production_eligible = 24`, `nodes = 26`, `edges = 59`, and `falsification_cases = 67`. Remote `hegemon-dev` validation is still pending for this revision.
...
Revision note 2026-06-07T05:52:56Z: Added the Lean bridge FlyClient sampler transcript/index kernel, generated sampler vectors, production sampler preimage and digest-prefix reduction helpers used by `flyclient_sample_indices`, formal metadata/docs updates, and passing local Lean/formal-core/release-build validation. Remote `hegemon-dev` validation is still pending for this revision.
