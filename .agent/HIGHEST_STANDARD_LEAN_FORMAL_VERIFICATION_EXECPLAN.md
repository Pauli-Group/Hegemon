...
- [x] (2026-06-15 00:15Z) Added `Hegemon.Native.PendingActionBytePublicationRefinement` with `PendingActionBytePublicationFacts` and `accepted_pending_action_bytes_bind_raw_canonical_publication`: accepted exact pending-action decode, accepted block-action decode, pending-action reload key/hash checks, action-hash admission, action wire-replay projection, and raw canonical publication now compose into one byte-admission package carrying exact parser, reload, action-hash, wire projection, replayed supply, leaf cursor, commitment-root publication, canonical commitment plan, final nullifier uniqueness, and final bridge replay-key uniqueness. This is a zero-runtime-cost accepted-predicate composition; parser-internal arbitrary SCALE/bincode equivalence, tx-leaf proof soundness, DA availability, external bridge receipt soundness, storage durability below the accepted predicate, and complete native-node equivalence remain open. Targeted `lake build Hegemon.Native.PendingActionBytePublicationRefinement Hegemon`, `lake env lean Hegemon/Native/PendingActionBytePublicationRefinement.lean`, and `bash scripts/check_formal_core.sh` passed locally; formal-core reported 100 claims, 1433 named Lean theorem declarations, 90 production-eligible claims, 413 falsification cases, 389 blueprint edges, and 200 implementation bindings. Current tracked completion is 76.54%; `hegemon-dev` validation is the next acceptance gate.
