...
- [x] (2026-06-14 05:44Z) Strengthened `Hegemon.Native.TxLeafCanonicalSurface` with `proof_keyed_transfer_payload_active_input_no_theft_full_binding`: proof-keyed active input no-theft facts now carry payload binding-hash/proof-binding-hash/fee agreement, public-input binding, root/anchor binding, statement/proof nullifier-slot alignment, native fee/proof/backend/ciphertext-payload equality gates, and field-level tx-leaf binding facts in one implementation-equivalence theorem. `lake build Hegemon.Native.TxLeafCanonicalSurface` passed locally. Current tracked completion is 74.91%.
