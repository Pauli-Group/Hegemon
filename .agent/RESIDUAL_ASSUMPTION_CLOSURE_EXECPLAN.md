...
- Observation: The native-devnet-host formal-core failure was not a consensus regression; `scripts/test-node.sh` was reusing a stale default `target/debug/hegemon-node` when the binary already existed. Remote tracing also emitted ANSI escapes into the SIGTERM log, making exact grep checks brittle.
...
The native-devnet-host validation slice closed a real deployment-evidence gap: remote formal-core now executes the current checkout, and the deployed release node was rebuilt/restarted and observed mining with wallet submission and restart/sync smokes green. This is validation/harness hardening, not a new cryptographic theorem.
