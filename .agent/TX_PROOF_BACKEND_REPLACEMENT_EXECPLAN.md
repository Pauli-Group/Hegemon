...
This still does not make SmallWood the default shipped backend, but it materially changes the state of the branch. The current candidate statement now has both an exact no-grinding `128-bit` note and a real semantic arithmetization over the native witness surface, the active integrated packed frontend projects to an exact serialized proof envelope of `108772` bytes while the passing release roundtrip emits `108780` proof bytes, and the warmed focused release runtimes are now about `3.7s` directly and `4.1s` through the wrapped `tx_leaf` seam. That is about `3.25x` smaller than the shipped `354081`-byte tx-proof baseline, still below the `524288`-byte native `tx_leaf` cap, and no longer stuck in the mid-teens on runtime. What remains open is product hardening and further geometry reduction toward the frozen `934`-row target, not proof-size feasibility.
