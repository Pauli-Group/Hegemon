...
- [x] (2026-04-08T23:34Z) Integrated `DirectPacked64V1` as a real alternate proof envelope instead of a dead engine mode. The engine now serializes a direct raw-witness payload bound to the transcript material, the frontend wrapper now carries the explicit arithmetization tag, and the verifier routes bridge vs direct mode honestly through the normal `TransactionProof` seam. Focused tests measured about `32109` bytes for the direct payload versus `106941` projected / `106949` actual bytes for the compact bridge path, so the shipped candidate backend remains on `Bridge64V1`.
