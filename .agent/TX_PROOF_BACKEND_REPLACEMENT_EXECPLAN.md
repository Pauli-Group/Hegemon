...
- [x] (2026-04-08T23:34Z) Integrated `DirectPacked64V1` as a real alternate proof envelope instead of a dead engine mode. The engine now serializes a direct packed witness-carrying payload bound to the transcript material, the frontend wrapper now carries the explicit arithmetization tag, and the verifier routes bridge vs direct mode honestly through the normal `TransactionProof` seam. Focused tests measured `478389` bytes for the direct payload versus `106941` projected / `106949` actual bytes for the compact bridge path, so the shipped candidate backend remains on `Bridge64V1`.
...
This still does not make SmallWood the default shipped backend, but it materially changes the state of the branch. The current candidate statement now has both an exact no-grinding `128-bit` note and a real semantic arithmetization over the native witness surface, the active integrated packed frontend projects to an exact serialized proof envelope of `106940` bytes while the passing release roundtrip emits `106948` proof bytes, and the warmed focused release runtimes are now about `1.74s` directly and `1.98s` through the wrapped `tx_leaf` seam. That is about `3.31x` smaller than the shipped `354081`-byte tx-proof baseline, still below the `524288`-byte native `tx_leaf` cap, and no longer stuck in the multi-second-high-single-digit range on runtime. What remains open is product hardening and further geometry reduction toward the frozen `934`-row target, not proof-size feasibility.
