...
- Observation: the scalar semantic fallback was useful for proving the relation, but it killed the SmallWood proof-size story.
  Evidence: the scalar fallback proved the real native relation but landed around `2.8 MB` because `packing_factor = 1` left `55,526` witness rows and forced the proof to ship three opened evaluation vectors over essentially the whole scalar witness. The later packed Rust candidate first cut the exact serialized envelope to `242124` bytes under the stale narrower note, then regressed to the wider `2982`-row bridge, and now lands at `224220` projected / `224228` actual proof bytes on the corrected `1430`-row integrated bridge, below the shipped Plonky3 proof and below the native `tx_leaf` cap.
...
This still does not make SmallWood live. The current candidate statement now has both an exact no-grinding `128-bit` note and a real semantic arithmetization over the native witness surface, and the active integrated packed frontend now projects to an exact serialized proof envelope of `224220` bytes while the passing release roundtrip emits `224228` proof bytes. That is below both the shipped `354081`-byte tx-proof baseline and the `524288`-byte native `tx_leaf` cap. What remains open is further geometry reduction toward the frozen `934`-row target and product hardening, not proof-size feasibility.
