...
- Observation: the `~86.84 kB/tx` raw result was a bench-mode mismatch, not a proof-format rewrite. Dev builds compile the tx circuit against the fast FRI profile (`FRI_LOG_BLOWUP = 3`, `FRI_NUM_QUERIES = 8`), while the frozen archive used release-mode production parameters (`4`, `32`).
  Evidence: `circuits/transaction-core/src/p3_config.rs` gates the fast profile behind `debug_assertions`; `output/prover-recovery/2026-03-14/merge-root-active/raw-compat.json` restores the release fingerprint to `tx_proof_bytes_avg = 352790`.
- Observation: the anchor-history mismatch exposed a second real bug in the commitment path. The commitment AIR parsed only `33` base public values and skipped the two kernel-root commitments that the prover emitted, which shifted the nullifier region and caused the debug row-0 constraint panic plus the release `OodEvaluationMismatch`.
