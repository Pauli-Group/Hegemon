...
`circuits/superneo-backend-lattice` now implements a direct in-repo SuperNeo-style folding backend over Goldilocks behind one explicit parameter object, `NativeBackendParams`. The active rewrite family is `goldilocks_128b_rewrite` with fingerprint `f5144a6f510c37ab3bef9782f71cfdd71051b3e68d8e9e1843ae73537deaa42efc13eac13cfa0d8afac6814307068663`, `security_bits = 128`, `ring_profile = GoldilocksCyclotomic24`, `matrix_rows = 8`, `matrix_cols = 8`, `challenge_bits = 63`, `fold_challenge_count = 5`, `max_fold_arity = 2`, `transcript_domain_label = "hegemon.superneo.fold.v3"`, `decomposition_bits = 8`, `opening_randomness_bits = 256`, `commitment_assumption_bits = 128`, `max_commitment_message_ring_elems = 513`, and `max_claimed_receipt_root_leaves = 128`. Its public manifest now exposes `spec_label = "hegemon.superneo.native-backend-spec.goldilocks-128b-rewrite.v2"`, `commitment_scheme_label = "neo_class_linear_commitment_128b_masking"`, `challenge_schedule_label = "quint_goldilocks_fs_challenge_negacyclic_mix"`, and `maturity_label = "rewrite_candidate"`. The exact current protocol surface is frozen in [docs/crypto/native_backend_spec.md](docs/crypto/native_backend_spec.md), and benchmark JSON now emits the corresponding `spec_digest` so archived measurements can be tied to one exact protocol contract. The older `heuristic_goldilocks_baseline` remains in tree only as a frozen comparison constructor. The fold transcript is no longer the killed single-challenge line: setup now rejects parameter sets whose advertised security target exceeds the configured challenge schedule and composition bound, the backend derives and verifies five independent 63-bit Goldilocks challenges, and the parameter fingerprint plus `spec_digest` now cover the manifest together with `fold_challenge_count`, `max_fold_arity`, transcript-domain settings, `commitment_assumption_bits`, `max_commitment_message_ring_elems`, and `max_claimed_receipt_root_leaves` so future drift there cannot silently change backend behavior. The method is still experimental and not the paper’s exact production commitment scheme, but it is no longer a pure mock. The current steps are:
...
For the live experimental `ReceiptRoot` path, import now consumes standalone native `TxLeaf` artifacts instead of decoding inline tx proof bytes on that lane. The verifier reconstructs the expected packed witness from the on-chain tx public view plus the serialized STARK public inputs and hidden opening carried inside the tx-leaf artifact, checks that reconstructed witness against the explicit commitment opening object, rejects noncanonical randomness seeds for the configured entropy bound, rejects mismatched `spec_digest` values, verifies the randomized commitment under the manifest-owned parameter set, derives the ordered `TxStatementBinding`s from those verified leaves, and only then accepts the folded receipt root. The older bridge path that still starts from inline tx proofs is benchmarked as `verified_tx_receipt`, but it now exists only as a comparison lane. The native artifact path no longer pays packed-witness transport in its byte accounting; instead it pays for the canonical hidden-witness opening plus the explicit commitment opening. Import hardening now adds exact size rejection for native tx-leaf and receipt-root artifacts plus a reusable verified-native-leaf store keyed by native artifact hash. The old native `ReceiptRoot` verifier remains intentionally linear so the benchmark has a baseline. A second experimental import adapter, `receipt_accumulation`, wraps the native receipt-root bytes together with the ordered native leaf hashes; when those leaves were already verified during candidate preparation or a previous import attempt, consensus can resolve them from the verified-leaf store and re-check the folded root without replaying every hidden-witness opening. Cached authoring reuse for that lane now re-runs the same verified-leaf prewarm before it returns a stored accumulation payload, so a cached payload cannot outlive the verifier-local store state it still depends on. Authoring also emits explicit native-lane selection logs when `receipt_root` is requested, distinguishing `native_artifacts_unavailable`, `native_verifier_profile_mismatch`, and `native_artifact_validation_failed` before it falls back to `InlineTx`, and the prove-ahead cache deliberately stores only successful native receipt-root outcomes so a temporary fallback does not pin that candidate set to `InlineTx`. `.agent/NATIVE_BACKEND_128B_SECURITY_PACKAGE_EXECPLAN.md` is now complete again after the redteam pass: the repo now has the exact spec in [docs/crypto/native_backend_spec.md](docs/crypto/native_backend_spec.md), the code-derived claim model in [docs/crypto/native_backend_security_analysis.md](docs/crypto/native_backend_security_analysis.md), the concrete break ledger in [docs/crypto/native_backend_attack_worksheet.md](docs/crypto/native_backend_attack_worksheet.md), the constant-time note plus timing harness in [docs/crypto/native_backend_constant_time.md](docs/crypto/native_backend_constant_time.md) and [tools/native-backend-timing](/Users/pldd/Projects/Reflexivity/Hegemon/tools/native-backend-timing), a fixed `11`-case vector bundle in [testdata/native_backend_vectors](/Users/pldd/Projects/Reflexivity/Hegemon/testdata/native_backend_vectors), an independent verifier in [tools/native-backend-ref](/Users/pldd/Projects/Reflexivity/Hegemon/tools/native-backend-ref) that no longer calls the production verification helpers, local and CI fuzz-smoke coverage, and a reproducible external-review package under [audits/native-backend-128b](/Users/pldd/Projects/Reflexivity/Hegemon/audits/native-backend-128b) plus its checksum file [package.sha256](/Users/pldd/Projects/Reflexivity/Hegemon/audits/native-backend-128b/package.sha256). The packaged bundle now carries `code_fingerprint.json`, which records `HEAD`, tracked/staged diff hashes, untracked file hashes, and a composite `worktree_fingerprint` instead of pretending a dirty working tree is a clean commit. Benchmark JSON for the canonical lane now includes both `native_backend_params` manifest fields and `native_security_claim`, and `cargo run -p superneo-bench -- --print-native-security-claim` prints the current claim directly without running a benchmark. `HEGEMON_REQUIRE_NATIVE=1` still fail-closes authoring/import by rejecting non-canonical selectors, rejecting `InlineTx` fallback outcomes, and rejecting non-canonical receipt-root payloads on import. The old closure verdict still applies to `heuristic_goldilocks_baseline`, whose code-derived claim bottoms out at `16` bits and whose review state is now `killed`. The active rewrite family now reports `claimed_security_bits = 128`, `transcript_soundness_bits = 157`, `opening_hiding_bits = 128`, `commitment_codomain_bits = 4032`, `commitment_same_seed_search_bits = 36936`, `commitment_random_matrix_bits = 0`, `commitment_binding_bits = 128`, `composition_loss_bits = 7`, `soundness_floor_bits = 128`, and `review_state = candidate_under_review` on the canonical surface. Its assumption ids are `random_oracle.blake3_fiat_shamir`, `serialization.canonical_native_artifact_bytes`, `fs.quint_goldilocks_negacyclic_fold_challenges`, `opening.canonical_256b_mask_seed`, and `commitment.neo_class_linear_binding`. That is the current honest split: the raw bounded-message random-matrix term is `0`, so the live `128`-bit floor still depends on the explicit `commitment_assumption_bits = 128` input rather than geometry alone. The archived current-tree release benchmark [native_tx_leaf_receipt_root_security_package_redteamfix_20260327.json](/Users/pldd/Projects/Reflexivity/Hegemon/.agent/benchmarks/native_tx_leaf_receipt_root_security_package_redteamfix_20260327.json) records a byte curve of `18,137..18,620 B/tx`. The wall-clock verify numbers remain host/load-sensitive and are deliberately left in the archive JSON instead of frozen into prose. The security-package closure still resolves `KEEP`, but only with the explicit qualifier that the backend remains under external review and still is not a paper-equivalent Neo/SuperNeo implementation. It uses small-ring negacyclic arithmetic and a manifest-owned Neo-class linear commitment with canonical 256-bit masking, but it still does not implement Neo/SuperNeo’s full Module-SIS commitment analysis, decomposition reduction, and sum-check machinery.
