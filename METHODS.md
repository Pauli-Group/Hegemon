...
The witness method is now split in three. The legacy synthetic receipt relation still exists for narrow crate-level tests. The active inner relation is now `TxLeafPublicRelation`: it binds a canonical tx-validity receipt to a fixed-width public transaction view (nullifiers, commitments, ciphertext hashes, balance tag, version binding) plus serialized STARK public inputs. Builders derive that witness from real `TransactionProof` objects and re-verify those proofs before emitting experimental artifacts. The older bridge lane, `verified_tx_receipt`, now folds that same public tx-leaf relation directly from inline tx proofs so it remains comparable to the proof-ready-leaf lane. This is still not a native AIR-to-CCS port or a proof-of-STARK-verification relation, but it is no longer “digests folded over digests” without a real public transaction object.
