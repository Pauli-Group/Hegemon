...
* In aggregation mode + `SelfContained`, proofless transfers require a valid `submit_proven_batch` in the same block; there is no consensus fallback to per-transaction proof verification.
