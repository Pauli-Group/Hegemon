...
Under the current exact no-grinding `128-bit` profile (`rho = 2`, `nb_opened_evals = 3`, `beta = 2`, `decs_nb_evals = 32768`, `decs_nb_opened_evals = 24`, `decs_eta = 3`, zero grinding bits), the exact serialized proof envelope for the current packed candidate now projects in-repo to `100956` bytes, and the passing release roundtrip emits `100956` proof bytes. That is about `3.51x` smaller than the legacy `354081`-byte Plonky3 proof while still staying below the current `524288`-byte native `tx_leaf` cap. The checked profile sweep in `docs/crypto/tx_proof_smallwood_profile_sweep.json` shows that this `32768 / 24 / 3` DECS point is the smallest passing point in the realistic no-grinding grid tested for both the live bridge and the compact-binding branch. The latest focused local release roundtrip on the current host completes in about `6.1s` directly in `transaction-circuit`, while the compact-binding branch finishes in about `5.4s`. The verifier now fail-closes on exact inner proof shapes, enforces distinct DECS opening indices, binds the full PCS commitment transcript into the PIOP transcript, hashes full opened combis for the DECS challenge, rejects bridge/direct opening-mode mismatches explicitly, rejects mismatched wrapper-vs-serialized public inputs before replay, and fails closed when a SmallWood wrapper cannot be decoded for arithmetization/profile binding. Proof-specific verifier-profile digests now bind the actual SmallWood arithmetization tag instead of assuming bridge mode, while the public `tx_leaf` bridge consumes the canonical receipt profile carried by the statement rather than trying to recompute nonexistent proof metadata from the tx view alone. This keeps the later SmallWood release cryptography swap local to one backend-specific byte format while leaving the folding layer unchanged, and it is no longer a witness-carrying mock or a geometry-proxy binding story. The new experimental compact-binding variant already proves and verifies under the same release profile and now measures `99828` release bytes, recorded in `docs/crypto/tx_proof_smallwood_compact_bindings_size_report.json`. That confirms the duplicated public-binding rows were real waste, but it still only buys about `1128` bytes (`~1.1%`) over the new live default, so larger structural work is still required.
