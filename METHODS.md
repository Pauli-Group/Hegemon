...
* `crypto::ml_kem` – wraps ML-KEM-1024 with `MlKemKeyPair`, `MlKemPublicKey`, and `MlKemCiphertext`. Public keys and ciphertexts are 1568 bytes, expanded compatibility secret keys are 3168 bytes, and shared secrets are 32 bytes. The deterministic seed-taking API exists for KATs and reproducible wrappers; protocol callers must feed it secret OS randomness and bind transcripts only through HKDF/info or AEAD AAD. The legacy network secure-channel KDF preimage labels, role-to-key mapping, nonce layout, and counter overflow behavior are represented in Lean and checked by generated vectors against the production `network` helpers.
