...
The native block-artifact binding admission table is represented separately in Lean: generated vectors check decoded tx-leaf/action nullifier, commitment, ciphertext-hash, version, and ciphertext payload-hash agreement, plus candidate artifact DA-root, tx statement-commitment, and verified recursive state-root agreement before a block can import or replay shielded transfer effects from a recursive artifact.
