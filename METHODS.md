...
For scalability, the system supports a per-block “aggregation mode” that allows shielded transfer extrinsics to omit the per-transaction proof bytes and rely on a single aggregation proof checked during block import. On the fresh-chain 0.10.0 shipped path those per-transaction proof bytes are native `tx_leaf` artifacts, not the older inline STARK payloads:
...
Aggregation payloads remain experimental block artifacts, not the shipped 0.10.0 transaction-validity path. Block producers can still include proof material via a `submit_proven_batch` payload (legacy runtime encoding: `BlockProofBundle`, fresh-testnet public vocabulary: `CandidateArtifact`), and those payloads still carry explicit proof modes such as `InlineTx`, `MergeRoot`, `ReceiptRoot`, and the now-disabled `FlatBatches` recovery history. But the fresh-chain product path is simpler: `consensus::types::Block` carries ordered `tx_validity_artifacts` plus an optional `block_artifact`, and when no experimental block artifact is present the shipped path verifies the ordered native `TxLeaf` artifacts directly during import. The legacy `InlineTx` label therefore survives only as a compatibility name for the “no extra block artifact, direct per-transaction verification” mode; on the fresh-chain shipped path those per-transaction artifacts are native `TxLeaf` bytes, not legacy inline STARK proofs.

`MergeRoot` remains an explicit recursion experiment bound to tree metadata + leaf manifest commitment (`HEGEMON_MERGE_ARITY`, defaulting to `HEGEMON_AGG_MERGE_FANIN`, so `2` on that experimental lane unless overridden). `ReceiptRoot` remains the native linear block-artifact baseline for explicit experiments: on that lane the block carries standalone native `TxLeaf` artifacts plus a folded receipt-root payload, and import derives the ordered statement bindings from those verified leaves rather than decoding inline tx proof bytes. `receipt_accumulation` is still the warm-store experiment layered on that same native receipt-root topology. A previously attempted `receipt_decider` selector was removed after review because it embedded public per-tx proof objects and replayed linear per-leaf verification on cold import. A later `receipt_residual_diag` selector and bench relation were also removed after the decision-grade benchmark gate: the prototype proved a receipt-only no-replay wiring shape, but it did not deliver a usable cold-import win and therefore no longer ships in consensus, node authoring, or `superneo-bench`. `FlatBatches` remains only for the legacy witness-batch STARK proof kind and the benchmarked recovery history; the proof-bytes `TxProofManifest` wrapper lost locally and is now dead, so `HEGEMON_BLOCK_PROOF_MODE=flat` falls back to `inline_tx`.

Canonical transaction-validity receipts are derived from a tx statement hash, proof digest, serialized public-input digest, and verifier-profile digest, and the experimental receipt-root adapters derive and re-check that exact receipt object during import. Generic consensus/node code now reaches that backend only through neutral receipt-root helper APIs, not by importing `superneo-*` crates directly. Node authoring still uses an asynchronous prover coordinator (`node/src/substrate/prover_coordinator.rs`) that runs a bounded worker queue and exposes open prover-market work packages through additive RPC methods (`prover_getWorkPackage`, `prover_submitWorkResult`, `prover_getWorkStatus`, `prover_getMarketParams`, `prover_getStageWorkPackage`, `prover_submitStageWorkResult`, `prover_getStagePlanStatus`) plus reusable-artifact discovery helpers (`prover_listArtifactAnnouncements`, `prover_getCandidateArtifact`). Work packages are bounded by payload-size caps, per-source limits, per-package submission limits, and expiry windows. The recursive external-worker topology, stage planner, and merge-root layout helpers remain in-tree for experimentation, but they are not the shipped transaction-validity path for the fresh-chain release.
...
Block-proof payload compatibility is now hard-cut to schema `2` + proof format id `5` in active import logic. Nodes reject malformed or legacy payload versions fail-closed. Import treats the optional block artifact as a neutral `(proof_kind, verifier_profile, bytes)` object even when the on-chain payload still carries the legacy `proof_mode` selector for compatibility. The fresh-chain 0.10.0 shipped import path is direct native transaction-artifact verification: consensus verifies the commitment proof when a block carries one, recomputes `tx_statements_commitment` from the canonical transaction statement hashes, then verifies the ordered native `TxLeaf` artifacts directly and rejects bad ordering, bad openings, commitment mismatches, duplicate/nullifier conflicts, or malformed/non-native proof bytes. The legacy `InlineTx` selector name therefore now denotes the “no experimental block artifact, direct ordered tx-artifact verification” lane, not a promise that the per-transaction bytes are old inline STARK proofs.

`ReceiptRoot` remains the native linear block-artifact baseline on that same import surface. When explicitly selected, import verifies the ordered native `TxLeaf` artifacts, reconstructs the native witness from the on-chain tx object plus the serialized public inputs and hidden opening carried in each artifact, deterministically derives the canonical tx-validity receipts and `TxStatementBinding`s from those verified leaves, checks that the resulting statement-hash list commits to the block’s expected `tx_statements_commitment`, and only then accepts the folded receipt-root artifact. The experimental `receipt_accumulation` kind still sits beside that baseline rather than replacing it. Import still checks the same ordered receipt list and folded native root, but the artifact bytes also carry the ordered native artifact hashes. If those leaves are already in the verified-native-leaf store, the accumulation verifier resolves them by hash and verifies the folded root from the stored commitment rows instead of replaying every hidden-witness opening. If the store is cold and there are no proof-carrying tx artifacts to warm it, the verifier fails closed. Authoring now applies the same rule on prove-ahead cache hits: a cached accumulation payload is reusable only after current native tx artifacts have re-warmed the verified-leaf store.

The removed `receipt_decider` branch remains relevant only as a negative result: a self-contained lane that embeds all per-tx proof objects and replays per-leaf verification is not a decider and does not satisfy the cold-import goal. The removed `receipt_residual_diag` branch is the second negative result: a receipt-only no-replay wrapper around the current aggregation backend still lost the release benchmark gate and was deleted. `receipt_arc_whir` remains a diagnostic native-leaf-backed residual lane on that same import surface, not a promoted product path. `FlatBatches` import validation still exists only for the old witness-batch STARK proof kind and local/trusted history because the proof-bytes `TxProofManifest` recovery prototype lost to raw tx-proof shipping at every measured `k` (`1,2,4,8`), so import rejects that proof kind and `HEGEMON_BLOCK_PROOF_MODE=flat` falls back to `inline_tx`.

Shared merge-root layout helpers still compute fan-in, arity, tree levels, and the leaf manifest commitment for consensus, node planning, and the benchmark harness. The benchmark harness still distinguishes the canonical native leaf baseline plus explicit diagnostics: `raw_shipping` is the frozen transport baseline; `raw_active` is the legacy inline-proof comparison lane; `merge_root_active` is aggregation plus commitment; `verified_tx_receipt` is the older bridge experiment built from real inline tx proofs plus a folded receipt-root artifact using the same `TxLeafPublicRelation`; `native_tx_validity` is the lower-level witness diagnostic lane; `native_tx_leaf_receipt_root` is the decision-grade native leaf baseline; and `native_tx_leaf_receipt_arc_whir` is a diagnostic residual lane over the same native artifacts. It reports common active-path timing/byte fields across those lanes, carries `edge_prepare_ns`, `peak_rss_bytes`, and an `import_comparison` block for the canonical native lane. Those numbers are current implementation measurements for the experimental topology and native relation, not final claims about paper-faithful Neo/SuperNeo security. Product rollout now starts from the direct native tx-artifact path; block-proof selectors such as `merge_root`, `receipt_root`, `receipt_accumulation`, and `receipt_arc_whir` remain explicit experiments while that cryptographic gap is still being closed.
