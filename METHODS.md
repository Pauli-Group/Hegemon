...
The witness method is now split in two. The legacy synthetic receipt relation still exists for narrow crate-level tests, but the active `ReceiptRoot` experiment has moved to a verified inline-tx receipt relation. In that path, `circuits/superneo-hegemon` derives canonical tx-validity receipts from real `TransactionProof` objects, re-verifies those proofs when building leaf assignments, and then packs the receipt witness lanes with declared bit widths so the experiment can measure pay-per-bit pressure directly in Goldilocks terms. This is still not a native AIR-to-CCS port, but it is no longer “digests folded over digests” without real tx-proof checks.
...
For the live experimental `ReceiptRoot` path, import now keeps the inline tx proof artifacts and verifies those proofs before validating the folded receipt root. That makes the benchmark and consensus behavior honest: the mode now measures “inline tx proofs plus receipt-root compression” rather than a receipt-only shortcut. The backend remains an in-repo approximation rather than the paper’s exact module ring: it uses small-ring negacyclic arithmetic and an Ajtai-style linear commitment, but it still does not implement Neo/SuperNeo’s full Module-SIS commitment analysis, decomposition reduction, or sum-check machinery.
