...
The repository now wires this design into executable modules. The `state/merkle` crate implements an append-only `CommitmentTree` that precomputes default subtrees, stores per-level node vectors, and exposes efficient `append`, `extend`, and `authentication_path` helpers. It uses the same Poseidon-style hashing domain as the transaction circuit, ensuring leaf commitments and tree updates are consistent with the ZK statement. `TransactionProof::verify` rejects missing STARK proof bytes/public inputs in production builds. On top of that, the `circuits/block` crate treats commitment proofs as the default: `CommitmentBlockProver` builds a `CommitmentBlockProof` that commits to transaction statement hashes via a Poseidon sponge (`tx_statements_commitment`), and consensus verifies the commitment proof alongside per-transaction input checks. In `InlineRequired`, blocks without an aggregation proof fall back to parallel per-transaction verification. In `SelfContained` aggregation mode, proofless transfers remain fail-closed (missing proven-batch payload is invalid), while proof-carrying transfers can still use inline verification if no ready bundle is attached. For one-time recovery from legacy segments produced before this invariant was enforced, operators can set `HEGEMON_ACCEPT_LEGACY_PROOFLESS_BLOCKS=1` during catch-up; import will accept proofless blocks missing `submit_proven_batch` but intentionally skips commitment/aggregation proof verification for those blocks. Recursive epoch proofs remain removed; aggregation proofs are the only recursion path in the live system today.

Aggregation proofs are produced from transaction proof bytes alone, never from spend witnesses. Block producers include aggregation + commitment proof material through a single `submit_proven_batch` payload (internal type: `BlockProofBundle`, external call name unchanged). The reference aggregation prover lives in `circuits/aggregation` and emits postcard-encoded `BatchProof` bytes that are embedded inside the bundle payload. Node authoring uses an asynchronous prover coordinator (`node/src/substrate/prover_coordinator.rs`) that runs a bounded worker queue and now also exposes open prover-market work packages through additive RPC methods (`prover_getWorkPackage`, `prover_submitWorkResult`, `prover_getWorkStatus`, `prover_getMarketParams`, `prover_getStageWorkPackage`, `prover_submitStageWorkResult`, `prover_getStagePlanStatus`). Work packages are bounded by payload-size caps, per-source limits, per-package submission limits, and expiry windows. Candidate scheduling is deterministic and geometric, with checkpointed upsizing by default: when liveness is enabled and queue capacity is at least 2, the coordinator climbs a planned ladder derived from `HEGEMON_BATCH_TARGET_TXS` (for example `1/2/4/8/.../target`) instead of scheduling every `+1` mempool increment. This avoids repeatedly building new heavy recursion shapes for adjacent tx counts while still preserving a singleton liveness lane. Operators can restore the legacy per-step behavior with `HEGEMON_BATCH_INCREMENTAL_UPSIZE=1`, or disable liveness entirely with `HEGEMON_PROVER_LIVENESS_LANE=0` to force strict large-batch proving. In strict throughput mode (`HEGEMON_PROVER_LIVENESS_LANE=0`), the coordinator defers scheduling until a full `HEGEMON_BATCH_TARGET_TXS` candidate is available, avoiding expensive intermediate proving jobs. `HEGEMON_PROVER_ADAPTIVE_LIVENESS_MS` is now opt-in (default disabled), so singleton fallback is only enabled when operators explicitly choose that tradeoff. When liveness mode is enabled and a full target batch arrives while a smaller batch is still proving, the coordinator rolls generation and republishes the larger work package so stale in-flight candidates do not pin scheduling on undersized batches. `HEGEMON_AGG_STAGE_LOCAL_PARALLELISM` (`HEGEMON_PROVER_WORKERS` fallback) now accepts `0` to disable in-node proving while still publishing prover-market work packages (for external prover-only deployments). Queue depth/inflight stage controls are configurable via `HEGEMON_AGG_STAGE_QUEUE_DEPTH` (`HEGEMON_BATCH_QUEUE_CAPACITY` fallback) and `HEGEMON_PROVER_STAGE_MAX_INFLIGHT_PER_LEVEL`. Before proving or block assembly, shielded candidate sets are sanitized to remove binding-hash duplicates and nullifier-conflicting transfers so block builders do not waste proving work on mutually-invalid spend sets. The coordinator can overscan the pool and then trim to target size using `HEGEMON_BATCH_CANDIDATE_OVERSCAN_FACTOR` (default `4`) so conflict filtering still leaves a dense candidate set. If `HEGEMON_BATCH_TARGET_TXS` is not explicitly set, node startup caps the coordinator default to `min(HEGEMON_MAX_BLOCK_TXS, HEGEMON_BATCH_DEFAULT_TARGET_TXS)` with `HEGEMON_BATCH_DEFAULT_TARGET_TXS=32` by default, which avoids pathological “always proving huge stale candidates” behavior on fresh nodes. During block assembly, proofless transfers are filtered against ready proven batches and only the ready subset is included; non-ready proofless transfers are deferred without halting block sealing. Bundle preparation now performs a single shared preprocessing pass per candidate (`decode + transfer extraction + statement hash derivation + DA encoding`) and then runs commitment-proof and aggregation-proof generation in parallel against that shared context. The node logs explicit attribution metrics (`context_stage_ms`, `commitment_stage_ms`, `aggregation_stage_ms`) so throughput runs can isolate bottlenecks instead of reporting only end-to-end timings. Consensus aggregation-verifier cache builds are now singleflight keyed by `(tx_count, pub_inputs_len, proof_shape)` so concurrent import workers do not stampede into duplicate cold cache builds for the same recursion shape. Prover-side aggregation caching is intentionally split: full recursion entries remain thread-local (the circuit object is not thread-safe), while the expensive `CommonData` preprocessing layer is process-global singleflight keyed by the same shape so concurrent workers reuse one cold `commit_preprocessed` build instead of duplicating it per worker thread. Aggregation circuit cache entries can be proactively built on first-proof shape discovery with `HEGEMON_AGG_PREWARM_MAX_TXS`; when it is unset, the prover now prewarms at least `HEGEMON_BATCH_TARGET_TXS` by default to remove first-batch cold-start stalls in strict mode. Commitment-proof jobs now seed from runtime compact tree snapshots (frontier + root history) rather than replaying all historical commitments per job, so proving setup stays bounded as height grows. The mining loop refreshes block templates when either the parent hash changes or the selected pending-transaction set changes, so newly prepared batches are not starved behind stale empty work.
...
Aggregation payload compatibility is now hard-cut to V4 in active import logic (`consensus::aggregation`): payloads must carry `proof_format=4`, tree metadata (`tree_arity/tree_levels/root_level`), and a deterministic `shape_id` bound to `(slot_tx_count, public-input shape, FRI shape)`. Nodes reject malformed or legacy payload versions fail-closed. Import treats payload `tx_count` as recursion `slot_tx_count` (not block tx count), requires `slot_tx_count >= block_tx_count`, and enforces bounded padding (`slot_tx_count <= block_tx_count * 16`) so shape stabilization cannot be abused as an unbounded amplification vector. Statement commitment derivation decodes all slot public inputs but commits only the active `block_tx_count` prefix, preserving block semantics while allowing padded recursion shapes for cache stability. Operators can tune recursion behavior with `HEGEMON_AGG_TREE_ARITY`, `HEGEMON_AGG_LEVEL_PARALLELISM`, `HEGEMON_AGG_WARMUP_TARGET_SHAPES`, `HEGEMON_AGG_FIXED_SLOT_COUNT`, `HEGEMON_AGG_STAGE_QUEUE_DEPTH`, `HEGEMON_AGG_STAGE_LOCAL_PARALLELISM`, `HEGEMON_PROVER_STAGE_MAX_INFLIGHT_PER_LEVEL`, and optional cache persistence (`HEGEMON_AGG_CACHE_PERSIST=1`, `HEGEMON_AGG_CACHE_DIR=...`). Prover-market work packages now include stage metadata (`stage_type`, `level`, `arity`, `shape_id`, `dependencies`) so external provers can coordinate deterministic recursion-stage work scheduling; root packages expose explicit dependency IDs for the previous recursion level (`leaf_verify` or `merge`) derived from a deterministic arity-aware DAG, and stage logs surface `HEGEMON_PROVER_STAGE_MEM_BUDGET_MB` as the configured per-stage memory budget hint.
