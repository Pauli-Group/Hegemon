...
* bind each leaf to its statement digest and witness-commitment digest in a compact leaf proof,
* fold commitments with transcript-derived linear challenges to produce parent commitments and parent statement digests.

For the live experimental `ReceiptRoot` path, import now consumes standalone native `TxLeaf` artifacts instead of decoding inline tx proof bytes on that lane. The verifier reconstructs the expected packed witness from the on-chain tx public view plus the serialized STARK public inputs and hidden opening carried inside the tx-leaf artifact, re-commits that witness under the backend key, checks that commitment against the compact tx-leaf proof, derives the ordered `TxStatementBinding`s from those verified leaves, and only then accepts the folded receipt root. The older bridge path that still starts from inline tx proofs is benchmarked as `verified_tx_receipt`, but it now exists only as a comparison lane. The native artifact path no longer pays packed-witness transport in its byte accounting; instead it pays for the canonical hidden-witness opening itself. Import hardening now adds exact size rejection for native tx-leaf and receipt-root artifacts plus a reusable verified-native-leaf store keyed by native artifact hash. The old native `ReceiptRoot` verifier remains intentionally linear so the benchmark has a baseline. A second experimental import adapter, `receipt_accumulation`, wraps the native receipt-root bytes together with the ordered native leaf hashes; when those leaves were already verified during candidate preparation or a previous import attempt, consensus can resolve them from the verified-leaf store and re-check the folded root without replaying every hidden-witness opening. Cached authoring reuse for that lane now re-runs the same verified-leaf prewarm before it returns a stored accumulation payload, so a cached payload cannot outlive the verifier-local store state it still depends on. Authoring also emits explicit native-lane selection logs when `receipt_root` is requested, distinguishing `native_artifacts_unavailable`, `native_verifier_profile_mismatch`, and `native_artifact_validation_failed` before it falls back to `InlineTx`, and the prove-ahead cache deliberately stores only successful native receipt-root outcomes so a temporary fallback does not pin that candidate set to `InlineTx`. The backend remains an in-repo approximation rather than the paper’s exact module ring: it uses small-ring negacyclic arithmetic and an Ajtai-style linear commitment, but it still does not implement Neo/SuperNeo’s full Module-SIS commitment analysis, decomposition reduction, or sum-check machinery, so the branch does not yet justify a production 128-bit PQ security claim.
