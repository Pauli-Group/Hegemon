...
Under the current exact no-grinding `128-bit` profile (`rho = 2`, `nb_opened_evals = 3`, `beta = 2`, `decs_nb_evals = 16384`, `decs_nb_opened_evals = 29`, `decs_eta = 3`, zero grinding bits), the exact serialized proof envelope for the current packed candidate now projects in-repo to `108012` bytes, and the passing release roundtrip emits `108012` proof bytes. That is about `3.28x` smaller than the shipped `354081`-byte Plonky3 proof while still staying below the current `524288`-byte native `tx_leaf` cap. The latest focused local release runs complete in about `1.93s` directly in `transaction-circuit` and about `3.30s` through the wrapped `superneo-hegemon` `tx_leaf` seam. The verifier now fail-closes on exact inner proof shapes, enforces distinct DECS opening indices, binds the full PCS commitment transcript into the PIOP transcript, hashes full opened combis for the DECS challenge, rejects bridge/direct opening-mode mismatches explicitly, rejects mismatched wrapper-vs-serialized public inputs before replay, and fails closed when a SmallWood wrapper cannot be decoded for arithmetization/profile binding. Proof-specific verifier-profile digests now bind the actual SmallWood arithmetization tag instead of assuming bridge mode, while the public `tx_leaf` bridge consumes the canonical receipt profile carried by the statement rather than trying to recompute nonexistent proof metadata from the tx view alone. This keeps the later SmallWood release cryptography swap local to one backend-specific byte format while leaving the folding layer unchanged, but the method is still experimental. It is no longer a witness-carrying mock or a geometry-proxy binding story.
