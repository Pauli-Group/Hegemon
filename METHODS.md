...
The native block-artifact binding admission table is represented separately in Lean: generated vectors check decoded tx-leaf/action nullifier, commitment, ciphertext-hash, version, and ciphertext payload-hash agreement, plus candidate artifact DA-root, tx statement-commitment, and verified recursive state-root agreement before a block can import shielded transfer effects from a recursive artifact.
...
The native `hegemon-node` process starts from a fresh native genesis, persists native block and shielded-state metadata in `sled`, mines PoW development blocks, syncs over the Hegemon PQ service, and keeps the wallet/app JSON-RPC method names stable. New native work must flow through `ConsensusBlock` validation, sled-backed native state, PQ sync, checked supply-digest advancement, and the same JSON-RPC compatibility surface. The release node does not link classical TLS client/server stacks: RPC is served as plaintext HTTP/WebSocket for loopback or a trusted operator control plane, and remote administrative access must be protected outside the process by a PQ-safe transport or host-level access policy.
...
- The proven-batch binding kernel proves the complete executable acceptance predicate for route compatibility, tx-count binding, statement-commitment binding, DA-root binding, nonzero DA chunks, block-artifact envelope kind/profile agreement, and recursive-block receipt-root exclusion with `Hegemon.Consensus.ProvenBatchBinding.accepts_iff_binding_preconditions`; companion accept/reject theorems pin representative rows. `gen_proven_batch_binding_vectors` emits examples that Rust `evaluate_proven_batch_binding` must match.
- `formal/lean` is the pinned Lean 4 project for actual machine-checked Hegemon kernels. The bridge replay kernel proves duplicate rejection for staging and importing inbound replay keys with `Hegemon.Bridge.stage_prevents_duplicate_pending`, `Hegemon.Bridge.import_prevents_reimport`, and `Hegemon.Bridge.import_prevents_restaging`; `gen_bridge_vectors` emits executable examples that the Rust `protocol-kernel` bridge encoding and `InboundReplayState` helper must match. The consensus fork-choice kernel proves deterministic two-tip ordering with `Hegemon.Consensus.higher_work_wins`, `Hegemon.Consensus.lower_work_loses`, `Hegemon.Consensus.equal_work_higher_height_wins`, `Hegemon.Consensus.equal_work_lower_height_loses`, `Hegemon.Consensus.equal_work_height_uses_hash_order`, and `Hegemon.Consensus.same_tip_not_better`; `gen_consensus_vectors` emits examples that the Rust `consensus::fork_choice` helper must match. The consensus PoW-admission kernel proves compact-target rejection, strict timestamp admission, hash-threshold rejection, and fixed-width Work48 cumulative-work behavior with `Hegemon.Consensus.compactTarget_rejects_zero_mantissa`, `Hegemon.Consensus.compactTarget_rejects_large_exponent`, `Hegemon.Consensus.timestamp_rejects_parent_equal`, `Hegemon.Consensus.timestamp_rejects_median_equal`, `Hegemon.Consensus.timestamp_rejects_future_skew`, `Hegemon.Consensus.checkedWorkAdd_ok`, `Hegemon.Consensus.checkedWorkAdd_rejects_overflow`, `Hegemon.Consensus.powAdmission_rejects_height`, `Hegemon.Consensus.powAdmission_rejects_pow_bits`, `Hegemon.Consensus.powAdmission_rejects_insufficient_work`, and `Hegemon.Consensus.powAdmission_accepts_valid`; `gen_pow_vectors` emits examples that Rust `consensus::pow` and `consensus-light-client` helpers must match. The consensus header-preimage kernel proves signing/full-header byte grammar with `Hegemon.Consensus.Header.signingPreimage_independent_of_auth_payloads` and the companion length/tag theorems; `gen_header_vectors` emits exact byte preimages that Rust `BlockHeader::signing_preimage_v1` and `BlockHeader::full_header_preimage_v1` must match. The bridge long-range kernel proves verifier-hash binding, message-count binding, header/MMR length shape, trusted/message/tip height ordering, selected message source, sample height/opening-index agreement, confirmation policy, tip-work policy, and claimed-output agreement with `Hegemon.Bridge.LongRange.valid_shape_accepts` and the companion rejection theorems; `gen_bridge_long_range_vectors` emits examples that Rust `consensus-light-client::evaluate_long_range_proof_shape` must match. The consensus proof-policy kernel proves empty-block proof-payload rejection, non-empty self-contained proof-shape requirements, legacy InlineRequired rejection, recursive commitment-proof-byte rejection, and recursive block-artifact requirements through `Hegemon.Consensus.empty_clean_accepts`, `Hegemon.Consensus.empty_with_payload_rejects`, `Hegemon.Consensus.nonempty_requires_tx_artifacts`, `Hegemon.Consensus.nonempty_rejects_inline_required`, `Hegemon.Consensus.recursive_rejects_commitment_proof_bytes`, `Hegemon.Consensus.recursive_requires_block_artifact`, and `Hegemon.Consensus.recursive_complete_accepts`, while generated examples check production `evaluate_block_proof_policy` behavior used by `ParallelProofVerifier`. The recursive semantic-input kernel proves nonempty/nullifier/DA admission, rejection precedence, and source binding for expected statement commitment, parent/applied commitment-tree roots, kernel roots, nullifier root, DA root, header message root, and recursive tree-state commitments with `Hegemon.Consensus.RecursiveSemanticInputs.semantic_accepts_iff_preconditions`, `Hegemon.Consensus.RecursiveSemanticInputs.valid_semantic_fields_match_sources`, and companion rejection theorems; `gen_recursive_semantic_input_vectors` emits examples that Rust `recursive_block_semantic_inputs_from_block` must match. The aggregation V5 header kernel proves version/format/encoding admission, statement-commitment binding, child/fan-in bounds, subtree tx-count binding, tree-level/root-level checks, packed public-input length agreement, and the binary-minimum merge fan-in rule with `Hegemon.Consensus.AggregationV5.valid_leaf_singleton_header_accepts`, `Hegemon.Consensus.AggregationV5.valid_merge_header_accepts`, and companion rejection theorems; `gen_aggregation_v5_vectors` emits examples that Rust `consensus::aggregation::v5` header admission must match. The supply-accounting kernel proves checked supply-digest increase/decrease, underflow/overflow rejection, no-coinbase native no-op behavior, and checked native coinbase advance with `Hegemon.Consensus.increaseSupplyDigest_ok`, `Hegemon.Consensus.increaseSupplyDigest_rejects_overflow`, `Hegemon.Consensus.decreaseSupplyDigest_ok`, `Hegemon.Consensus.decreaseSupplyDigest_rejects_underflow`, `Hegemon.Consensus.nativeSupplyDelta_without_coinbase`, `Hegemon.Consensus.advanceNativeSupplyDigest_checked`, and `Hegemon.Consensus.advanceNativeSupplyDigest_rejects_supply_overflow`; `gen_supply_vectors` emits examples that Rust `CoinbaseData::net_native_delta`, `consensus::reward::update_supply_digest`, and native checked supply helpers must match. The native action-ordering kernel proves that transfer-key subsequences must be nondecreasing and that non-transfer actions are ignored with `Hegemon.Native.empty_canonical`, `Hegemon.Native.single_transfer_canonical`, `Hegemon.Native.non_transfer_ignored`, `Hegemon.Native.ordered_pair_accepts`, `Hegemon.Native.descending_pair_rejects`, and `Hegemon.Native.equal_pair_accepts`; `gen_action_order_vectors` emits examples that the Rust native transfer-order helper must match. The native action hash/count admission kernel proves count agreement, embedded action-hash agreement, duplicate-hash rejection, and rejection precedence with `Hegemon.Native.ActionHashAdmission.accepts_iff_admission_preconditions` and companion theorems; `gen_action_hash_admission_vectors` emits examples that Rust `evaluate_native_action_hash_admission` must match. The native action scope admission kernel proves candidate-artifact payload scoping, bridge/candidate/coinbase state-delta scope, candidate payload presence, supported route selection, transfer public metadata shape, and rejection precedence with `Hegemon.Native.ActionScopeAdmission.accepts_iff_scope_preconditions` and companion theorems; `gen_action_scope_admission_vectors` emits examples that Rust `evaluate_native_action_scope_admission` must match. The native transfer action payload admission kernel proves decoded proof presence/size caps, anchor and commitment agreement, inline ciphertext byte caps, ciphertext metadata agreement, binding-hash agreement, fee agreement, and rejection precedence with `Hegemon.Native.TransferActionPayloadAdmission.accepts_iff_payload_preconditions` and companion theorems; `gen_transfer_action_payload_admission_vectors` emits examples that Rust `evaluate_native_transfer_payload_admission` must match. The native coinbase-accounting admission kernel proves at-most-one coinbase, no-coinbase fee-burn acceptance, checked subsidy-plus-fee reward admission, fee/reward overflow rejection, missing amount rejection, reward mismatch rejection, and rejection precedence with `Hegemon.Native.CoinbaseAccountingAdmission` theorems; `gen_coinbase_accounting_admission_vectors` emits examples that Rust `evaluate_native_coinbase_accounting_admission` must match. The native coinbase action payload-admission kernel proves nonzero reward amount, commitment agreement/nonzero checks, ciphertext byte caps, ciphertext hash/size metadata agreement, and rejection precedence with `Hegemon.Native.CoinbaseActionPayloadAdmission` theorems; `gen_coinbase_action_payload_admission_vectors` emits examples that Rust `evaluate_native_coinbase_action_payload_admission` must match. The native candidate-artifact admission kernel proves no state-delta smuggling, payload presence, schema agreement, tx-count and DA-count bounds, shipped recursive_block_v2 route/profile, empty legacy proof fields, absent receipt-root payload, nonempty recursive proof bytes, proof byte caps, and rejection precedence with `Hegemon.Native.CandidateArtifactAdmission` theorems; `gen_candidate_artifact_admission_vectors` emits examples that Rust `evaluate_native_candidate_artifact_admission` must match. The native block commitment-admission kernel proves tx-count, state-root, kernel-root, nullifier-root, action-root, bridge message-root/count, header-MMR root/length, and supply-digest agreement with `Hegemon.Native.BlockCommitmentAdmission.accepts_iff_commitment_preconditions` and companion rejection/precedence theorems; `gen_block_commitment_admission_vectors` emits examples that Rust `evaluate_native_block_commitment_admission` must match. The native tx-leaf artifact kernel proves bounded byte-parser facts for serialized STARK/public-tx counts, proof-length caps, commitment row/coeff caps, backend-byte defaults, bad backend IDs, trailing bytes, and truncation with `Hegemon.Native.TxLeafArtifact.valid_artifact_parses`, `Hegemon.Native.TxLeafArtifact.missing_backend_defaults_to_current_backend`, `Hegemon.Native.TxLeafArtifact.legacy_missing_backend_defaults_to_plonky3`, and the companion rejection theorems; `gen_native_tx_leaf_artifact_vectors` emits examples that Rust `superneo-hegemon` decoding and canonical re-encoding must match. The native receipt-root kernel proves bounded parser and structural schedule facts for leaf/fold caps, non-empty expected leaf agreement, `fold_count = leaf_count - 1`, exact fold challenge count, exact parent-row dimensions, trailing bytes, and truncation with `Hegemon.Native.ReceiptRoot.valid_single_parses`, `Hegemon.Native.ReceiptRoot.two_leaf_schedule_accepts`, `Hegemon.Native.ReceiptRoot.three_leaf_schedule_accepts`, and the companion rejection theorems; `gen_native_receipt_root_vectors` emits examples that Rust `superneo-hegemon` decoding, canonical re-encoding, and structural validation must match. The transaction-balance kernel proves valid balances have concrete slots, slot overflow is rejected, the native slot delta equals `fee - value_balance`, and stablecoin/non-native conservation rules are enforced with `Hegemon.Transaction.validBalance_has_slots`, `Hegemon.Transaction.validBalance_rejects_slot_overflow`, `Hegemon.Transaction.validBalance_native_delta`, and `Hegemon.Transaction.validBalance_stablecoin_rules`; `gen_transaction_vectors` emits examples that Rust `TransactionWitness::balance_slots` and `TransactionWitness::validate` must match. The transaction Merkle-path kernel proves empty-path behavior, even/odd position-bit orientation, computed-root acceptance, wrong-length rejection, and wrong-root rejection with `Hegemon.Transaction.foldPath_empty`, `Hegemon.Transaction.foldPath_even_step`, `Hegemon.Transaction.foldPath_odd_step`, `Hegemon.Transaction.verifyPath_accepts_computed_root`, `Hegemon.Transaction.verifyPath_rejects_wrong_length`, and `Hegemon.Transaction.verifyPath_rejects_wrong_root`; `gen_merkle_vectors` emits examples that the Rust `MerklePath` fold helper must match. The transaction public-input shape kernel proves fixed vector widths, boolean flags, inactive-field zeroing, active nullifier/commitment nonzero checks, nonempty transaction admission, canonical balance-slot asset ordering, and stablecoin asset membership with `Hegemon.Transaction.PublicInputs.validPublicInputShape_accepts_valid` and the companion rejection theorems; `gen_public_input_vectors` emits examples that Rust `TransactionPublicInputsP3::validate` must match. The transaction public-input binding kernel proves public/serialized field agreement for merkle root, fee, signed value balance, balance-slot assets, stablecoin policy, oracle, and attestation payload with `Hegemon.Transaction.PublicInputBinding.bindPublicInputs_accepts_valid` and the companion rejection theorems; `gen_public_input_binding_vectors` emits examples that Rust `transaction_public_inputs_p3_from_parts` must match. The transaction statement-hash kernel proves domain, padding, little-endian scalar, raw stablecoin flag, and signed balance preimage facts with `Hegemon.Transaction.StatementHash.statementPreimage_accepts_valid` and the companion rejection theorems; `gen_statement_hash_vectors` emits exact preimage bytes that the shared Rust statement-hash helper must match. The shielded nullifier kernel proves zero, pending, and spent nullifier rejection with `Hegemon.Shielded.stage_rejects_zero`, `Hegemon.Shielded.import_rejects_zero`, `Hegemon.Shielded.stage_prevents_duplicate_pending`, `Hegemon.Shielded.import_prevents_reimport`, and `Hegemon.Shielded.import_prevents_restaging`; `gen_shielded_vectors` emits examples that the Rust `protocol-shielded-pool::NullifierState` helper must match. This does not prove pending_action_hash collision resistance, hash derivation, BLAKE2 action-order key derivation, complete bridge light-client soundness, SHA-256 implementation equivalence, monetary policy economics, retarget economics, aggregation STARK soundness, recursive proof cryptographic soundness, native lattice backend security, native tx-leaf proof soundness, native receipt-root cryptographic fold soundness, full network/finality behavior, note commitment correctness, Poseidon2 Merkle hash security or implementation equivalence, nullifier derivation, ciphertext hash derivation, BLAKE3 statement-hash collision resistance or implementation equivalence, full AIR/proof-system soundness, or full native-node equivalence.
...
- The native block-artifact binding admission kernel proves deterministic equality gates after coupling and decoding: tx-leaf/action nullifier, commitment, ciphertext-hash, version, and ciphertext payload-hash agreement, plus candidate DA-root, tx statement-commitment, and verified recursive state-root agreement with `Hegemon.Native.BlockArtifactBindingAdmission.tx_leaf_action_accepts_iff_preconditions` and `Hegemon.Native.BlockArtifactBindingAdmission.candidate_artifact_binding_accepts_iff_preconditions`. `gen_block_artifact_binding_admission_vectors` emits examples that Rust `evaluate_native_tx_leaf_action_binding_admission` and `evaluate_native_candidate_artifact_binding_admission` must match before native block artifact effects are accepted.
...
- The consensus version-policy kernel proves initial bindings, activation/retirement boundary behavior, same-height retirement precedence, duplicate initial binding deduplication, and first unsupported transaction ordering with `Hegemon.Consensus.versionPolicy_initial_accepts` and the companion rejection theorems. `gen_version_policy_vectors` emits examples that Rust `VersionSchedule::validate_versions` checks on the PoW and BFT import paths. This does not prove social upgrade governance, manifest correctness, per-version proof-system soundness, or native-node equivalence.
