...
* Block template assembly now waits for a ready proven batch before giving up on pending proofless transfers, but the retry loop is bounded to avoid OOM under stalled proofless load. `HEGEMON_PENDING_PROVEN_BATCH_WAIT_MS` overrides the total wait budget (default follows `HEGEMON_BATCH_JOB_TIMEOUT_MS`, fallback `30000` ms) and `HEGEMON_PENDING_PROVEN_BATCH_MAX_ATTEMPTS` caps retry attempts (default `20`) with exponential backoff.
