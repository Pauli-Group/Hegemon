...
For the live experimental `ReceiptRoot` path, import now consumes standalone `TxLeaf` artifacts instead of decoding inline tx proof bytes on that lane. The verifier reconstructs the expected packed witness from the on-chain tx public view plus the serialized STARK public inputs carried inside the tx-leaf artifact, re-commits that witness under the backend key, checks that commitment against the compact tx-leaf proof, derives the ordered `TxStatementBinding`s from those verified leaves, and only then accepts the folded receipt root. The older bridge path that still starts from inline tx proofs is benchmarked as `verified_tx_receipt`, but it now reuses the same public tx-leaf relation rather than a separate digest-only bridge relation. The new native witness lane, `native_tx_validity`, is benchmark-only for now: because the backend still verifies leaves by recomputing commitments from an expected packed witness, its byte accounting currently includes packed witness transport instead of a compact hidden-witness opening. The backend remains an in-repo approximation rather than the paper’s exact module ring: it uses small-ring negacyclic arithmetic and an Ajtai-style linear commitment, but it still does not implement Neo/SuperNeo’s full Module-SIS commitment analysis, decomposition reduction, or sum-check machinery.
