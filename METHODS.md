...
The witness method is now split in four. The legacy synthetic receipt relation still exists for narrow crate-level tests. `TxLeafPublicRelation` is the active public bridge: it binds a canonical tx-validity receipt to a fixed-width public transaction view (nullifiers, commitments, ciphertext hashes, balance tag, version binding) plus serialized STARK public inputs. Builders derive that witness from real `TransactionProof` objects and re-verify those proofs before emitting experimental artifacts. The older bridge lane, `verified_tx_receipt`, now folds that same public tx-leaf relation directly from inline tx proofs so it remains comparable to the native lane. Beside that, `NativeTxValidityRelation` is the real witness-driven path. It takes `TransactionWitness` directly, validates witness semantics plus Merkle membership, derives the canonical public-input object without going through a Plonky3 proof, and then feeds that witness into a native `TxLeaf` artifact format with a compact hidden-witness opening. That native `TxLeaf -> ReceiptRoot` topology is now the only decision-grade benchmark surface on this branch; the raw `native_tx_validity` lane remains a lower-level diagnostic only, and `superneo-bench` now refuses to run any diagnostic lane unless the caller passes `--allow-diagnostic-relation`.

`circuits/superneo-backend-lattice` now implements a direct in-repo SuperNeo-style folding backend over Goldilocks behind one explicit parameter object, `NativeBackendParams`. The active family is now `goldilocks_128b_structural_commitment` with fingerprint `c24ea2de5d61afbe99ccc1befeb7eea3df8ada33965369f22ff220fa377078ef68ce6179a0769e6db2202a989f5eb559`, `spec_digest = 5c11c4456ae0492ecefca5301e1d76816ec55283d9e4b697818f8d0a4d67dc67`, `security_bits = 128`, `ring_profile = GoldilocksCyclotomic24`, `matrix_rows = 74`, `matrix_cols = 8`, `challenge_bits = 63`, `fold_challenge_count = 5`, `max_fold_arity = 2`, `transcript_domain_label = "hegemon.superneo.fold.v3"`, `decomposition_bits = 8`, `opening_randomness_bits = 256`, `commitment_assumption_bits = 0`, `derive_commitment_binding_from_geometry = true`, `max_commitment_message_ring_elems = 513`, and `max_claimed_receipt_root_leaves = 128`. Its manifest exposes `spec_label = "hegemon.superneo.native-backend-spec.goldilocks-128b-structural-commitment.v3"`, `commitment_scheme_label = "bounded_message_random_matrix_commitment"`, `challenge_schedule_label = "quint_goldilocks_fs_challenge_negacyclic_mix"`, and `maturity_label = "structural_candidate"`. The exact current protocol surface is frozen in [docs/crypto/native_backend_spec.md](docs/crypto/native_backend_spec.md), and benchmark JSON emits both the `spec_digest` and the code-derived claim so archived measurements can be tied to one exact protocol contract. The older `heuristic_goldilocks_baseline` and `goldilocks_128b_rewrite` families remain in tree only as frozen historical baselines. The fold transcript is still the five-challenge Goldilocks line, but the commitment side is no longer assumption-fed on the active family: setup now rejects parameter sets whose advertised security target exceeds the configured challenge schedule, geometry-derived binding floor, or composition bound, and the parameter fingerprint plus `spec_digest` cover the manifest together with `fold_challenge_count`, `max_fold_arity`, transcript-domain settings, `commitment_assumption_bits`, `derive_commitment_binding_from_geometry`, `max_commitment_message_ring_elems`, and `max_claimed_receipt_root_leaves` so future drift there cannot silently change backend behavior. The method is still experimental and not the paper’s exact production commitment scheme, but it is no longer a pure mock or an assumption-fed binding story. The current steps are:
...
* commit to the resulting ring vector with a deterministic Ajtai-style public matrix over that ring plus an explicit randomized mask derived from a per-artifact seed,
* carry a native commitment opening object that includes the packed witness, the randomness seed, and an opening digest so malformed openings, wrong randomness, and mixed parameter sets fail verification directly,
* bind each leaf to its statement digest, witness-commitment digest, and opening digest in a compact native leaf proof,
...
For the live experimental `ReceiptRoot` path, import now consumes standalone native `TxLeaf` artifacts instead of decoding inline tx proof bytes on that lane. The verifier reconstructs the expected packed witness from the on-chain tx public view plus the serialized STARK public inputs and hidden opening carried inside the tx-leaf artifact, checks that reconstructed witness against the explicit commitment opening object, rejects noncanonical randomness seeds for the configured entropy bound, rejects mismatched `spec_digest` values, verifies the randomized commitment under the manifest-owned parameter set, derives the ordered `TxStatementBinding`s from those verified leaves, and only then accepts the folded receipt root. The older bridge path that still starts from inline tx proofs is benchmarked as `verified_tx_receipt`, but it now exists only as a comparison lane. The native artifact path no longer pays packed-witness transport in its byte accounting; instead it pays for the canonical hidden-witness opening plus the explicit commitment opening. Import hardening now adds exact size rejection for native tx-leaf and receipt-root artifacts plus a reusable verified-native-leaf store keyed by native artifact hash. The old native `ReceiptRoot` verifier remains intentionally linear so the benchmark has a baseline. A second experimental import adapter, `receipt_accumulation`, wraps the native receipt-root bytes together with the ordered native leaf hashes; when those leaves were already verified during candidate preparation or a previous import attempt, consensus can resolve them from the verified-leaf store and re-check the folded root without replaying every hidden-witness opening. Cached authoring reuse for that lane now re-runs the same verified-leaf prewarm before it returns a stored accumulation payload, so a cached payload cannot outlive the verifier-local store state it still depends on. Authoring also emits explicit native-lane selection logs when `receipt_root` is requested, distinguishing `native_artifacts_unavailable`, `native_verifier_profile_mismatch`, and `native_artifact_validation_failed` before it falls back to `InlineTx`, and the prove-ahead cache deliberately stores only successful native receipt-root outcomes so a temporary fallback does not pin that candidate set to `InlineTx`. `.agent/NATIVE_BACKEND_128B_SECURITY_PACKAGE_EXECPLAN.md` is now historical package-scaffolding record, and `.agent/STRUCTURAL_COMMITMENT_GEOMETRY_EXECPLAN.md` records the structural commitment rewrite that replaced the assumption-fed active family. The repo still has the exact spec in [docs/crypto/native_backend_spec.md](docs/crypto/native_backend_spec.md), the code-derived claim model in [docs/crypto/native_backend_security_analysis.md](docs/crypto/native_backend_security_analysis.md), the concrete break ledger in [docs/crypto/native_backend_attack_worksheet.md](docs/crypto/native_backend_attack_worksheet.md), the constant-time note plus timing harness in [docs/crypto/native_backend_constant_time.md](docs/crypto/native_backend_constant_time.md) and [tools/native-backend-timing](/Users/pldd/Projects/Reflexivity/Hegemon/tools/native-backend-timing), the fixed `11`-case vector bundle in [testdata/native_backend_vectors](/Users/pldd/Projects/Reflexivity/Hegemon/testdata/native_backend_vectors), the independent verifier in [tools/native-backend-ref](/Users/pldd/Projects/Reflexivity/Hegemon/tools/native-backend-ref), local and CI fuzz-smoke coverage, and the reproducible external-review package under [audits/native-backend-128b](/Users/pldd/Projects/Reflexivity/Hegemon/audits/native-backend-128b) plus its checksum file [package.sha256](/Users/pldd/Projects/Reflexivity/Hegemon/audits/native-backend-128b/package.sha256). The packaged bundle still carries `code_fingerprint.json`, which records `HEAD`, tracked/staged diff hashes, untracked file hashes, and a composite `worktree_fingerprint` instead of pretending a dirty working tree is a clean commit. Benchmark JSON for the canonical lane still includes both `native_backend_params` manifest fields and `native_security_claim`, and `cargo run -p superneo-bench -- --print-native-security-claim` prints the current claim directly without running a benchmark. `HEGEMON_REQUIRE_NATIVE=1` still fail-closes authoring/import by rejecting non-canonical selectors, rejecting `InlineTx` fallback outcomes, and rejecting non-canonical receipt-root payloads on import. The old closure verdict still applies to `heuristic_goldilocks_baseline`, whose code-derived claim bottoms out at `16` bits and whose review state is now `killed`. The active structural family now reports `claimed_security_bits = 128`, `transcript_soundness_bits = 157`, `opening_hiding_bits = 128`, `commitment_codomain_bits = 37296`, `commitment_same_seed_search_bits = 36936`, `commitment_random_matrix_bits = 360`, `commitment_binding_bits = 360`, `composition_loss_bits = 7`, `soundness_floor_bits = 128`, and `review_state = candidate_under_review` on the canonical surface. Its assumption ids are `random_oracle.blake3_fiat_shamir`, `serialization.canonical_native_artifact_bytes`, `fs.quint_goldilocks_negacyclic_fold_challenges`, `opening.canonical_256b_mask_seed`, and `commitment.bounded_message_random_matrix_union_bound`. That is the current honest split: the active `128`-bit floor now comes from the geometry-derived commitment term instead of the old `commitment_assumption_bits` override, but it pays materially larger artifacts because the matrix grew to `74 x 8`. The archived current-tree release benchmark [native_tx_leaf_receipt_root_structural_commitment_20260328.json](/Users/pldd/Projects/Reflexivity/Hegemon/.agent/benchmarks/native_tx_leaf_receipt_root_structural_commitment_20260328.json) records a byte curve of `22,625..27,561 B/tx`. The wall-clock verify numbers remain host/load-sensitive and are deliberately left in the archive JSON instead of frozen into prose. The security-package closure still resolves `KEEP`, but only with the explicit qualifier that the backend remains under external review and still is not a paper-equivalent Neo/SuperNeo implementation. It uses small-ring negacyclic arithmetic and a bounded-message random-matrix commitment with canonical 256-bit masking, but it still does not implement Neo/SuperNeo’s full Module-SIS commitment analysis, decomposition reduction, and sum-check machinery.
...
Aggregation payloads are produced from transaction proof bytes alone on the shipping path, never from spend witnesses. Block producers include proof material via a single `submit_proven_batch` payload (legacy runtime encoding: `BlockProofBundle`, fresh-testnet public vocabulary: `CandidateArtifact`). The active hard-cut payload is schema `2` with proof format id `5` and explicit proof modes: `InlineTx`, `MergeRoot`, `ReceiptRoot`, and the now-disabled `FlatBatches` recovery history. Every payload now also carries an explicit backend-neutral `(proof_kind, verifier_profile)` pair beside the legacy `proof_mode`, and consensus routes verification through a verifier registry keyed by artifact kind instead of wiring raw Plonky3 checks directly into import. `consensus::types::Block` now carries `tx_validity_artifacts` plus an optional `block_artifact` envelope rather than a raw `transaction_proofs` vector. `InlineTx` is the live low-TPS lane: the artifact carries the commitment proof plus block-level metadata, while the tx-validity artifacts embed the canonical ordered tx proofs. `MergeRoot` remains an experimental recursive mode bound to tree metadata + leaf manifest commitment (`HEGEMON_MERGE_ARITY`, defaulting to `HEGEMON_AGG_MERGE_FANIN`, so `2` on that experimental lane unless overridden). `ReceiptRoot` is the first additive neutral artifact kind over proof-ready tx-validity artifacts: on the experimental lane the block now carries standalone native `TxLeaf` tx artifacts, and import derives the ordered statement bindings from those verified leaves rather than decoding inline tx proof bytes. That lane is now native-only: if authoring cannot materialize native `TxLeaf` artifacts for the candidate set, block production falls back to `InlineTx` instead of mixing in the older bridge profile, and the service logs whether it used the native lane or fell back with an explicit reason. Receipt-root-family selectors are now split by self-containment. `receipt_root` and `receipt_accumulation` still require embedded proof bytes for every transfer because they are not cold-self-contained. A second explicit selector, `HEGEMON_BLOCK_PROOF_MODE=receipt_accumulation`, reuses the same legacy `ReceiptRoot` payload field but switches `proof_kind` / `verifier_profile` to a custom accumulation adapter. Its artifact bytes are a small wrapper around the folded native receipt-root bytes plus the ordered native leaf hashes, which lets import hit the verified-native-leaf store before it touches any hidden witness data. Cached prove-ahead reuse for that selector now re-runs verified-leaf prewarm from the current native tx artifacts before it returns a stored accumulation payload, so a cache hit cannot reuse bytes whose required verifier-local store state has gone stale. A previously attempted `receipt_decider` selector was removed after review because it embedded public per-tx proof objects and replayed linear per-leaf verification on cold import. A later `receipt_residual_diag` selector and bench relation were also removed after the decision-grade benchmark gate: the prototype proved a receipt-only no-replay wiring shape, but it did not deliver a usable cold-import win and therefore no longer ships in consensus, node authoring, or `superneo-bench`. `FlatBatches` remains only for the legacy witness-batch STARK proof kind and the benchmarked recovery history; the proof-bytes `TxProofManifest` wrapper lost locally and is now dead, so `HEGEMON_BLOCK_PROOF_MODE=flat` falls back to `inline_tx`. Canonical transaction-validity receipts are derived from a tx statement hash, proof digest, serialized public-input digest, and verifier-profile digest, and the experimental receipt-root adapters derive and re-check that exact receipt object during import. Generic consensus/node code now reaches that backend only through neutral receipt-root helper APIs, not by importing `superneo-*` crates directly. Node authoring uses an asynchronous prover coordinator (`node/src/substrate/prover_coordinator.rs`) that runs a bounded worker queue and exposes open prover-market work packages through additive RPC methods (`prover_getWorkPackage`, `prover_submitWorkResult`, `prover_getWorkStatus`, `prover_getMarketParams`, `prover_getStageWorkPackage`, `prover_submitStageWorkResult`, `prover_getStagePlanStatus`) plus reusable-artifact discovery helpers (`prover_listArtifactAnnouncements`, `prover_getCandidateArtifact`). Work packages are bounded by payload-size caps, per-source limits, per-package submission limits, and expiry windows. The live raw/inline-tx mode does not publish any external proving work at all: it schedules only the parent-bound finalize/commitment job and requires proof-ready transactions with canonical inline tx proofs already attached. The recursive external-worker topology, stage planner, and merge-root layout helpers remain in-tree for experimentation, and they still use one canonical stage namespace plus parent-independent expensive work ids, but they are no longer the default authoring path. Before proving or block assembly, shielded candidate sets are sanitized to remove binding-hash duplicates while retaining nullifier-overlap candidates for runtime-side conflict resolution. The coordinator can overscan and trim to target size using `HEGEMON_BATCH_CANDIDATE_OVERSCAN_FACTOR` (default `4`) so conflict filtering still leaves a dense candidate set. If `HEGEMON_BATCH_TARGET_TXS` is unset, startup caps the default to `min(HEGEMON_MAX_BLOCK_TXS, HEGEMON_BATCH_DEFAULT_TARGET_TXS)` with `HEGEMON_BATCH_DEFAULT_TARGET_TXS=32`. During block assembly, proofless transfers are now a `merge_root` experiment gated by `HEGEMON_AGGREGATION_PROOFS`; the live `inline_tx` lane skips proofless transfers entirely. The service-level authoring selector resolves `HEGEMON_BLOCK_PROOF_MODE` aliases immediately into `(proof_kind, verifier_profile, legacy_mode)` before choosing a build path, so future backends can reuse the same routing surface. Bundle preparation performs a single shared preprocessing pass per candidate (`decode + transfer extraction + statement hash derivation + DA encoding`) and then runs the commitment stage against that shared context; when `merge_root`, `receipt_root`, or `receipt_accumulation` is selected explicitly, the parent-independent payload stage runs in parallel, but in `inline_tx` mode the aggregation stage is a no-op. When a native receipt-root selector is requested, that shared preparation also emits a structured native-lane selection report so operators and harnesses can tell whether block authoring used native `TxLeaf` artifacts or fell back to `InlineTx`, and whether the fallback came from unavailable artifacts, a verifier-profile mismatch, or native artifact validation failure. Successful native `ReceiptRoot` payloads participate in the prove-ahead cache; `InlineTx` fallback outcomes for that selector do not, so a transient miss cannot poison the candidate set. Prove-ahead remains active across block transitions: import success immediately re-runs candidate scheduling for the new parent so proving for block `N+1` starts while `N` is already sealed/propagating. The throughput harness exports the three honest comparison surfaces now in use: `raw_shipping` for the frozen tx-proof transport baseline; `raw_active` for canonical tx proofs plus commitment; and `merge_root_active` for recursion plus commitment. It also carries the older `verified_tx_receipt` bridge lane and the lower-level `native_tx_validity` lane as explicit diagnostics, but only `native_tx_leaf_receipt_root` remains decision-grade for future architecture, and the CLI requires `--allow-diagnostic-relation` before it will run the others. The canonical native benchmark now prints an `import_comparison` block that reports baseline linear receipt-root verification beside the accumulation path’s store-prewarm time and warm-verify time for the same ordered leaf set. The harness supports `--cold` / `--warm`, `--raw-only`, and `--skip-merge-root`, logs stage attribution and structured lane failures instead of aborting, and uses the same anchor-history-consistent tx fixture plus full commitment public-input layout as the live node.
...
Update (2026-03-26): `HEGEMON_BLOCK_PROOF_MODE=receipt_arc_whir` is no longer described as a receipt-only cold-import win. After review, the supported implementation now verifies the ordered native `TxLeaf` artifacts first, derives the canonical `TxValidityReceipt` list from those verified leaves, checks the block statement commitment against the verified bindings, and only then verifies the residual ARC/WHIR artifact. `receipt_root` remains the native linear baseline, `receipt_accumulation` remains the warm-store experiment, and `receipt_arc_whir` remains a diagnostic residual lane because cold verification still replays native leaf verification and the residual verifier itself is still linear in receipt count.

Update (2026-03-26): the benchmark CLI's canonical experimental surface is back to `native_tx_leaf_receipt_root`; `native_tx_leaf_receipt_arc_whir` now requires `--allow-diagnostic-relation`. The diagnostic ARC/WHIR benchmark still reports an `import_comparison` object, but its guard metrics are now measured from actual execution tracing instead of hard-coded constants.

Block-proof payload compatibility is now hard-cut to schema `2` + proof format id `5` in active import logic. Nodes reject malformed or legacy payload versions fail-closed. Import now treats the block artifact as a neutral `(proof_kind, verifier_profile, bytes)` object even when the on-chain payload still carries the legacy `proof_mode` selector for compatibility. `InlineTx` is the live import path: consensus verifies the commitment proof, recomputes `tx_statements_commitment` from the canonical transaction-proof statement hashes, then verifies the ordered inline tx validity artifacts directly and rejects mismatched proofs, bad ordering, commitment mismatches, duplicate/nullifier conflicts, or missing inline proof bytes. `ReceiptRoot` remains the native linear baseline on that same path: import verifies the ordered native `TxLeaf` artifacts, reconstructs the native witness from the on-chain tx object plus the serialized STARK public inputs and hidden opening carried in each artifact, deterministically derives the canonical tx-validity receipts and `TxStatementBinding`s from those verified leaves, checks that the resulting statement-hash list commits to the block’s expected `tx_statements_commitment`, and only then accepts the folded receipt-root artifact. The receipt-root verifier now accepts only the native receipt-root verifier profile on that lane; if native leaves are unavailable, authoring/import stay on `InlineTx`. The experimental `receipt_accumulation` kind still sits beside that baseline rather than replacing it. Import still checks the same ordered receipt list and folded native root, but the artifact bytes also carry the ordered native artifact hashes. If those leaves are already in the verified-native-leaf store, the accumulation verifier resolves them by hash and verifies the folded root from the stored commitment rows instead of replaying every hidden-witness opening. If the store is cold and there are no proof-carrying tx artifacts to warm it, the verifier fails closed. Authoring now applies the same rule on prove-ahead cache hits: a cached accumulation payload is reusable only after current native tx artifacts have re-warmed the verified-leaf store. The current import mitigation stack for per-leaf cost is therefore: parallel native-leaf verification, a reusable verified-leaf store keyed by artifact hash, exact size rejection for native artifacts, and the additive `receipt_accumulation` path for warm-cache measurement. The removed `receipt_decider` branch remains relevant only as a negative result: a self-contained lane that embeds all per-tx proof objects and replays per-leaf verification is not a decider and does not satisfy the cold-import goal. The removed `receipt_residual_diag` branch is now the second negative result: a receipt-only no-replay wrapper around the current aggregation backend is still not enough if it loses the release benchmark gate. Its `k=1` release measurement came in worse than the native baseline on both bytes and cold verify (`237,686 B/tx`, `29.2 ms` verify versus `5,481 B/tx`, `15.0 ms` baseline from the same run), and the detached `k=32` release run exited `137` before emitting JSON, so the branch was deleted instead of being promoted. `receipt_arc_whir` is now a diagnostic native-leaf-backed residual lane on that same import surface. It no longer accepts receipt-only tx artifacts, and a cold verification intentionally measures one native-leaf replay per tx while still checking the proof-carried Merkle openings plus each fold relation and keeping `used_old_aggregation_backend = false`. `FlatBatches` import validation still enforces deterministic sorted contiguous coverage of `[0, tx_count)` with no gaps/overlaps and verifies every batch artifact against its covered transaction subset, but it now exists only for the old witness-batch STARK proof kind and local/trusted history because the proof-bytes `TxProofManifest` recovery prototype lost to raw tx-proof shipping at every measured `k` (`1,2,4,8`). On an 8-tx release run the wrapper added roughly `66-69 ms` of extra build time, slightly increased payload bytes (`~355.2 KiB/tx` vs `~354.2 KiB/tx` raw), and did not reduce verification time enough to matter, so import rejects that proof kind and `HEGEMON_BLOCK_PROOF_MODE=flat` falls back to `inline_tx`. `MergeRoot` validation still verifies the root proof plus manifest/tree metadata bindings, and the recursive path still derives dependency ids and worker dispatch from one canonical stage namespace while splitting reusable parent-independent aggregation artifacts from the final parent-bound bundle assembly step. Shared merge-root layout helpers compute fan-in, arity, tree levels, and the leaf manifest commitment for consensus, node planning, and the benchmark harness, which removes a silent divergence risk between those paths. The benchmark harness now distinguishes the canonical native leaf baseline plus explicit diagnostics: `raw_shipping` is the frozen transport baseline; `raw_active` is canonical tx proofs plus the live commitment proof path; `merge_root_active` is aggregation plus commitment; `verified_tx_receipt` is the older bridge experiment built from real inline tx proofs plus a folded receipt-root artifact using the same `TxLeafPublicRelation`; `native_tx_validity` is the lower-level witness diagnostic lane; `native_tx_leaf_receipt_root` is the decision-grade native leaf baseline; and `native_tx_leaf_receipt_arc_whir` is a diagnostic residual lane over the same native artifacts. It reports common active-path timing/byte fields across those lanes, carries `edge_prepare_ns`, `peak_rss_bytes`, and an `import_comparison` block for the canonical native lane. It supports `--cold` / `--warm`, converts merge-root panics or commitment-stage failures into structured lane errors instead of aborting the process, exposes `--raw-only` for release-fingerprint checks, exposes `--skip-merge-root` so `raw_active` can still be archived cleanly once merge-root has already crossed the stall line, and requires `--allow-diagnostic-relation` before it will run non-canonical lanes. The benchmark tx fixture feeds the active lanes from a shared anchor-history tree, and the commitment AIR consumes the full 45-element public-input prefix including the kernel-root commitments. With those honesty fixes in place, the low-TPS comparison has already resolved for recursive merge-root: `raw_active(k=1)` beats `merge_root_active(k=1)` on bytes and active-path latency (`536098 B/tx`, `70812417 ns` prove, `18299167 ns` verify versus `536258 B/tx`, `79701375 ns`, `25680001 ns`), and `merge_root_active(k=2)` fails to clear a `65s` wall-clock budget while `raw_active(k=2)` finishes at `456262 B/tx`, `108371875 ns` active-path prove, and `29954584 ns` active-path verify. The decision-grade native SuperNeo-style release run now records `native_tx_leaf_receipt_root` at `5637/5642/5645/5646 B/tx`, `82791/144250/263541/523041 ns` active-path root prove, `28701375/44856792/74192458/139407375 ns` active-path verify, `25748834/39487791/61555375/113261416 ns` edge leaf preparation, and `252788736/272580608/308215808/378421248` peak RSS bytes for `k=16/32/64/128`. The same run’s `import_comparison` block measured baseline linear receipt-root verification at `28.7/44.9/74.2/139.4 ms`, accumulation prewarm at `18.0/47.1/65.3/130.1 ms`, and warm accumulation verification at `0.24/0.37/0.67/1.36 ms` for those same `k` values. Those are current implementation numbers for the experimental topology and native relation, not final claims about paper-faithful Neo/SuperNeo security. Production still defaults to the raw/inline-tx live block-proof mode until that cryptographic gap is closed and the result is stress-tested more broadly. Operators tune rollout profiles primarily through `HEGEMON_BATCH_SLOT_TXS` (default `16`, then `32`, then `64`) and mode selection via `HEGEMON_BLOCK_PROOF_MODE` (`inline_tx` default, `merge_root` explicit, `receipt_root` experimental, `receipt_accumulation` experimental warm-import prototype, `receipt_arc_whir` diagnostic residual prototype, `flat` forced back to `inline_tx`), while prover-market stage controls remain available for experiments (`HEGEMON_AGGREGATION_PROOFS`, `HEGEMON_AGG_STAGE_QUEUE_DEPTH`, `HEGEMON_AGG_STAGE_LOCAL_PARALLELISM`, `HEGEMON_PROVER_STAGE_MAX_INFLIGHT_PER_LEVEL`, `HEGEMON_PROVER_STAGE_MEM_BUDGET_MB`).
...
Update (2026-03-26): the old receipt-only ARC/WHIR promotion claim is withdrawn. The current targeted cold verification test measures `cold_residual_replayed_leaf_verifications = tx_count` and `cold_residual_used_old_aggregation_backend = false` after the native-artifact anchoring fix, which makes the lane honest but also disqualifies it as a solved cold-import accumulator.

Update (2026-03-26): operator-facing mode selection should now be read as `HEGEMON_BLOCK_PROOF_MODE` = `inline_tx` by default, `merge_root` explicit, `receipt_root` experimental native baseline, `receipt_accumulation` experimental warm-import prototype, `receipt_arc_whir` diagnostic residual prototype, and `flat` forced back to `inline_tx`.
