...
Under the current exact no-grinding `128-bit` profile (`rho = 2`, `nb_opened_evals = 3`, `beta = 2`, `decs_nb_evals = 32768`, `decs_nb_opened_evals = 24`, `decs_eta = 3`, zero grinding bits), the exact serialized proof envelope for the shipped default now projects in-repo to `98532` bytes, and the passing release roundtrip emits `98532` proof bytes. That is about `3.59x` smaller than the legacy `354081`-byte Plonky3 proof, about `2.4%` smaller than the old `100956`-byte bridge baseline, and still below the current `524288`-byte native `tx_leaf` cap. The checked profile sweep in `docs/crypto/tx_proof_smallwood_profile_sweep.json` now covers the live bridge, the older compact-binding branch, and the shipped `DirectPacked64CompactBindingsSkipInitialMdsV1` line, and it shows that this same `32768 / 24 / 3` DECS point is the smallest passing point in the realistic no-grinding grid on all three. The verifier now fail-closes on exact inner proof shapes, enforces distinct DECS opening indices, binds the full PCS commitment transcript into the PIOP transcript, hashes full opened combis for the DECS challenge, rejects bridge/direct opening-mode mismatches explicitly, rejects mismatched wrapper-vs-serialized public inputs before replay, and fails closed when a SmallWood wrapper cannot be decoded for arithmetization/profile binding. Proof-specific verifier-profile digests now bind the actual SmallWood arithmetization tag instead of assuming bridge mode, and the version-only helper is now pinned to the canonical shipped `DirectPacked64CompactBindingsSkipInitialMdsV1` profile so default proving and version-only receipt-profile derivation stay aligned. This keeps the later SmallWood release cryptography swap local to one backend-specific byte format while leaving the folding layer unchanged, and it is no longer a witness-carrying mock or a geometry-proxy binding story. `Bridge64V1` remains a measured baseline at `100956` bytes. `DirectPacked64CompactBindingsV1` still proves and verifies under the same release profile and measures `99828` release bytes, recorded in `docs/crypto/tx_proof_smallwood_compact_bindings_size_report.json`. The shipped default is now `DirectPacked64CompactBindingsSkipInitialMdsV1` at `98532` bytes, recorded in `docs/crypto/tx_proof_smallwood_current_size_report.json`. The repo now also carries a semantic LPPC seam in `circuits/transaction/src/smallwood_lppc_frontend.rs`: it materializes the exact `NativeTxValidityRelation` witness order from `TransactionWitness`, binds the same native statement/public-input digests the shipped tx proof uses, fixes the v1 semantic LPPC window at `4096` elements, and runs those shapes through the current SmallWood structural projection and soundness path. The checked structural frontier in `docs/crypto/tx_proof_smallwood_semantic_lppc_frontier_report.json` is `1024x4=54240`, `512x8=37776`, and `256x16=32712`, all still clearing the no-grinding `128-bit` structural floor. The repo now also has an exact current-engine opening-layer spike over that same witness window in `docs/crypto/tx_proof_smallwood_semantic_lppc_identity_spike_report.json`, and it reproduces those exact bytes in real prove/verify roundtrips. The obvious auxiliary fallback is dead too: `docs/crypto/tx_proof_smallwood_semantic_lppc_auxiliary_poseidon_spike_report.json` shows that carrying the full current Poseidon subtrace as auxiliary witness would balloon the object to `472008 .. 493536` bytes, and the exact `512x8` attempt fail-closes on transcript replay. The final current-backend floor check now lives in `docs/crypto/tx_proof_smallwood_semantic_helper_floor_report.json`: once the nonlinear relation regains the lane-visible helper rows it actually needs on this backend, the best `64x` point rises to `102120` bytes, above the shipped `98532`-byte default proof. That narrows the remaining problem further. The current opening layer is already compatible with the semantic LPPC witness window, the auxiliary-witness escape hatch is dead, and the current opened-row model itself is now the blocker: on this backend, a true semantic adapter appears to lose as soon as the helper-row tax comes back.
...
The repository now wires this design into executable modules. The `state/merkle` crate implements an append-only `CommitmentTree` that precomputes default subtrees, stores per-level node vectors, and exposes efficient `append`, `extend`, and `authentication_path` helpers. It uses the same Poseidon-style hashing domain as the transaction circuit, ensuring leaf commitments and tree updates are consistent with the ZK statement. `TransactionProof::verify` rejects missing STARK proof bytes/public inputs in production builds. On top of that, the shipped block-artifact path now lives in `circuits/block-recursion`: consensus verifies the constant-size recursive block artifact against the canonical ordered verified-leaf stream plus semantic tuple derived from the parent state and block body, and the recursive lane requires empty legacy `commitment_proof` bytes so no linear public-input payload can be smuggled through the old field. The `circuits/block` crate and its `CommitmentBlockProof` remain available only for the explicit native `ReceiptRoot` compatibility lane. The fresh-chain product path no longer accepts `InlineRequired` verification for non-empty shielded blocks, and strict authoring now requires a ready `submit_proven_batch` / `submit_candidate_artifact` payload for shielded candidates instead of falling back to `proven_batch: None`. Recursive epoch proofs remain removed; the live product block-artifact path is `RecursiveBlock`, while `ReceiptRoot` remains an alternate native lane. The experimental tree-reduced `RecursiveBlockV2` lane is now bounded explicitly instead of inheriting the old `v1` proof cap: it uses `TREE_RECURSIVE_CHUNK_SIZE_V2 = 128`, supports at most `1000` txs per recursive artifact version, derives level-specific child proof caps through the canonical tree schedule, and currently serializes to a fixed `1,239,940`-byte outer artifact under that bounded domain. Consensus/runtime admission now accepts explicit `RecursiveBlockV2` payloads by that derived cap while the default shipped mode remains `RecursiveBlockV1`.

Aggregation payloads are now part of the shipped 0.10.0 shielded block path. `consensus::types::Block` still carries ordered `tx_validity_artifacts` plus an optional `block_artifact` to keep the interface neutral, but on the fresh-chain product path any non-empty shielded block must carry the native `recursive_block` artifact and verify under `SelfContainedAggregation`. The old `InlineTx` label survives only as historical vocabulary in archived benchmarks and compatibility-oriented test vectors, not as a live product consensus mode, and `ReceiptRoot` is now the explicit alternate native lane rather than the shipped default.
...
* `pallet_attestations` / `pallet_settlement` – historically stored `StarkVerifierParams` in pallet storage, but the proof-native cut treats these as protocol-release parameters rather than ad-hoc runtime setters. The active default tx proof backend is now `SmallwoodCandidate`, and the version-owned manifest surface in `protocol/versioning` / `runtime/src/manifest.rs` no longer advertises a Plonky3 FRI profile for that default binding. The old fixed-`log_blowup = 4`, `num_queries = 32`, `query_pow_bits = 0` Plonky3 profile remains checked in under `docs/crypto/tx_proof_profile_sweep.json` and `docs/crypto/tx_proof_soundness_analysis.md` as legacy comparison material. The active SmallWood profile note lives in `docs/crypto/tx_proof_smallwood_no_grinding_soundness.md`. The first conservative STIR PCS spike is still checked in under `docs/crypto/tx_proof_stir_spike.json` and `docs/crypto/tx_proof_stir_soundness.md`; on the exact `8192`-row, `2^-4` tx surface, the best release-safe STIR candidate only projects to about `273145` total bytes (`1.30x` shrink), so it is not by itself a compelling reason to revisit the old Plonky3 opening layer. With 384-bit digests, PQ collision resistance reaches ~128 bits.
