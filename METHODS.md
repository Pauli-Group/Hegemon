...
One new distinction is now real in code: `DirectPacked64V1` is no longer just a fail-closed marker. It proves and verifies through the normal proof envelope with an explicit arithmetization tag and a witness-carrying payload. That mode is still not the product default, because the measured direct payload is `478389` bytes versus `106941` projected / `106949` actual bytes for the compact bridge path.
