...
For the live experimental `ReceiptRoot` path, import now consumes standalone native `TxLeaf` artifacts instead of decoding inline tx proof bytes on that lane. The verifier reconstructs the expected packed witness from the on-chain tx public view plus the serialized STARK public inputs and hidden opening carried inside the tx-leaf artifact, checks that reconstructed witness against the explicit commitment opening object, rejects noncanonical randomness seeds for the configured entropy bound, verifies the randomized commitment under the manifest-owned parameter set, derives the ordered `TxStatementBinding`s from those verified leaves, and only then accepts the folded receipt root. The older bridge path that still starts from inline tx proofs is benchmarked as `verified_tx_receipt`, but it now exists only as a comparison lane. The native artifact path no longer pays packed-witness transport in its byte accounting; instead it pays for the canonical hidden-witness opening plus the explicit commitment opening. Import hardening now adds exact size rejection for native tx-leaf and receipt-root artifacts plus a reusable verified-native-leaf store keyed by native artifact hash. The old native `ReceiptRoot` verifier remains intentionally linear so the benchmark has a baseline. A second experimental import adapter, `receipt_accumulation`, wraps the native receipt-root bytes together with the ordered native leaf hashes; when those leaves were already verified during candidate preparation or a previous import attempt, consensus can resolve them from the verified-leaf store and re-check the folded root without replaying every hidden-witness opening. Cached authoring reuse for that lane now re-runs the same verified-leaf prewarm before it returns a stored accumulation payload, so a cached payload cannot outlive the verifier-local store state it still depends on. Authoring also emits explicit native-lane selection logs when `receipt_root` is requested, distinguishing `native_artifacts_unavailable`, `native_verifier_profile_mismatch`, and `native_artifact_validation_failed` before it falls back to `InlineTx`, and the prove-ahead cache deliberately stores only successful native receipt-root outcomes so a temporary fallback does not pin that candidate set to `InlineTx`. The closure pass added one explicit `NativeSecurityEnvelope` and one explicit native-only guard, and the rewrite pass first upgraded the schedule from one challenge to two, then replaced the toy opening path with the current 128-bit commitment-opening regime. Benchmark JSON for the canonical lane now includes both `native_backend_params` manifest fields and `native_security_envelope`, and `HEGEMON_REQUIRE_NATIVE=1` fail-closes authoring/import by rejecting non-canonical selectors, rejecting `InlineTx` fallback outcomes, and rejecting non-canonical receipt-root payloads on import. The old closure verdict still applies to `heuristic_goldilocks_baseline`, whose archived benchmark `.agent/benchmarks/native_tx_leaf_receipt_root_finish_native_proof_line_20260326.json` kept the byte curve inside budget (`18,073..18,552 B/tx`) but reported only `63` soundness bits and missed the verify budget at `k=32` (`274.83 ms > 250 ms`) and `k=64` (`585.35 ms > 500 ms`). The current rewrite family now reports `claimed_security_bits = 128`, `transcript_soundness_bits = 128`, `opening_hiding_bits = 128`, `commitment_binding_bits = 128`, and `soundness_floor_bits = 128` on the canonical surface. The archived release benchmark `.agent/benchmarks/native_tx_leaf_receipt_root_goldilocks_128b_rewrite_20260327.json` kept the byte curve at `18,073..18,572 B/tx` and observed `total_active_path_verify_ns` between roughly `19.82 ms` and `268.95 ms` for `k=1..128` on this host. The backend still remains an in-repo approximation rather than the paper’s exact module ring: it uses small-ring negacyclic arithmetic and a manifest-owned Neo-class linear commitment with canonical 128-bit masking, but it still does not implement Neo/SuperNeo’s full Module-SIS commitment analysis, decomposition reduction, or sum-check machinery. The keep/kill rerun in `.agent/REBUILD_NATIVE_BACKEND_EXECPLAN.md` therefore resolves narrowly: keep this line in tree as the live experimental proof-native candidate, but do not confuse that with a paper-equivalent production backend.
