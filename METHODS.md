...
`circuits/superneo-backend-lattice` now implements a direct in-repo SuperNeo-style folding backend over Goldilocks behind one explicit parameter object, `NativeBackendParams`. The current hardened candidate is `native-backend-v3` with fingerprint `d062b963d3e064a04ba3b2d9acecf17203e3f9afb9ec2608bfc21e9ca58d21d05ce072f5520edbbb197fe35a0c6ff64c`, `security_bits = 63`, `ring_profile = GoldilocksCyclotomic24`, `matrix_rows = 8`, `matrix_cols = 8`, `challenge_bits = 63`, `max_fold_arity = 2`, `transcript_domain_label = "hegemon.superneo.fold.v1"`, `decomposition_bits = 8`, and `opening_randomness_bits = 16`. The fold transcript is now field-limited rather than silently clamped to 16 bits: setup rejects parameter sets whose advertised security target exceeds the configured challenge width, fold challenges use the full configured width up to 63 bits, and the parameter fingerprint now covers the previously ambient `max_fold_arity` and transcript-domain settings so future drift there cannot silently change backend behavior. The method is still experimental and not the paper’s exact production commitment scheme, but it is no longer a pure mock. The current steps are:
...
For the live experimental `ReceiptRoot` path, import now consumes standalone native `TxLeaf` artifacts instead of decoding inline tx proof bytes on that lane. The verifier reconstructs the expected packed witness from the on-chain tx public view plus the serialized STARK public inputs and hidden opening carried inside the tx-leaf artifact, checks that reconstructed witness against the explicit commitment opening object, verifies the randomized commitment under the versioned parameter set, derives the ordered `TxStatementBinding`s from those verified leaves, and only then accepts the folded receipt root. The older bridge path that still starts from inline tx proofs is benchmarked as `verified_tx_receipt`, but it now exists only as a comparison lane. The native artifact path no longer pays packed-witness transport in its byte accounting; instead it pays for the canonical hidden-witness opening plus the explicit commitment opening. Import hardening now adds exact size rejection for native tx-leaf and receipt-root artifacts plus a reusable verified-native-leaf store keyed by native artifact hash. The old native `ReceiptRoot` verifier remains intentionally linear so the benchmark has a baseline. A second experimental import adapter, `receipt_accumulation`, wraps the native receipt-root bytes together with the ordered native leaf hashes; when those leaves were already verified during candidate preparation or a previous import attempt, consensus can resolve them from the verified-leaf store and re-check the folded root without replaying every hidden-witness opening. Cached authoring reuse for that lane now re-runs the same verified-leaf prewarm before it returns a stored accumulation payload, so a cached payload cannot outlive the verifier-local store state it still depends on. Authoring also emits explicit native-lane selection logs when `receipt_root` is requested, distinguishing `native_artifacts_unavailable`, `native_verifier_profile_mismatch`, and `native_artifact_validation_failed` before it falls back to `InlineTx`, and the prove-ahead cache deliberately stores only successful native receipt-root outcomes so a temporary fallback does not pin that candidate set to `InlineTx`. Re-running the canonical benchmark on 2026-03-26 under `native-backend-v3` kept the byte curve intact: bytes per tx stayed between `18,073` and `18,552` for `k = 1..128`, and the fingerprint above stayed stable across reruns. Exact verifier ns are no longer frozen into this document. The benchmark output now states that `total_active_path_*_ns` are single-run wall-clock measurements that must be rerun on the target host before being treated as decision-grade latency numbers, and two archived current-tree reruns live in `.agent/benchmarks/native_tx_leaf_receipt_root_v3_run_a_20260326.json` and `.agent/benchmarks/native_tx_leaf_receipt_root_v3_run_b_20260326.json`. Inline tx proof baselines on the same harness remained hundreds of kB per tx, so the hardened native lane remains the primary experimental native-folding candidate on this branch. The backend still remains an in-repo approximation rather than the paper’s exact module ring: it uses small-ring negacyclic arithmetic and an Ajtai-style linear commitment, but it still does not implement Neo/SuperNeo’s full Module-SIS commitment analysis, decomposition reduction, or sum-check machinery, so the branch still does not justify a production 128-bit PQ security claim.
