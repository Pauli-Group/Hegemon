...
- The pre-heavy-work resource-bound surface is a Lean composition over accepted `ResourceBudgetAdmission`, `RpcAdmission`, `SidecarUploadAdmission`, `MineableActionAdmission`, `ActionRequestProjectionAdmission`, `TransferActionPayloadAdmission`, `CandidateArtifactAdmission`, tx-leaf artifact parser inputs, and receipt-root schedule inputs. `Hegemon.Native.PreHeavyWorkResourceBoundSurface.accepted_preheavy_resource_bound_surface_exposes_bounds` packages pending-action and staged-proof byte caps, timestamp row caps, hex/base64 byte caps, JSON-RPC batch caps, sidecar request/capacity/proof preconditions, and mineable-action preconditions into one review surface before expensive proof or block-template work. `action_request_projection_accepts_implies_preheavy_bounds` derives strict submit-action JSON, empty kernel-envelope, supported route, nullifier, public-args, and exact route-payload predicates from accepted action-request projection before heavy work. `accepted_preheavy_public_input_parser_admission_bounds_verification_paths` adds those action-request facts, transfer proof/ciphertext caps, recursive candidate tx/proof caps, tx-leaf byte-shape facts, and receipt-root nonzero expected-leaf plus exact leaf/fold schedule facts before heavy verification or receipt-root folding, with parser correctness and benchmark caps carried as explicit assumptions. `Hegemon.Resource.BoundedRequestAdmission` is now the reusable formal table for the remaining parser families: it proves raw-byte, decoded-byte, item-count, per-item, aggregate-byte, and work-unit caps with exact-limit acceptance and deterministic first-failing-cap rejection. It adds no runtime path and inherits production conformance from the underlying gates where those gates are already bound; it does not prove JSON parser correctness, receipt-root cryptographic fold soundness, RPC authentication, unsafe-method exposure safety, per-family bounded-request production equivalence, DA availability, sled durability, proof-system soundness, or complete native-node equivalence.
...
- The recursive-block admission kernel proves the cheap consensus envelope/profile/decode/header/count/statement/public-replay table before heavy recursive proof verification with `Hegemon.Consensus.RecursiveBlockAdmission.artifact_accepts_iff_preconditions`; companion accept/reject and precedence theorems pin representative rows. It also proves `Hegemon.Consensus.RecursiveBlockAdmission.direct_v1_requires_semantic_replay` and `Hegemon.Consensus.RecursiveBlockAdmission.direct_v2_requires_semantic_replay`, so the generic recursive-block registry verifier fails closed unless the product path supplies verified-record semantic replay inputs. `Hegemon.Consensus.RecursiveBlockV2VerifierSurface.surface_accepts_iff_preconditions`, `full_verifier_accepts_iff_surface_and_crypto`, and `full_accepts_implies_surface_accepts` model the shipped `recursive_block_v2` pre-proof verifier surface before cryptographic child proof verification. `gen_recursive_block_admission_vectors` and `gen_recursive_block_v2_verifier_surface_vectors` emit examples that Rust `evaluate_recursive_block_artifact_admission`, direct `RecursiveBlockVerifier` rejection, and `verify_block_recursive_v2_surface` must match. The blueprint requires the artifact-admission helper to dominate `verify_block_recursive_v1` and `verify_block_recursive_v2`, requires `verify_block_recursive_v2_surface` to dominate `verify_tree_child_v2`, and requires `ParallelProofVerifier::verify_block_with_backend` to propagate `verify_recursive_block_artifact_against_verified_records` before tree transition. Skipped edge: `RecursiveBlockVerifier::verify_block_artifact` is a direct fail-closed registry stub with no backend successor to order against.
...
- `native_tx_leaf_ciphertext_privacy_game_all_active_outputs_select_same_public_boundary` and `native_tx_leaf_ciphertext_privacy_game_all_active_outputs_projected_da_boundary` lift the selected public-summary and projected-DA privacy-game packages to every active output slot in an accepted native tx-leaf surface. `active_output_public_metadata_boundary_secret_resampling`, `native_tx_leaf_ciphertext_privacy_game_active_output_secret_resampling_boundary`, and `native_tx_leaf_ciphertext_privacy_game_all_active_outputs_secret_resampling_boundary` prove that the selected/all-active native public metadata output-row package survives independent private-witness/prover-randomness resampling while carrying explicit proof-system zero knowledge, wallet metadata hygiene, timing/batching policy, and network metadata policy assumptions. `native_tx_leaf_ciphertext_privacy_game_open_assumption_decrypt_da_boundary` composes the selected decrypt-to-commitment/projected-DA hash theorem with `ciphertext_privacy_game_open_assumption_boundary_facts`, so deterministic observer leakage, independent secret-resampling stability, same-wire full-leakage discharge, raw-wire indistinguishability, proof-system zero knowledge, wallet metadata hygiene, timing/batching policy, and network metadata policy are carried together with the native selected-output implementation boundary.
