...
`circuits/superneo-backend-lattice` now implements a direct in-repo SuperNeo-style folding backend over Goldilocks behind one explicit parameter object, `NativeBackendParams`. The active rewrite family is `goldilocks_128b_rewrite` with fingerprint `f1204d753f4bec604c0e3626b0b4b010dab4d5bf27d7aae35a3fdb696e358b600bd30cee98f39700b94caf53ac329f9a`, `security_bits = 128`, `ring_profile = GoldilocksCyclotomic24`, `matrix_rows = 8`, `matrix_cols = 8`, `challenge_bits = 63`, `fold_challenge_count = 3`, `max_fold_arity = 2`, `transcript_domain_label = "hegemon.superneo.fold.v3"`, `decomposition_bits = 8`, and `opening_randomness_bits = 128`. Its public manifest also exposes `spec_label = "hegemon.superneo.native-backend-spec.goldilocks-128b-rewrite.v1"`, `commitment_scheme_label = "neo_class_linear_commitment_128b_masking"`, `challenge_schedule_label = "triple_goldilocks_fs_challenge_negacyclic_mix"`, and `maturity_label = "rewrite_candidate"`. The exact current protocol surface is frozen in [docs/crypto/native_backend_spec.md](docs/crypto/native_backend_spec.md), and benchmark JSON now emits the corresponding `spec_digest` so archived measurements can be tied to one exact protocol contract. The older `heuristic_goldilocks_baseline` remains in tree only as a frozen comparison constructor. The fold transcript is no longer a single challenge: setup now rejects parameter sets whose advertised security target exceeds the configured challenge schedule, the backend derives and verifies three independent 63-bit Goldilocks challenges, and the parameter fingerprint plus `spec_digest` now cover the manifest together with `fold_challenge_count`, `max_fold_arity`, and transcript-domain settings so future drift there cannot silently change backend behavior. The method is still experimental and not the paper’s exact production commitment scheme, but it is no longer a pure mock. The current steps are:
