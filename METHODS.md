...
The native tx-leaf canonical-surface composition is represented separately in Lean: an accepted native tx-leaf/action binding gate, a canonical deployed transaction statement surface, and the explicit deployed verifier soundness assumption now imply the accepted transaction relation, authorized asset-delta values, active input spend facts, total input-slot authorization facts, output commitment/ciphertext facts bound to the same statement and proof-binding vectors, and the packaged canonical proof-system boundary facts. This closes a native artifact-to-spend-facts implementation-equivalence gap, but it still does not prove deployed AIR/STARK/SmallWood soundness, hash security, tx-leaf parser completeness, or full native-node refinement.
...
- The wallet note-ciphertext wire kernel proves representative crypto-format and chain-format parser facts with `Hegemon.Wallet.NoteCiphertextWire.crypto_valid_accepts`, `Hegemon.Wallet.NoteCiphertextWire.crypto_truncated_after_kem_rejects`, `Hegemon.Wallet.NoteCiphertextWire.crypto_trailing_byte_rejects`, `Hegemon.Wallet.NoteCiphertextWire.chain_valid_accepts`, and companion chain rejection theorems for memo overrun, nonzero fixed-container padding, noncanonical compact KEM length, and trailing bytes. `gen_note_ciphertext_wire_vectors` emits examples that Rust checks against `synthetic_crypto::note_encryption::NoteCiphertext::from_bytes` and `wallet::NoteCiphertext::from_chain_bytes`. This does not prove ML-KEM security or implementation equivalence, AEAD confidentiality, note commitment/nullifier correctness, BLAKE3 ciphertext-hash security, DA availability, wallet key security, or complete wallet/native equivalence.
