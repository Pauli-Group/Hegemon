...
| `formal-core` | Runs `bash scripts/check_formal_core.sh`, which builds the pinned Lean proof kernel and checks the formal model inventory, claims ledger, blueprint DAG, bridge reference vectors, and native backend reference vectors. |
...
- `bash scripts/check_formal_core.sh` is the mandatory formal-core gate for release branches. It runs `bash scripts/check_lean_formal.sh`, validates `config/formal-security-claims.json`, validates the blueprint DAG in `config/formal-security-blueprint.json`, checks the formal inventory, verifies independent bridge message/replay vectors, and reruns the native backend reference vectors. The claims ledger is the source of record for formal-security claim status, production eligibility, gates, and residual risks. The blueprint DAG is a JSON review map for that ledger: it makes dependencies, target review, implementation bindings, cheap falsification cases, Lean theorem evidence, and scope boundaries machine-checkable.
- `formal/lean` is the pinned Lean 4 project for actual machine-checked Hegemon kernels. The first theorem is `Hegemon.Bridge.accept_prevents_duplicate`, proving duplicate inbound replay rejection for the executable replay-state transition. This does not prove hash derivation or Rust implementation equivalence; those require follow-on Lean kernels plus generated vectors and conformance tests.
