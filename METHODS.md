...
Aggregation payload compatibility is now hard-cut to V4 in active import logic (`consensus::aggregation`): payloads must carry `proof_format=4`, tree metadata (`tree_arity/tree_levels/root_level`), and a deterministic `shape_id` bound to `(tx_count, public-input shape, FRI shape)`. Nodes reject malformed or legacy payload versions fail-closed. Operators can tune recursion behavior with `HEGEMON_AGG_TREE_ARITY`, `HEGEMON_AGG_LEVEL_PARALLELISM`, `HEGEMON_AGG_WARMUP_TARGET_SHAPES`, `HEGEMON_AGG_STAGE_QUEUE_DEPTH`, `HEGEMON_AGG_STAGE_LOCAL_PARALLELISM`, `HEGEMON_PROVER_STAGE_MAX_INFLIGHT_PER_LEVEL`, and optional cache persistence (`HEGEMON_AGG_CACHE_PERSIST=1`, `HEGEMON_AGG_CACHE_DIR=...`). Prover-market work packages now include stage metadata (`stage_type`, `level`, `arity`, `shape_id`, `dependencies`) so external provers can coordinate deterministic recursion-stage work scheduling; root packages expose explicit dependency IDs for the previous recursion level (`leaf_verify` or `merge`) derived from a deterministic arity-aware DAG, and stage logs surface `HEGEMON_PROVER_STAGE_MEM_BUDGET_MB` as the configured per-stage memory budget hint.
