...
Node, pallet, and consensus routing are also now collapsed onto one explicit route object. The pallet uses `BlockProofRoute = (BlockProofMode, ProofArtifactKind)`, while consensus uses the parallel `ArtifactRoute = (ProvenBatchMode, ProofArtifactKind)`. That route is the real product selector above the backend seam. The shipped route is `(RecursiveBlock, RecursiveBlockV2)`. The explicit alternate native route is `(ReceiptRoot, ReceiptRoot)`. Keeping that pair explicit matters because the on-chain payload still carries the older `proof_mode` field for compatibility, while authoring caches, RPC announcements, and selector logic need exact route identity to avoid wrong-lane reuse. In other words, mode and artifact kind are no longer treated as independently meaningful choices in product code; they travel together as one route and the rest of the system keys on that route plus the verifier-profile digest. The same cleanup also pulled receipt conversion into explicit node-side proof-boundary helpers, moved backend tx artifacts out of the generic block object and into `BlockBackendInputs` at the verifier edge, and made canonical tx claims/receipts the only generic cross-layer tx-proof object above backend adapters. The proof-layer split above that edge is now explicit too: `consensus/src/backend_interface.rs` is the single façade that re-exports backend proof helpers for the rest of `consensus` and `node`, while `consensus/src/proof_interface.rs` owns the stable backend-neutral contracts (`ProofVerifier`, `HeaderProofExt`, `BlockBackendInputs`, receipt-root interface helpers, and claim/binding helpers). `consensus/src/lib.rs` no longer broadly re-exports those proof APIs, so callers must import the owning module directly instead of depending on crate-root shortcuts. Service/RPC/pool code therefore no longer imports `block_recursion`, `block_circuit`, `superneo_hegemon`, or `transaction_circuit::proof` directly, and it no longer has to import `consensus::proof` just to reach the generic contracts. Those node-side seams are now split three ways: `node/src/substrate/proof_boundary.rs` owns receipt translation plus backend-input wrapping, `node/src/substrate/receipt_root_compat.rs` owns the explicit `ReceiptRoot` compatibility-lane checks, and `node/src/substrate/receipt_root_builder.rs` owns the alternate-lane work-plan/build helpers. That leaves `service.rs` with orchestration and native-lane policy, not payload translation or compatibility-builder internals.
...
Canonical transaction-validity receipts are still derived from a tx statement hash, proof digest, serialized public-input digest, and verifier-profile digest, and the experimental receipt-root adapter re-checks that exact receipt object during import. Generic consensus/node code now reaches that backend only through neutral receipt-root helper APIs, not by importing `superneo-*` crates directly. Node authoring still uses an asynchronous prover coordinator (`node/src/substrate/prover_coordinator.rs`) that runs a bounded local worker queue, deterministic candidate upsizing, exact proof-lane identity on prepared-bundle lookup, and current-parent-scoped prepared-bundle reuse; the old external prover-market work-package surface is gone, leaving only local reusable-artifact discovery helpers (`prover_listArtifactAnnouncements`, `prover_getCandidateArtifact`). Aggregation candidate selection still drops proof-sidecar transfers whose ciphertext bytes are not available in the local pending sidecar store, preventing endless reproving loops on nodes that did not receive the sidecar payloads directly.
