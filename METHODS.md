...
The witness method is now split in three. The legacy synthetic receipt relation still exists for narrow crate-level tests. The bridge relation, `VerifiedTxProofReceiptRelation`, derives canonical tx-validity receipts from real `TransactionProof` objects, re-verifies those proofs when building leaf assignments, and then packs the receipt witness lanes with declared bit widths so the experiment can measure pay-per-bit pressure directly in Goldilocks terms. The current honest topology then turns those canonical receipts into standalone `TxLeaf` artifacts, whose proof bytes contain the SuperNeo leaf proof plus serialized transaction public inputs needed to recover a full `TxStatementBinding`. This is still not a native AIR-to-CCS port, but it is no longer “digests folded over digests” without real tx-proof checks.
...
For the live experimental `ReceiptRoot` path, import now consumes standalone `TxLeaf` artifacts instead of decoding inline tx proof bytes on that lane. The verifier reconstructs the expected packed witness deterministically from the canonical receipt, re-commits it under the backend key, checks that commitment against the compact tx-leaf proof, derives the ordered `TxStatementBinding`s from those verified leaves, and only then accepts the folded receipt root. The older bridge path that rechecked inline tx proofs is still benchmarked as `verified_tx_receipt`, but it is no longer the primary topology experiment. The backend remains an in-repo approximation rather than the paper’s exact module ring: it uses small-ring negacyclic arithmetic and an Ajtai-style linear commitment, but it still does not implement Neo/SuperNeo’s full Module-SIS commitment analysis, decomposition reduction, or sum-check machinery.
