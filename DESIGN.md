...
* Native sync, metadata, and action decoding are exact-admission gates: bounded sync messages reject legacy bincode and trailing bytes, SCALE and bincode helpers reject unconsumed bytes, and block action payload counts must match the header before semantic validation. After exact decode, block actions must also satisfy embedded `tx_hash` recomputation and uniqueness before import or decode-only consumers observe them. Persisted pending-action reload checks that the sled key, embedded `tx_hash`, and recomputed `pending_action_hash` agree before locally reloaded actions can enter mining selection. These codec and action-hash boundaries are represented in Lean and checked against the production native decode helpers, with focused reload regressions covering the local persistence boundary.
