...
Concrete modules in the repository now reflect this plan. A dedicated `state/merkle` crate maintains the append-only commitment tree with Poseidon-style hashing, while the `circuits/block` crate now treats commitment proofs as the primary block-validity artifact: `CommitmentBlockProver` builds a `CommitmentBlockProof` that commits to transaction proof hashes via a Poseidon sponge, and consensus verifies the commitment proof alongside parallel transaction-proof verification. The commitment proof currently binds `tx_proofs_commitment` plus the starting/ending state roots, nullifier root, and DA root as public inputs; consensus still enforces the actual state transition, nullifier uniqueness, and DA computation until the next milestone adds those constraints in-circuit. The legacy recursion utilities live under `circuits/block::recursive` and are retained for dev/test maintenance only. Consensus validates a block by comparing the header’s `commitment_proof_hash` and `tx_proofs_commitment` against the attached `CommitmentBlockProof`, then verifies each transaction proof in parallel before applying commitments to the state tree. The `protocol-versioning` crate defines a `VersionBinding { circuit, crypto }` pair plus helpers for recording per-block version matrices. Every transaction now commits to its binding, the block header publishes a 32-byte `version_commitment`, and consensus still attests how many transactions ran under each circuit/primitive pair even when miners operate autonomously.
