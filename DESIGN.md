...
* a separate live integrated `64`-lane bridge statement that the current backend actually proves (`raw_witness_len = 272`, `poseidon_permutation_count = 143`, `poseidon_state_row_count = 4576`, `expanded_witness_len = 91264`, `lppc_row_count = 1426`, `lppc_packing_factor = 64`).

The SmallWood branch is now past the old scalar fallback. The Rust engine exposes sparse linear constraints, the active packed semantic bridge over the native witness is real, the packed witness satisfies the LPPC checks, and the integrated candidate prover/verifier roundtrip exists on the live backend seam. The old vendored C bridge is no longer part of the active backend path. The important distinction is now explicit: the frozen `934`-row target statement remains the structural optimization target, but the live integrated backend now proves the narrower `1418`-row bridge statement rather than the earlier `1426`-row, `1428`-row, `1430`-row, or `2982`-row bridges. The latest frontend shrink removed the duplicated input/output selector rows from the bridge witness and now derives those routing weights directly from the duplicated asset rows and the public balance-slot assets with low-degree slot-membership polynomials, which keeps the live engine honest while trimming the bridge layout again. The live candidate path also now builds one shared witness context and fail-closes if the frozen `934`-row packed object no longer validates before the narrower bridge prover runs, so direct/bridge drift is now a product-path invariant instead of a test-only assumption. The engine/native boundary is now explicit too: prove, verify, and projected-size calls consume a `SmallwoodConstraintAdapter` statement object instead of raw bridge-layout tuples, so bridge-specific nonlinear assumptions are isolated to one adapter seam. The engine surface also now carries an explicit SmallWood arithmetization mode: `Bridge64V1` is the live proving path, and `DirectPacked64V1` is an explicit unsupported mode in the proof engine instead of an implicit alias through `row_count`. The repo now also materializes the frozen direct-packed object as an explicit `DirectPackedProgram` over the canonical `934 x 64` matrix, plus an explicit `DirectPackedPoseidonProgram` that names the PRF, commitment, Merkle, nullifier, output-commitment, and balance-tag permutation spans inside that matrix. The direct packed semantics path is now real too: `DirectPacked64V1` goes through `PackedStatement::compute_constraints_u64` on a `DirectPackedMatrix` view, reserializes the canonical raw witness, recomputes public inputs, and checks the named Poseidon spans instead of relying on a detached flat-matrix helper. What is still missing is the proving-side half of that mode. The current proof engine still only opens one scalar per row polynomial to the verifier, so `Bridge64V1` remains the only live proving path until the proof object can expose the extra lane-local information the direct packed layout needs. Under the current exact no-grinding `128-bit` profile (`rho = 2`, `nb_opened_evals = 3`, `beta = 2`, `decs_nb_evals = 16384`, `decs_nb_opened_evals = 29`, `decs_eta = 3`, zero grinding bits), the exact current serialized proof envelope for that packed candidate now projects in-repo to `106940` bytes, and the passing release roundtrip emits `106948` proof bytes. That is about `3.31x` smaller than the shipped `354081`-byte Plonky3 proof while still staying below the current `524288`-byte native `tx_leaf` cap. The focused warmed release roundtrip is now about `1.74s` directly in `transaction-circuit`, and the wrapped `superneo-hegemon` `tx_leaf` seam test completes in about `1.98s`. The latest runtime/front-end attack therefore was not another loop trick; it combined the smaller `1418`-row bridge, the secure `beta = 2` LVCS geometry, the `16384 / 29 / 3` DECS point, and the earlier preallocated DECS row buffers with thread-local scratch. That leaves the dominant `decs_commit -> domain_evals` stage at about `1.54s` on the direct warmed run instead of the previous `~1.64s`, and far below the older `~3.4s`. The verifier also now fail-closes on exact inner proof shapes, requires distinct DECS opening indices, binds the full PCS commitment transcript into the PIOP transcript, hashes full opened combis for the DECS challenge, and derives both commitment-time and verifier-time polynomial openings from the exact interpolation domain rather than the earlier broken shortcut helpers. So malformed candidate proofs no longer panic the release verifier, forged self-consistent PCS layers are rejected, and the implemented query count matches the note. The branch is therefore no longer blocked on raw proof size, and it is materially less blocked on proving cost than before, but the `934`-row bridge target is still the next structural efficiency step. The earlier `75 KB .. 121 KB` SmallWood numbers are now close enough to the live result that they no longer read like fiction; they remain the structural target for the next bridge shrink rather than the current integrated candidate size. The exact current tx-proof sweep and soundness note for the shipped Plonky3 line live in `docs/crypto/tx_proof_profile_sweep.json` and `docs/crypto/tx_proof_soundness_analysis.md`. The first conservative STIR replacement spike is also checked in under `docs/crypto/tx_proof_stir_spike.json` / `docs/crypto/tx_proof_stir_soundness.md`; it clears the security gate but only projects to about a `1.30x` total-byte reduction, so it is not currently a sufficient reason to replace the shipped tx proof opening layer. With 384-bit digests, PQ collision resistance reaches ~128 bits for application-level commitments, and 48-byte encodings are used end-to-end.

The current direct-packed mode is no longer “unsupported” at the engine/frontend seam. It now proves and verifies as an explicit alternate arithmetization with a serialized raw-witness payload and an explicit arithmetization tag in the proof envelope. It is still not the default product path because that direct payload is about `32109` bytes, versus `106941` projected / `106949` actual bytes for the compact bridge path.
...
On the product side, authoring now treats the receipt-root build as a deterministic hierarchy rather than one opaque serial job. The builder splits the ordered native `tx_leaf` set into fixed `8`-leaf mini-roots, reuses cached verified leaves and cached chunk folds keyed by native artifact identity, and then folds those mini-roots up to one block root. Fresh builds still do about `N - 1` folds overall, but repeated or near-repeated candidates can reuse most of the lower tree, and service-side worker planning can run the build on a dedicated local Rayon pool sized by `HEGEMON_RECEIPT_ROOT_WORKERS`. Native `tx_leaf` artifacts now also carry an explicit tx-proof-backend selector byte after the embedded proof payload so the verifier can dispatch cleanly between proof families while keeping the lattice folding layer unchanged. The new `SmallwoodCandidate` backend already uses that seam: a candidate tx proof can be wrapped into a native `tx_leaf`, verified independently, and then aggregated into the same `receipt_root` object beside normal Plonky3 leaves.

Import also no longer pays the native leaf verifier twice by default. Consensus derives verified native leaf records while checking the ordered `tx_leaf` artifacts, and the shipped receipt-root verifier consumes those verified records on the `verified_records` path. The older replay-heavy root verifier remains in-tree as a diagnostic and cross-check lane, not as the default product verifier.

The experimental relation layer is now deliberately narrow. `TxLeafPublicRelation` remains the bridge/comparison relation over nullifiers, commitments, ciphertext hashes, balance tag, version binding, and serialized STARK public inputs. `NativeTxValidityRelation` remains the witness-driven source relation: it consumes `TransactionWitness` directly, checks witness validation plus Merkle membership locally, derives the canonical public-input object without going through a Plonky3 proof, and feeds the native `TxLeaf -> ReceiptRoot` baseline. The explicit node/consensus research surface is now only that native `ReceiptRoot` baseline. The old warm-store accumulation and residual ARC/WHIR lanes were removed from product routing, consensus verification, and operator selection after they failed the cold-import goal.
...
Concrete modules in the repository now reflect the cleaned split. A dedicated `state/merkle` crate maintains the append-only commitment tree with Poseidon-style hashing, and `circuits/block` still owns the optional parent-bound commitment proof for explicit block-artifact experiments. The shipped fresh-chain 0.10.0 path is simpler: ordered native `TxLeaf` artifacts are the only shielded transaction-validity bytes wallets submit by default, block import verifies those ordered artifacts directly when no explicit block artifact is present, and the node’s prove-ahead coordinator only prepares parent-bound bundles for the explicit native `ReceiptRoot` research lane. The coordinator keeps a bounded local worker queue, current-parent-scoped readiness, deterministic candidate upsizing, and reuse of stale-parent prepared bundles for cache amortization, but it no longer publishes external work packages or recursive stage jobs.

Block-proof compatibility is still hard-cut to `BlockProofBundle` schema `2` with proof format id `5`, and import still fail-closes on legacy payloads. But the product meaning of those compatibility fields is now narrow: `InlineTx` is the shipped direct-per-transaction mode, and `ReceiptRoot` is the only remaining explicit research block artifact. The old recursive `MergeRoot`, manifest-style `FlatBatches`, warm-store accumulation wrapper, and residual ARC/WHIR block lanes are gone from consensus, node routing, tests, and operator selectors. The node still exposes additive artifact-market RPC surfaces (`prover_listArtifactAnnouncements`, `prover_getCandidateArtifact`) so builders can discover reusable prepared artifacts without another consensus rewrite, but the old external prover-worker market surface was deleted with the dead proof lanes.
...
The product no longer carries an external prover-worker market. For the fresh-chain 0.10.x line, the shipped shielded path is the same-block native `tx_leaf -> receipt_root` lane, and the prover coordinator prepares that block artifact on a dedicated long-lived local worker pool (`hegemon-artifact-worker-*`) instead of spawning one-off blocking tasks for each candidate.

Receipt-root authoring is now hierarchy-aware. The node plans deterministic `8`-leaf mini-roots, the circuit builder reuses verified-leaf and chunk-fold caches keyed by native artifact identity, and the aggregation stage can run on a dedicated Rayon pool sized by `HEGEMON_RECEIPT_ROOT_WORKERS` (falling back to the local artifact-worker count). The coarse prove-ahead prepared-bundle cache still serves exact repeats, while the lower-level leaf/chunk caches make one-leaf and near-repeat rebuilds materially cheaper without changing consensus semantics.

Artifact preparation is now bounded to the only product-relevant choice:

- shipped and enforced path: `HEGEMON_BLOCK_PROOF_MODE=receipt_root`

Unknown proof-mode values now clamp back to the canonical native receipt-root selector instead of reviving removed lanes. The old recursive `MergeRoot`, manifest-style `FlatBatches`, warm-store accumulation wrapper, and residual ARC/WHIR path are gone from live routing, tests, and operator selectors.

Import was hardened in parallel with authoring. Non-empty shielded blocks still fail closed unless they carry a canonical native `receipt_root` payload, but the default verifier now consumes the verified native leaf records already produced during tx-artifact validation (`verified_records`) rather than replaying the full leaf verifier a second time. Replay-heavy root verification remains available as a diagnostic cross-check path.
