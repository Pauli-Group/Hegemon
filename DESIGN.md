...
The v0.10.0 native header surface now exposes bridge-friendly commitments: `chain_id`, `rules_hash`, `cumulative_work`, `kernel_root`, `message_root`, `message_count`, and `header_mmr_root`/`header_mmr_len`. The header MMR is a real Merkle Mountain Range over historical header hashes, and outbound bridge witnesses use compact MMR openings plus deterministic FlyClient-style samples instead of replaying every header from genesis. The shared `consensus-light-client` crate owns the deterministic `PowHeaderV1` verifier and is intentionally `no_std + alloc` compatible so the same logic can be used by the node, witness builders, and zkVM guests. The long-range bridge verifier routes its cheap structural admission table through a Lean-conformance-checked helper for verifier hash, message count, height/MMR length, message source, FlyClient sample shape, confirmation, tip-work, and output-binding checks; SHA-256, MMR hash binding, and FlyClient sampling soundness remain separate assumptions. The native node still performs full validation of shielded state, nullifiers, recursive block artifacts, DA roots, supply accounting, and future-skew bounds outside that minimal light-client kernel.
