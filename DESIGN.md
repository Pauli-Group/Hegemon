...
Bridge messages are protocol actions, not log scraping. `protocol-kernel` defines the bridge family and `BridgeMessageV1`; outbound bridge actions are ordered into a canonical `message_root`, while inbound bridge actions consume proven messages once with replay protection keyed by `(source_chain_id, source_message_nonce)`. Hegemon-to-Hegemon bridging now uses a RISC Zero receipt envelope whose authenticated journal is the SCALE-encoded `BridgeCheckpointOutputV1`; the destination verifies `Receipt::verify(image_id)`, checks the journal/output against the inbound message, and never uses the receipt for fork choice. RISC Zero is a bridge transport dependency of the native node, not a dependency of `consensus-light-client`, and the PQ path must verify native STARK/hash-based receipts rather than Groth16/KZG wrappers.
