...
The current backend is no longer a pure digest mock. It now implements a concrete SuperNeo-style experimental path in stable Rust: packed witness bits are projected through a deterministic public matrix over Goldilocks, leaf proofs bind those packed witnesses to statement digests, and fold steps combine commitments with transcript-derived linear challenges. This is still not a production lattice/Ajtai implementation and should not be treated as a security-equivalent substitute for Neo/SuperNeo’s commitment scheme, but it is a real folding backend rather than a placeholder hash tree. The experiment remains benchmarkable and deletable, and it is still not consensus-relevant beyond the additive receipt-root research path. The naming rule in code follows the same boundary: generic layers speak in `ProofEnvelope`, `TxValidityReceipt`, `TxValidityArtifact`, and `ReceiptRoot` terms, while `superneo-*` names stay confined to the experimental backend crates that implement one concrete receipt-root backend today.
