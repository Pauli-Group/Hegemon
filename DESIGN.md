...
The product routing layer is now normalized around one explicit route pair instead of treating `proof_mode` and `proof_kind` as separate product choices. On the pallet side that object is `BlockProofRoute = (BlockProofMode, ProofArtifactKind)`. On the consensus side it is `ArtifactRoute = (ProvenBatchMode, ProofArtifactKind)`. That distinction matters because the shipped route is exactly `(RecursiveBlock, RecursiveBlockV2)`, while the explicit alternate route is `(ReceiptRoot, ReceiptRoot)`, and everything else is compatibility-only decoding or research surface. The pallet and consensus payloads still carry the older split fields for schema compatibility, but pallet validation, node artifact selection, prepared-bundle cache matching, RPC announcement mapping, and cross-layer translation now key on the explicit route object so future backend swaps do not require another round of duplicated “mode implies kind” logic across consensus and authoring. Generic consensus blocks now carry only canonical `tx_validity_claims` plus the neutral block-artifact envelope; backend-specific tx artifacts are handed to the verifier/build path through explicit backend-input helpers instead of riding along in the generic block object. On the node side, the proof and compatibility seams now live in dedicated modules: `node/src/substrate/proof_boundary.rs` owns receipt/claim conversion and verifier backend-input wrapping, `node/src/substrate/receipt_root_compat.rs` owns the explicit `ReceiptRoot` compatibility-lane payload checks, and `node/src/substrate/receipt_root_builder.rs` owns the alternate-lane work-plan/build mechanics. `service.rs` now orchestrates prove-ahead/build/import flow without open-coding those boundary rules or builder internals locally.
