...
The SmallWood branch is now past the old scalar fallback. The Rust engine exposes sparse linear constraints, the packed four-lane semantic relation over the native witness is real, the packed witness satisfies the LPPC checks, and the integrated candidate prover/verifier roundtrip exists on the live backend seam. The old vendored C bridge is no longer part of the active backend path. The exact current serialized proof envelope for that packed candidate is now projected in-repo at `239460` bytes, which is below both the shipped `354081`-byte Plonky3 proof and the current `524288`-byte native `tx_leaf` cap. So the blocker is no longer proof size. The blocker is prover runtime: the current Rust SmallWood release prover is still too slow for the default product/test lane, with the traced hot path dominated by witness-polynomial construction and PCS commit well before verification. The earlier `75 KB .. 121 KB` SmallWood numbers therefore remain the next structural target, not the current integrated candidate size. The exact current tx-proof sweep and soundness note for the shipped Plonky3 line live in `docs/crypto/tx_proof_profile_sweep.json` and `docs/crypto/tx_proof_soundness_analysis.md`. The first conservative STIR replacement spike is also checked in under `docs/crypto/tx_proof_stir_spike.json` / `docs/crypto/tx_proof_stir_soundness.md`; it clears the security gate but only projects to about a `1.30x` total-byte reduction, so it is not currently a sufficient reason to replace the shipped tx proof opening layer. With 384-bit digests, PQ collision resistance reaches ~128 bits for application-level commitments, and 48-byte encodings are used end-to-end.
