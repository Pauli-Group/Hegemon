...
The SmallWood branch is now past the old scalar fallback. The Rust engine exposes sparse linear constraints, the active packed semantic bridge over the native witness is real, the packed witness satisfies the LPPC checks, and the integrated candidate prover/verifier roundtrip exists on the live backend seam. The old vendored C bridge is no longer part of the active backend path. The important distinction is now explicit: the frozen `934`-row target statement remains the structural optimization target, but the live integrated backend now proves the narrower `1430`-row bridge statement rather than the earlier `2982`-row bridge. Under the same exact no-grinding profile that already cleared the wider bridge geometry's `128-bit` bar (`rho = 2`, `nb_opened_evals = 3`, `beta = 3`, `decs_nb_evals = 4096`, `decs_nb_opened_evals = 65`, `decs_eta = 10`, zero grinding bits), the exact current serialized proof envelope for that packed candidate now projects in-repo to `224220` bytes, and the passing release roundtrip emits `224228` proof bytes. Both are below the shipped `354081`-byte Plonky3 proof and the current `524288`-byte native `tx_leaf` cap. The focused release roundtrip is now about `1.9s` both directly in `transaction-circuit` and through the wrapped `superneo-hegemon` `tx_leaf` seam, so the branch is no longer blocked on raw proof size or the old multi-second runtime wall. The earlier `75 KB .. 121 KB` SmallWood numbers therefore remain the next structural target, not the current integrated candidate size. The exact current tx-proof sweep and soundness note for the shipped Plonky3 line live in `docs/crypto/tx_proof_profile_sweep.json` and `docs/crypto/tx_proof_soundness_analysis.md`. The first conservative STIR replacement spike is also checked in under `docs/crypto/tx_proof_stir_spike.json` / `docs/crypto/tx_proof_stir_soundness.md`; it clears the security gate but only projects to about a `1.30x` total-byte reduction, so it is not currently a sufficient reason to replace the shipped tx proof opening layer. With 384-bit digests, PQ collision resistance reaches ~128 bits for application-level commitments, and 48-byte encodings are used end-to-end.
