...
The shipping architecture remains `InlineTx`, and no consensus/runtime path depends on any alternate folding stack today. To make research on witness-free post-proof compression concrete without destabilizing the live system, the repo now carries an additive experimental workspace under `circuits/superneo-*`.
...
The current backend is no longer a pure digest mock. It now implements a concrete SuperNeo-style experimental path in stable Rust: packed witness values are decomposed into low-bit digits, embedded into small negacyclic ring elements over Goldilocks, committed with a deterministic Ajtai-style public matrix, and then folded with transcript-derived linear challenges. The compact-leaf step was also tightened again: native tx-leaf artifacts now use a manual hidden-witness opening format instead of serializing the packed witness or relying on a generic serializer for external witness structs. A `TxLeaf` artifact carries the canonical native receipt, serialized STARK public inputs, a compact native opening (`sk_spend`, input witnesses, output witnesses, Merkle paths), the commitment rows, and the compact leaf proof. Verification reconstructs the expected packed witness from that opening, re-commits it under the backend key, and checks the resulting digest against the tx-leaf proof. `ReceiptRoot` is also now native-only on the experimental lane: consensus folds and verifies only native `TxLeaf` artifacts there and falls back to `InlineTx` rather than accepting the older bridge profile on that path. Import hardening now includes exact native artifact-size bounds plus a reusable verified-native-leaf store keyed by native artifact hash. The old native `ReceiptRoot` verifier is intentionally still linear so the repo preserves a baseline, while the new experimental `receipt_accumulation` kind wraps the folded native receipt-root bytes together with the ordered native leaf hashes. That wrapper lets import hit the verified-leaf store first and re-check the folded root without re-running every hidden-witness opening when those leaves were already verified during candidate preparation or an earlier import attempt. Node authoring and import now reject both `receipt_root` and `receipt_accumulation` when proof bytes exist only in local sidecar state: those lanes are experimental, but they still have to be reconstructable from block contents for any peer.

The experimental relation layer now has one decision-grade lane and explicit diagnostics beside it. `TxLeafPublicRelation` is retained as a bridge/comparison relation over nullifiers, commitments, ciphertext hashes, balance tag, version binding, and serialized STARK public inputs. `NativeTxValidityRelation` is the actual performance lane: it consumes `TransactionWitness` directly, checks witness validation plus Merkle membership locally, derives the canonical public-input object without going through a Plonky3 proof, and feeds that witness into the native `TxLeaf -> ReceiptRoot` topology. The benchmark CLI now defaults to `native_tx_leaf_receipt_root`, labels that lane as canonical in its JSON `note` field, and requires `--allow-diagnostic-relation` before it will run the lower-level or bridge surfaces. That canonical benchmark now also emits an explicit `import_comparison` object that reports the old linear receipt-root verify time beside the new accumulation path’s prewarm time and warm verify time for the same ordered native leaf set; the fast accumulation number is explicitly a warm-store measurement after verified-leaf prewarm, not a generic cold-peer import figure. Node authoring now also emits structured selection logs for the native lane: when `HEGEMON_BLOCK_PROOF_MODE=receipt_root` is requested, the service records whether it used the native lane or fell back to `InlineTx`, and the fallback reason is one of `native_artifacts_unavailable`, `native_verifier_profile_mismatch`, or `native_artifact_validation_failed`. A second explicit selector, `HEGEMON_BLOCK_PROOF_MODE=receipt_accumulation`, keeps the same legacy `ReceiptRoot` payload shape but advertises a distinct custom `proof_kind` / verifier-profile pair so consensus can benchmark the hash-backed import prototype without deleting the baseline. Prove-ahead caching is deliberately asymmetric there: successful native `ReceiptRoot` payloads are cacheable, but `InlineTx` fallback outcomes are not cached for that selector so a later candidate with valid native artifacts can still take over. The current hardening therefore focuses on defensive bounds and reuse, not on pretending the security story is complete: the branch now has parallel native-leaf verification, a reusable verified-leaf store, explicit artifact-size rejection, the additive `receipt_accumulation` import path, explicit rejection of sidecar-only proof material on receipt-root lanes, and explicit fallback observability, but it still does not justify claiming production 128-bit PQ security. This is still not Neo/SuperNeo’s full commitment stack and should not be treated as a security-equivalent substitute for the papers: the in-repo backend still stops short of the exact Module-SIS commitment analysis, decomposition reduction, and sum-check machinery. The experiment remains benchmarkable and deletable, and it is still not consensus-relevant beyond the additive receipt-root research path. The naming rule in code follows the same boundary: generic layers speak in `ProofEnvelope`, `TxValidityReceipt`, `TxValidityArtifact`, `TxLeaf`, and `ReceiptRoot` terms, while `superneo-*` names stay confined to the experimental backend crates that implement one concrete receipt-root backend today.
