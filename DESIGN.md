...
Concrete modules in the repository now reflect this plan. A dedicated `state/merkle` crate maintains the append-only commitment tree with poseidon-style hashing, while the `circuits/block` crate now treats recursive proofs as the primary block-validity artifact: `prove_block` builds a `RecursiveBlockProof` (plus `version_counts`), and `verify_block` checks that proof against the running tree. The lower-level recursion utilities live under `circuits/block::recursive` and are used to generate or verify the batch proof itself. Consensus can validate a block by comparing the header’s `recursive_proof_hash` against an attached `RecursiveBlockProof` and verifying the recursive proof payload once, instead of re-verifying each transaction proof individually. The `protocol-versioning` crate defines a `VersionBinding { circuit, crypto }` pair plus helpers for recording per-block version matrices. Every transaction now commits to its binding, the block header publishes a 32-byte `version_commitment`, and the block circuit records `version_counts` so consensus can attest how many transactions ran under each circuit/primitive pair even when miners operate autonomously.
