...
The `circuits/formal/transaction_balance.tla` model captures the MASP balance rules (nullifier uniqueness + per-asset conservation) using a compact TLA+ spec. Any change to the AIR/witness layout must update that spec plus rerun TLC/Apalache when those tools are available, recording the outcome in the associated README and in `docs/SECURITY_REVIEWS.md`. The release-facing formal gate is `scripts/check_formal_core.sh`: it builds the pinned Lean project under `formal/lean`, checks generated Lean-to-Rust bridge, bridge checkpoint-output canonical/journal bytes, bridge long-range proof-shape, bridge header-MMR opening-shape, bridge header-MMR parent/root transcript, bridge FlyClient sampling transcript/index, shielded-nullifier, consensus fork-choice, consensus PoW-admission, consensus header-preimage, consensus PoW miner-identity, consensus version-policy, consensus proof-policy, consensus native tx-leaf admission, consensus receipt-root admission, consensus recursive-block admission, consensus recursive public replay, consensus recursive semantic inputs, consensus block tree-transition, consensus DA-root byte binding, consensus proven-batch binding, consensus aggregation V5 header admission, supply-accounting, native action-ordering, native action hash/count admission, native action scope admission, native transfer action payload admission, native coinbase accounting admission, native coinbase action payload admission, native candidate artifact admission, native block commitment admission, native tx-leaf artifact, native receipt-root, transaction-balance, transaction Merkle-path, transaction public-input shape, transaction public-input binding, and transaction statement-hash conformance vectors, checks the formal model inventory, validates the machine-readable claims ledger in `config/formal-security-claims.json`, validates the blueprint DAG in `config/formal-security-blueprint.json`, verifies independent bridge message/replay vectors, and reruns the native backend reference vectors. The bridge replay kernel proves duplicate rejection for staging and importing inbound replay keys through `Hegemon.Bridge.stage_prevents_duplicate_pending`, `Hegemon.Bridge.import_prevents_reimport`, and `Hegemon.Bridge.import_prevents_restaging`, while generated examples check production `protocol-kernel` bridge byte encoding and `InboundReplayState` behavior. The bridge long-range kernel proves structural admission facts for verifier-hash binding, message-count binding, header/MMR length shape, trusted/message/tip height ordering, selected message source, FlyClient sample height/opening-index agreement, confirmation policy, tip-work policy, and claimed-output agreement through `Hegemon.Bridge.LongRange.valid_shape_accepts` and the companion rejection theorems, while generated examples check the production `consensus-light-client::evaluate_long_range_proof_shape` helper called by the long-range verifier. The bridge checkpoint-output kernel proves the `BridgeCheckpointOutputV1` canonical/journal byte grammar: exact output domain, 439-byte domain-separated canonical preimage, 404-byte authenticated journal tuple, field order, scalar little-endian encoding, and fixed-length decode rejection. Generated examples check `consensus-light-client` output encoding/decoding helpers. The bridge header-MMR kernel proves opening-shape facts for peak decomposition, context mismatch rejection, leaf bounds, sibling-count admission, local-index derivation, and left/right path orientation through `Hegemon.Bridge.HeaderMmr.valid_shape_accepts`, `Hegemon.Bridge.HeaderMmr.singleton_shape_accepts`, `Hegemon.Bridge.HeaderMmr.four_leaf_left_orientation`, `Hegemon.Bridge.HeaderMmr.four_leaf_right_orientation`, and companion rejection theorems, while generated examples check the production `consensus-light-client::evaluate_header_mmr_opening_shape` helper used by standalone and long-range MMR opening verification. The bridge header-MMR transcript kernel proves parent/root preimage byte facts for exact domains, u32/u64 little-endian counters, left/right child order, ordered peak concatenation, empty roots, and reversed-peak distinction through `Hegemon.Bridge.HeaderMmrTranscript.sample_parent_hex`, `Hegemon.Bridge.HeaderMmrTranscript.max_level_parent_hex`, `Hegemon.Bridge.HeaderMmrTranscript.empty_root_hex`, `Hegemon.Bridge.HeaderMmrTranscript.two_peak_root_hex`, `Hegemon.Bridge.HeaderMmrTranscript.reversed_two_peak_root_hex`, and `Hegemon.Bridge.HeaderMmrTranscript.root_order_binds_peak_sequence`, while generated examples check the production `consensus-light-client` parent/root preimage helpers hashed by MMR construction and verification. The consensus fork-choice kernel proves deterministic two-tip ordering through `Hegemon.Consensus.higher_work_wins`, `Hegemon.Consensus.lower_work_loses`, `Hegemon.Consensus.equal_work_higher_height_wins`, `Hegemon.Consensus.equal_work_lower_height_loses`, `Hegemon.Consensus.equal_work_height_uses_hash_order`, and `Hegemon.Consensus.same_tip_not_better`, while generated examples check the production `consensus::fork_choice` helper used by `PowConsensus` and native block import. The consensus PoW-admission kernel proves compact-target rejection, strict timestamp admission, hash-threshold rejection, and fixed-width Work48 cumulative-work behavior through `Hegemon.Consensus.compactTarget_rejects_zero_mantissa`, `Hegemon.Consensus.compactTarget_rejects_large_exponent`, `Hegemon.Consensus.timestamp_rejects_parent_equal`, `Hegemon.Consensus.timestamp_rejects_median_equal`, `Hegemon.Consensus.timestamp_rejects_future_skew`, `Hegemon.Consensus.checkedWorkAdd_ok`, `Hegemon.Consensus.checkedWorkAdd_rejects_overflow`, `Hegemon.Consensus.powAdmission_rejects_height`, `Hegemon.Consensus.powAdmission_rejects_pow_bits`, `Hegemon.Consensus.powAdmission_rejects_insufficient_work`, and `Hegemon.Consensus.powAdmission_accepts_valid`, while generated examples check production `consensus::pow` and `consensus-light-client` helpers. The consensus header-preimage kernel proves the signing preimage domain and fixed-field byte layout, proves signing-preimage independence from signature, BFT bitmap, and PoW seal payloads, and proves BFT bitmap, PoW seal, and absent-auth tag shapes through `Hegemon.Consensus.Header.signingPreimage_independent_of_auth_payloads`, `Hegemon.Consensus.Header.sample_signing_preimage_length`, `Hegemon.Consensus.Header.sample_pow_full_header_length`, `Hegemon.Consensus.Header.bft_full_header_length`, `Hegemon.Consensus.Header.unsigned_full_header_length`, `Hegemon.Consensus.Header.pow_header_has_pow_tag`, `Hegemon.Consensus.Header.bft_header_has_bitmap_tag`, and `Hegemon.Consensus.Header.unsigned_header_has_absent_auth_tags`, while generated examples check production `BlockHeader::signing_preimage_v1` and `BlockHeader::full_header_preimage_v1`. The consensus proof-policy kernel proves empty-block proof-payload rejection, non-empty self-contained proof-shape requirements, legacy InlineRequired rejection, recursive commitment-proof-byte rejection, and recursive block-artifact requirements through `Hegemon.Consensus.empty_clean_accepts`, `Hegemon.Consensus.empty_with_payload_rejects`, `Hegemon.Consensus.nonempty_requires_tx_artifacts`, `Hegemon.Consensus.nonempty_rejects_inline_required`, `Hegemon.Consensus.recursive_rejects_commitment_proof_bytes`, `Hegemon.Consensus.recursive_requires_block_artifact`, and `Hegemon.Consensus.recursive_complete_accepts`, while generated examples check production `evaluate_block_proof_policy` behavior used by `ParallelProofVerifier`. The consensus tree-transition kernel proves proof starting-root agreement, commitment application success, proof ending-root agreement, accepted applied-root return, and rejection precedence through `Hegemon.Consensus.TreeTransition.tree_transition_accepts_iff_preconditions`, `Hegemon.Consensus.TreeTransition.accepted_transition_returns_applied_root`, and companion rejection theorems, while generated examples check production `verify_and_apply_tree_transition_without_anchors` behavior used after block proof verification. The aggregation V5 header kernel proves version/format/encoding admission, statement-commitment binding, child/fan-in bounds, subtree tx-count binding, tree-level/root-level checks, packed public-input length agreement, and the binary-minimum merge fan-in rule through `Hegemon.Consensus.AggregationV5.valid_leaf_singleton_header_accepts`, `Hegemon.Consensus.AggregationV5.valid_merge_header_accepts`, and companion rejection theorems, while generated examples check the production `consensus::aggregation::v5` header helper called before heavy proof verification. The supply-accounting kernel proves checked supply-digest increase/decrease, underflow/overflow rejection, no-coinbase native no-op behavior, and native coinbase advance through `Hegemon.Consensus.increaseSupplyDigest_ok`, `Hegemon.Consensus.increaseSupplyDigest_rejects_overflow`, `Hegemon.Consensus.decreaseSupplyDigest_ok`, `Hegemon.Consensus.decreaseSupplyDigest_rejects_underflow`, `Hegemon.Consensus.nativeSupplyDelta_without_coinbase`, `Hegemon.Consensus.advanceNativeSupplyDigest_checked`, and `Hegemon.Consensus.advanceNativeSupplyDigest_rejects_supply_overflow`, while generated examples check production `CoinbaseData::net_native_delta`, `consensus::reward::update_supply_digest`, and native checked supply helpers. The native action-ordering kernel proves empty/single/equal/ascending/descending and non-transfer-ignored cases for the shielded transfer-key subsequence through `Hegemon.Native.empty_canonical`, `Hegemon.Native.single_transfer_canonical`, `Hegemon.Native.non_transfer_ignored`, `Hegemon.Native.ordered_pair_accepts`, `Hegemon.Native.descending_pair_rejects`, and `Hegemon.Native.equal_pair_accepts`, while generated examples check the production transfer-key ordering predicate used by native block import. The native action hash/count admission kernel proves action payload count agreement, embedded action-hash agreement, duplicate action-hash rejection, and rejection precedence through `Hegemon.Native.ActionHashAdmission.accepts_iff_admission_preconditions` and companion theorems, while generated examples check the production helper called by native block decode/import. The native action scope admission kernel proves candidate-artifact payload scoping, bridge/candidate/coinbase state-delta scope, candidate payload presence, supported route selection, transfer public metadata shape, and rejection precedence through `Hegemon.Native.ActionScopeAdmission.accepts_iff_scope_preconditions` and companion theorems, while generated route/reject examples check the production helper called by native mempool and block action validation. The native transfer action payload admission kernel proves decoded proof presence/size caps, anchor and commitment agreement, inline ciphertext byte caps, ciphertext metadata agreement, binding-hash agreement, fee agreement, and rejection precedence through `Hegemon.Native.TransferActionPayloadAdmission.accepts_iff_payload_preconditions` and companion theorems, while generated examples check the production helper called by inline and sidecar transfer payload validation. The native coinbase-accounting admission kernel proves at-most-one coinbase, no-coinbase fee-burn acceptance, checked subsidy-plus-fee reward admission, fee/reward overflow rejection, missing amount rejection, reward mismatch rejection, and rejection precedence through `Hegemon.Native.CoinbaseAccountingAdmission` theorems, while generated examples check the production helper called by native block import and mining validation. The native coinbase action payload-admission kernel proves nonzero reward amount, commitment agreement/nonzero checks, ciphertext byte caps, ciphertext hash/size metadata agreement, and rejection precedence through `Hegemon.Native.CoinbaseActionPayloadAdmission` theorems, while generated examples check the production helper called by mempool and block action validation. The native candidate-artifact admission kernel proves no state-delta smuggling, payload presence, schema agreement, tx-count and DA-count bounds, shipped recursive_block_v2 route/profile, empty legacy proof fields, absent receipt-root payload, nonempty recursive proof bytes, proof byte caps, and rejection precedence through `Hegemon.Native.CandidateArtifactAdmission` theorems, while generated examples check the production helper called by candidate action/artifact validation. The native block commitment-admission kernel proves derived block-body/header commitment agreement for tx count, state root, kernel root, nullifier root, action root, bridge message root/count, header-MMR root/length, and supply digest through `Hegemon.Native.BlockCommitmentAdmission.accepts_iff_commitment_preconditions` and companion rejection/precedence theorems, while generated examples check the helper used by native mined-block, announced-block, and replay admission. The native tx-leaf artifact kernel proves bounded byte-parser facts for serialized STARK/public-tx counts, proof-length caps, commitment row/coeff caps, backend-byte defaults, bad backend IDs, trailing bytes, and truncation through `Hegemon.Native.TxLeafArtifact.valid_artifact_parses` and the companion rejection theorems, while generated examples check production `superneo-hegemon` decoding and canonical re-encoding. The native receipt-root kernel proves bounded parser and structural schedule facts for leaf/fold caps, non-empty expected leaf agreement, `fold_count = leaf_count - 1`, exact fold challenge count, exact parent-row dimensions, trailing bytes, and truncation through `Hegemon.Native.ReceiptRoot.valid_single_parses`, `Hegemon.Native.ReceiptRoot.two_leaf_schedule_accepts`, `Hegemon.Native.ReceiptRoot.three_leaf_schedule_accepts`, and the companion rejection theorems, while generated examples check production `superneo-hegemon` decoding, canonical re-encoding, and the structural validator called before backend fold verification. The transaction-balance kernel proves valid balances have concrete slots, slot overflow is rejected, the native slot delta equals `fee - value_balance`, and stablecoin/non-native conservation rules are enforced through `Hegemon.Transaction.validBalance_has_slots`, `Hegemon.Transaction.validBalance_rejects_slot_overflow`, `Hegemon.Transaction.validBalance_native_delta`, and `Hegemon.Transaction.validBalance_stablecoin_rules`, while generated examples check production `TransactionWitness::balance_slots` and `TransactionWitness::validate` behavior. The transaction Merkle-path kernel proves empty-path behavior, even/odd position-bit orientation, computed-root acceptance, wrong-length rejection, and wrong-root rejection through `Hegemon.Transaction.foldPath_empty`, `Hegemon.Transaction.foldPath_even_step`, `Hegemon.Transaction.foldPath_odd_step`, `Hegemon.Transaction.verifyPath_accepts_computed_root`, `Hegemon.Transaction.verifyPath_rejects_wrong_length`, and `Hegemon.Transaction.verifyPath_rejects_wrong_root`, while generated examples check production `MerklePath` fold behavior used by transaction membership validation. The transaction public-input shape kernel proves fixed vector widths, boolean flags, inactive-field zeroing, active nullifier/commitment nonzero checks, nonempty transaction admission, canonical balance-slot asset ordering, and stablecoin asset membership through `Hegemon.Transaction.PublicInputs.validPublicInputShape_accepts_valid` and the companion rejection theorems, while generated examples check production `TransactionPublicInputsP3::validate`. The transaction public-input binding kernel proves public/serialized field agreement for merkle root, fee, signed value balance, balance-slot assets, stablecoin policy, oracle, and attestation payload through `Hegemon.Transaction.PublicInputBinding.bindPublicInputs_accepts_valid` and the companion stablecoin, fallback, and mismatch theorems, while generated examples check production `transaction_public_inputs_p3_from_parts`. The transaction statement-hash kernel proves domain, padding, little-endian scalar, raw stablecoin flag, and signed balance preimage facts through `Hegemon.Transaction.StatementHash.statementPreimage_accepts_valid` and the companion rejection theorems, while generated examples check the shared Rust statement-hash helper used by transaction receipts and consensus tx-leaf binding. The shielded nullifier kernel proves zero, pending, and spent nullifier rejection through `Hegemon.Shielded.stage_rejects_zero`, `Hegemon.Shielded.import_rejects_zero`, `Hegemon.Shielded.stage_prevents_duplicate_pending`, `Hegemon.Shielded.import_prevents_reimport`, and `Hegemon.Shielded.import_prevents_restaging`, while generated examples check production `protocol-shielded-pool::NullifierState` behavior used by the native node. This is real theorem plus conformance evidence, but it is narrow: it does not prove BLAKE3 replay-key derivation, BLAKE3 pending-action-hash collision resistance, BLAKE3 header-MMR/checkpoint-output transcript security or implementation equivalence, BLAKE2 action-order key derivation, complete bridge light-client soundness, MMR hash binding, FlyClient sampling soundness, SHA-256 implementation equivalence, retarget economics, full network/finality behavior, monetary policy economics, DA Reed-Solomon correctness, DA sampling probability, DA chunk network availability, BLAKE3 DA-root security or implementation equivalence, aggregation STARK soundness, recursive proof cryptographic soundness, native lattice backend security, native tx-leaf proof soundness, native receipt-root cryptographic fold soundness, commitment-tree Merkle hash security or implementation equivalence, note commitment correctness, Poseidon2 Merkle hash security or implementation equivalence, nullifier derivation, ciphertext hash derivation, BLAKE3 statement-hash collision resistance or implementation equivalence, full AIR/proof-system soundness, or full native-node equivalence. The claims ledger is the source of record for formal-security claim status, production eligibility, gates, and residual risks. Documentation may summarize it, but must not promote a claim beyond the ledger status. The blueprint DAG is a JSON review map for the claims ledger. It makes dependencies, target review, implementation bindings, falsification cases, Lean theorem evidence, and scope boundaries machine-checkable, and the gate rejects missing nodes, dangling edges, self-dependencies, dependency cycles, missing implementation or evidence paths, and claim/blueprint drift so stale evidence cannot silently support a production claim. On the implementation side, `circuits/transaction/tests/security_fuzz.rs` performs property-based fuzzing of `TransactionWitness::balance_slots` and `public_inputs` to catch serialization edge cases. The fuzz harness remains in the `security-adversarial` CI job, while the Lean proof, formal inventory, blueprint, conformance-vector, and reference-vector checks run in the dedicated `formal-core` CI job.

Native DA sidecar upload admission is now represented separately in Lean: generated vectors check `da_submitCiphertexts`/`da_submitProofs` request-count caps, staged sidecar capacity with replacement semantics, proof binding-hash metadata, nonempty proof admission, exact proof-cap acceptance, and decoded proof oversize rejection against the production native helpers. This proof does not prove RPC authentication, public exposure safety for unsafe methods, base64/hex parser correctness, ciphertext hash security or implementation equivalence, sled durability, DA sidecar availability, native tx-leaf proof soundness, or complete native-node equivalence.
...
* Difficulty retargeting is deterministic: every `RETARGET_WINDOW = 10` blocks the chain recomputes the target from the window's
  ...
  timestamps cannot skew difficulty even after a reorg. The native verifier also requires the compact difficulty bits
