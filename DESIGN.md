...
The SmallWood branch is now past the old scalar fallback. The Rust engine exposes sparse linear constraints, the packed four-lane semantic relation over the native witness is real, the packed witness satisfies the LPPC checks, and the integrated candidate prover/verifier roundtrip now works. The old vendored C bridge is no longer part of the active backend path. But it is still not the promised proof-size win: the current packed candidate proof is about `1.35 MB`, which is above both the shipped `354081`-byte Plonky3 proof and the current `524288`-byte native `tx_leaf` cap. So the blocker is no longer missing semantics or missing Rust ownership; it is that the current LPPC geometry is still too row-heavy. The earlier `75 KB .. 121 KB` SmallWood numbers therefore remain the expected range only once a smaller packed relation lands, not the performance of the current integrated candidate backend. The exact current tx-proof sweep and soundness note for the shipped Plonky3 line live in `docs/crypto/tx_proof_profile_sweep.json` and `docs/crypto/tx_proof_soundness_analysis.md`. The first conservative STIR replacement spike is also checked in under `docs/crypto/tx_proof_stir_spike.json` / `docs/crypto/tx_proof_stir_soundness.md`; it clears the security gate but only projects to about a `1.30x` total-byte reduction, so it is not currently a sufficient reason to replace the shipped tx proof opening layer. With 384-bit digests, PQ collision resistance reaches ~128 bits for application-level commitments, and 48-byte encodings are used end-to-end.
