...
The current backend is no longer a pure digest mock. It now implements a concrete SuperNeo-style experimental path in stable Rust: packed witness values are decomposed into low-bit digits, embedded into small negacyclic ring elements over Goldilocks, committed with a deterministic Ajtai-style public matrix, and then folded with transcript-derived linear challenges. The compact-leaf step was also tightened: tx-leaf proofs no longer carry the packed witness back inside the proof object. Instead, tx-leaf verification reconstructs the expected packed witness deterministically from the on-chain tx public view plus the serialized STARK public inputs carried in the artifact, re-commits that witness under the backend key, and checks the resulting commitment against the tx-leaf proof digest. The experimental relation layer now has two serious lanes. `TxLeafPublicRelation` is the public bridge over nullifiers, commitments, ciphertext hashes, balance tag, version binding, and serialized STARK public inputs, and both the proof-ready-leaf topology and the older bridge benchmark reuse it. `NativeTxValidityRelation` is the first witness-driven native lane: it consumes `TransactionWitness` directly, checks witness validation plus Merkle membership locally, derives the canonical public-input object without going through a Plonky3 proof, and benchmarks folding over that native witness. The experimental `ReceiptRoot` import path still consumes standalone `TxLeaf` artifacts instead of decoding inline tx STARK proof bytes on that lane, and the benchmark harness now records three SuperNeo-adjacent surfaces separately: the native witness lane, the proof-ready-leaf topology, and the older bridge path. This is still not Neo/SuperNeo’s full commitment stack and should not be treated as a security-equivalent substitute for the papers: the in-repo backend still stops short of the exact Module-SIS commitment analysis, decomposition reduction, and sum-check machinery, and the new native lane is still benchmark-only because hidden-witness verification is not yet compactly represented in the artifact path. The experiment remains benchmarkable and deletable, and it is still not consensus-relevant beyond the additive receipt-root research path. The naming rule in code follows the same boundary: generic layers speak in `ProofEnvelope`, `TxValidityReceipt`, `TxValidityArtifact`, `TxLeaf`, and `ReceiptRoot` terms, while `superneo-*` names stay confined to the experimental backend crates that implement one concrete receipt-root backend today.
