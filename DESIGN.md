...
The SmallWood branch is now past the old scalar fallback. The Rust engine exposes sparse linear constraints, the active packed semantic bridge over the native witness is real, the packed witness satisfies the LPPC checks, and the integrated candidate prover/verifier roundtrip exists on the live backend seam. The old vendored C bridge is no longer part of the active backend path. The important distinction is now explicit: the failed flat `934 x 64` chunked direct layout is no longer treated as a live proving target on this PCS. Both live arithmetizations, `Bridge64V1` and `DirectPacked64V1`, now use the same row-aligned `1416`-row local-gate geometry (`raw_witness_len = 264`, `poseidon_permutation_count = 143`, `poseidon_state_row_count = 4576`, `expanded_witness_len = 90624`, `lppc_packing_factor = 64`) because that is the geometry the current row-polynomial PCS can actually prove succinctly. The latest frontend shrink removed the duplicated public witness rows from the bridge proving object and now carries public values directly in the statement/transcript path, while the earlier bridge shrinks already removed the duplicated input-position, stable-selector, and input/output selector rows. The engine/native boundary is explicit too: prove, verify, and projected-size calls consume a `SmallwoodConstraintAdapter` statement object instead of raw bridge-layout tuples, so bridge-specific nonlinear assumptions are isolated to one adapter seam. `DirectPacked64V1` is no longer a witness-carrying alternate envelope; it is now a second succinct statement mode on the normal row-scalar PCS/opening path, with the arithmetization tag bound in the proof wrapper but no raw-witness payload and no matrix-opening side payload. Under the current exact no-grinding `128-bit` profile (`rho = 2`, `nb_opened_evals = 3`, `beta = 2`, `decs_nb_evals = 16384`, `decs_nb_opened_evals = 29`, `decs_eta = 3`, zero grinding bits), the exact current serialized proof envelope for the live packed candidate now projects in-repo to `106884` bytes, and the passing release roundtrip emits `106884` proof bytes. That is about `3.31x` smaller than the shipped `354081`-byte Plonky3 proof while still staying below the current `524288`-byte native `tx_leaf` cap. The latest focused local release runs complete in about `2.23s` directly in `transaction-circuit` and about `2.69s` through the wrapped `superneo-hegemon` `tx_leaf` seam. The direct row-aligned mode is now required by regression to stay at or below that same bridge baseline instead of carrying a side payload to fake progress.

The latest runtime/front-end attack therefore was not another loop trick; it combined the smaller `1416`-row bridge, the secure `beta = 2` LVCS geometry, the `16384 / 29 / 3` DECS point, the earlier preallocated DECS row buffers with thread-local scratch, and the proof-object cleanup for explicit arithmetization tags without witness-carrying alternate payloads. The verifier also now fail-closes on exact inner proof shapes, requires distinct DECS opening indices, binds the full PCS commitment transcript into the PIOP transcript, hashes full opened combis for the DECS challenge, derives both commitment-time and verifier-time polynomial openings from the exact interpolation domain rather than the earlier broken shortcut helpers, and rejects bridge/direct opening-mode mismatches explicitly. So malformed candidate proofs no longer panic the release verifier, forged self-consistent PCS layers are rejected, and the implemented query count matches the note. The branch is therefore no longer blocked on raw proof size, and it is materially less blocked on proving cost than before. The earlier `75 KB .. 121 KB` SmallWood numbers remain a separate structural research target for a future PCS-aware layout change, not the current proving object. The exact current tx-proof sweep and soundness note for the shipped Plonky3 line live in `docs/crypto/tx_proof_profile_sweep.json` and `docs/crypto/tx_proof_soundness_analysis.md`. The first conservative STIR replacement spike is also checked in under `docs/crypto/tx_proof_stir_spike.json` / `docs/crypto/tx_proof_stir_soundness.md`; it clears the security gate but only projects to about a `1.30x` total-byte reduction, so it is not currently a sufficient reason to replace the shipped tx proof opening layer. With 384-bit digests, PQ collision resistance reaches ~128 bits for application-level commitments, and 48-byte encodings are used end-to-end.
