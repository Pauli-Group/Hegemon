...
Concrete modules in the repository now reflect this plan. A dedicated `state/merkle` crate maintains the append-only commitment tree with poseidon-style hashing, while the `circuits/block` crate keeps the legacy `prove_block`/`verify_block` path (root trace plus a `RecursiveAggregation` digest) and also exposes `circuits/block::recursive` for real `RecursiveBlockProof` generation and verification. Consensus can validate a block by comparing the header’s `recursive_proof_hash` against an attached `RecursiveBlockProof` and verifying the recursive proof payload once, instead of re-verifying each transaction proof individually. Miners still call `prove_block` with their current tree before attempting a PoW solution until mining emits recursive proofs by default; at that point the aggregation digest becomes fully legacy. The `protocol-versioning` crate defines a `VersionBinding { circuit, crypto }` pair plus helpers for recording per-block version matrices. Every transaction now commits to its binding, the block header publishes a 32-byte `version_commitment`, and the block circuit still records `version_counts` so consensus can attest how many transactions ran under each circuit/primitive pair even when miners operate autonomously.
