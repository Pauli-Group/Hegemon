...
- **Formal verification** – TLA+ specs under `circuits/formal/` (transaction balance) and `consensus/spec/formal/` (PoW fork choice/finality), Lean kernels under `formal/lean`, and `config/formal-security-claims.json` are part of the release checklist. Any modification to witness layouts, balance tags, bridge message semantics, proof-artifact byte grammar, supply-digest chain replay, or consensus phases must update the corresponding spec/vector/claim file and README.
