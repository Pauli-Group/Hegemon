...
* Native sync, metadata, and action decoding are exact-admission gates: bounded sync messages reject legacy bincode and trailing bytes, SCALE and bincode helpers reject unconsumed bytes, and block action payload counts must match the header before semantic validation. After exact decode, block actions must also satisfy embedded `tx_hash` recomputation and uniqueness before import or decode-only consumers observe them. Persisted pending-action reload is represented by a separate Lean-conformance-checked admission table: malformed sled keys, sled-key/embedded-`tx_hash` mismatch, recomputed `pending_action_hash` mismatch, and duplicate loaded hashes reject before locally reloaded actions can enter mining selection. These codec, action-hash, and pending-action reload boundaries are represented in Lean and checked against the production native helpers.
