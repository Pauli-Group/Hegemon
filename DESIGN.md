...
Bridge messages are protocol actions, not log scraping. `protocol-kernel` defines the bridge family and `BridgeMessageV1`; outbound bridge actions are ordered into a canonical `message_root`, while inbound bridge actions consume proven messages once with replay protection keyed by `(source_chain_id, source_message_nonce)`. Hegemon-to-Hegemon bridging now uses a RISC Zero receipt envelope whose authenticated journal is the fixed-width 404-byte `BridgeCheckpointOutputV1` wire tuple; the destination verifies `Receipt::verify(image_id)`, checks the journal/output against the inbound message, and never uses the receipt for fork choice. RISC Zero is a bridge transport dependency of the native node, not a dependency of `consensus-light-client`, and the PQ path must verify native STARK/hash-based receipts rather than Groth16/KZG wrappers.

The bridge statement is intentionally proof-system agile. `zk/cashvm-bridge` is the first non-RISC-Zero adapter experiment: it maps the same Hegemon `BridgeCheckpointOutputV1` plus `BridgeMessageV1` into SHA-256-friendly CashVM objects, models a 128-byte CashToken state commitment, and measures BCH-style standardness constraints. Current measurements show the 9951-byte compact Hegemon long-range proof input and 516-byte CashVM bridge output fit one standard CashVM transaction model, but the measured RISC Zero STARK receipt envelopes do not: the 224508-byte succinct envelope needs about 23 stack-sized pushes or 3 proof-fragment transactions at a 90000-byte fragment payload, while the 492158-byte composite envelope needs about 50 stack-sized pushes or 6 fragment transactions. That is why a production BCH landing area should target a CashVM-native STARK/hash verifier object, not a raw RISC Zero receipt blob.
