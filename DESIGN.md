...
* a real packed Rust frontend material builder that matches the frozen `64`-lane target statement (`raw_witness_len = 3991`, `poseidon_permutation_count = 145`, `expanded_witness_len = 59749`, `lppc_row_count = 934`, `lppc_packing_factor = 64`).

The SmallWood branch is now past the old scalar fallback. The Rust engine exposes sparse linear constraints, the active `64`-lane packed semantic bridge over the native witness is real, the packed witness satisfies the LPPC checks, and the integrated candidate prover/verifier roundtrip exists on the live backend seam. The old vendored C bridge is no longer part of the active backend path. The exact current serialized proof envelope for that packed candidate now projects in-repo to `242124` bytes, and the passing release roundtrip emits `242132` proof bytes. Both are below the shipped `354081`-byte Plonky3 proof and the current `524288`-byte native `tx_leaf` cap. So the blocker is no longer proof size. The blocker is prover runtime: after parallel witness interpolation and the new consecutive-domain DECS evaluation cache, the current Rust SmallWood release prover is down to about `9.8s` for the direct `transaction-circuit` roundtrip and about `8.4s` once wrapped through the `superneo-hegemon` `tx_leaf` seam on the focused release tests. The traced hot path is now dominated by LVCS row interpolation, DECS domain-evaluation extension, and PIOP constraint construction rather than Merkle leaf hashing. The earlier `75 KB .. 121 KB` SmallWood numbers therefore remain the next structural target, not the current integrated candidate size. The exact current tx-proof sweep and soundness note for the shipped Plonky3 line live in `docs/crypto/tx_proof_profile_sweep.json` and `docs/crypto/tx_proof_soundness_analysis.md`. The first conservative STIR replacement spike is also checked in under `docs/crypto/tx_proof_stir_spike.json` / `docs/crypto/tx_proof_stir_soundness.md`; it clears the security gate but only projects to about a `1.30x` total-byte reduction, so it is not currently a sufficient reason to replace the shipped tx proof opening layer. With 384-bit digests, PQ collision resistance reaches ~128 bits for application-level commitments, and 48-byte encodings are used end-to-end.
