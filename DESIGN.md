...
STARK verifier parameters (hash function choice, query counts, blowup factors, field extension) should be treated as protocol-release parameters, not live governance knobs. The live transaction-proof family is now also a version-owned protocol parameter: `protocol/versioning` and `runtime/src/manifest.rs` commit both the active tx proof backend and, for the current Plonky3 line, the release tx FRI profile. The active default binding still resolves to `backend = Plonky3Fri` with `(log_blowup = 4, num_queries = 32, query_pow_bits = 0)`, while a non-default `SMALLWOOD_CANDIDATE_VERSION_BINDING` now resolves cleanly to `backend = SmallwoodCandidate`. That candidate path already runs through the exact same `tx_leaf -> receipt_root` aggregation lane. Its proof bytes now contain a real vendored SmallWood PCS/ARK transcript over the packed expanded native witness, using a Hegemon-local Goldilocks BLAKE3 XOF/hash adapter and a witness-free public statement made of the direct `TransactionPublicInputsP3` field surface plus fixed witness-shape metadata. The old transcript-bound dense random linear checks are gone; the current candidate backend uses sparse public selector constraints over the packed expanded witness instead. The backend seam is therefore real, not a placeholder, and the repo now carries an exact no-grinding note for the implemented witness-free statement in `docs/crypto/tx_proof_smallwood_no_grinding_soundness.md`. What is still transitional is the semantic layer: the current SmallWood candidate does not yet encode full native tx validity inside the SmallWood polynomial constraints, so it remains a non-default candidate backend rather than a release tx proof. The exact current tx-proof sweep and soundness note live in `docs/crypto/tx_proof_profile_sweep.json` and `docs/crypto/tx_proof_soundness_analysis.md`. The first conservative STIR replacement spike is also checked in under `docs/crypto/tx_proof_stir_spike.json` / `docs/crypto/tx_proof_stir_soundness.md`; it clears the security gate but only projects to about a `1.30x` total-byte reduction, so it is not currently a sufficient reason to replace the shipped tx proof opening layer. With 384-bit digests, PQ collision resistance reaches ~128 bits for application-level commitments, and 48-byte encodings are used end-to-end.
