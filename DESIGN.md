...
Block-proof compatibility is now hard-cut to `BlockProofBundle` schema `2` with proof format id `5`. Import fail-closes on legacy payloads. The payload supports explicit proof modes: `FlatBatches` (deterministic contiguous proof batches over canonical tx order) and `MergeRoot` (root recursion payload with bound tree metadata + leaf manifest commitment). `FlatBatches` now has two proof kinds behind the same envelope: legacy witness-batch STARKs for trusted/local use, and a proof-bytes `ProofBatch` prototype for the public-market recovery work. The proof-bytes path removes spend-witness leakage from the market interface because workers receive canonical transaction proof bytes plus decoded public inputs instead of `TransactionWitness`, and the chunk payload exposes ordered statement hashes, padded nullifiers, padded commitments, and fee summary as public outputs. It is not yet the accepted end-state throughput primitive; until the replacement crate becomes a real fixed-shape batch prover with passing measurements, it should be treated as the recovery prototype rather than a finished permissionless lane. In `FlatBatches`, consensus verifies full contiguous coverage of `[0, tx_count)` with no gaps/overlaps and validates each batch artifact against the corresponding transaction subset. In `MergeRoot`, consensus verifies the root artifact and bound metadata under the same `tx_statements_commitment` and `tx_count` commitment constraints. Runtime and operator controls for this path are explicit: `HEGEMON_BATCH_SLOT_TXS` (`16` default, then `32`/`64` during throughput rollout), `HEGEMON_BLOCK_PROOF_MODE` (`flat` or `merge_root`), `HEGEMON_MERGE_ARITY` (defaults to `HEGEMON_AGG_MERGE_FANIN`, so `2` on the live lane unless overridden), `HEGEMON_AGG_STAGE_LOCAL_PARALLELISM`, `HEGEMON_AGG_STAGE_QUEUE_DEPTH`, and `HEGEMON_PROVER_STAGE_MAX_INFLIGHT_PER_LEVEL`. Sync admission remains strict: unknown peers must pass a compatibility probe that matches local `genesis_hash`, `sync_protocol_version`, and proof format id before discovery/sync traffic treats them as eligible.

For the open prover market, coordinator work identity is now parent-independent on the heavy lane: `candidate_set_id` and proof-batch chunk `package_id` depend only on the ordered transaction set and chunk range, not on `parent_hash` or `block_number`. Parent binding is deferred to the final commitment proof stage, which remains the only parent-specific proof artifact on the live flat lane.
