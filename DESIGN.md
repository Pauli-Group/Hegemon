...
The current direct-packed mode is no longer “unsupported” at the engine/frontend seam. It now proves and verifies as an explicit alternate arithmetization with a witness-carrying payload and an explicit arithmetization tag in the proof envelope. It is still not the default product path because that direct payload is `478389` bytes, versus `106941` projected / `106949` actual bytes for the compact bridge path.
