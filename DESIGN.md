...
Native local mined-work admission is represented separately in Lean: generated vectors check current-best parent-hash agreement and checked next-height arithmetic against the production mined-block import helper before pending-action replay, body commitment checks, supply accounting, and PoW metadata verification. Native mined-block import now also verifies final PoW metadata before pending-action mutation, persistence, or broadcast, and the blueprint gate plus regression coverage keep invalid local seals from partially mutating pending, commitment, or supply state. This proof does not prove PoW hash-threshold validity beyond the production verifier call, action validity, storage durability, fork-choice liveness, or complete native-node equivalence.
