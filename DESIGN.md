...
The v0.10.0 native header surface now exposes bridge-friendly commitments: `chain_id`, `rules_hash`, `cumulative_work`, `kernel_root`, `message_root`, `message_count`, and `header_mmr_root`/`header_mmr_len`. The header MMR is a real Merkle Mountain Range over historical header hashes, and outbound bridge witnesses use compact MMR openings plus deterministic FlyClient-style samples instead of replaying every header from genesis. The shared `consensus-light-client` crate owns the deterministic `PowHeaderV1` verifier and is intentionally `no_std + alloc` compatible so the same logic can be used by the node, witness builders, and zkVM guests. The native node still performs full validation of shielded state, nullifiers, recursive block artifacts, DA roots, supply accounting, and future-skew bounds outside that minimal light-client kernel.
