...
use super::rpo_air::{RpoProver, ROWS_PER_PERMUTATION, STATE_WIDTH, TRACE_WIDTH};
use super::rpo_proof::{rpo_hash_bytes, rpo_hash_elements, rpo_merge, RpoProofOptions};
...
fn test_rpo_air_dimensions() {
    ...
    assert_eq!(
        TRACE_WIDTH, 13,
        "Trace width should be 13 (12 state + 1 round)"
    );
...
fn test_rpo_stark_proof_generation_and_verification() {
    ...
    let input_state: [BaseElement; STATE_WIDTH] =
        core::array::from_fn(|i| BaseElement::new((i as u64 + 1) * 12345));
    ...
    // Verify the proof
    let verified = winterfell::verify::<
        RpoAir,
        Blake3,
...
fn test_rpo_proof_invalid_output_fails() {
    ...
    let input_state: [BaseElement; STATE_WIDTH] =
        core::array::from_fn(|i| BaseElement::new(i as u64 + 1));
    ...
    let verified = winterfell::verify::<
        ...
        DefaultRandomCoin<Blake3>,
        MerkleTree<Blake3>,
    >(proof, wrong_pub_inputs, &acceptable_options);
    assert!(
        verified.is_err(),
