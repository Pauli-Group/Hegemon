...
namespace SmallWoodSmz8ProofWire

open CanonicalBytes
open SmallWoodProofWire

def openedLeafCount : Nat := 19
...
/-- SMZ8 uses the compact authentication-path grammar, but fixes the query
count to 19 and the maximum individual path depth to 23. Zero-length
individual paths are canonical because other opened leaves can supply every
sibling subtree in the aggregate multiproof. -/
...
theorem decodeAuthPathsPrefix_sound
  ...
  | some pair =>
      ...
      split at decoded
      · rename_i exactShape
        have pairEq : (candidate, rest) = (paths, suffix) :=
...
def proofMagic : List Byte := [83, 77, 90, 56]
...
/-- Exact SMZ8 decoding checks the 131072-byte cap before entering any
collection decoder, then requires exact consumption. -/
...
theorem decodeProofExact_sound
    ...
    proof.Canonical
      ∧ input = proof.encode
      ∧ input.length ≤ maximumInnerProofBytes := by
  ...
  · rename_i withinCap
    ...
    | some prefixPair =>
        ...
        subst parsedProof
        subst suffix
        rcases decodeProofPrefix_sound prefixResult with
          ⟨canonical, inputEq⟩
...
theorem decodeProofExact_rejects_oversize
    {input : List Byte}
    (oversize : maximumInnerProofBytes < input.length) :
...
def makeDecs (authPaths : AuthPathsWire) : DecsWire :=
  { authPaths,
    leafTapeBytes := zeroBytes openedLeafTapesBytes,
...
def wrongMagicBytes : List Byte :=
  [0, 77, 90, 56] ++ proofPayloadCodec.encode canonicalMinimalProof
...
theorem canonical_minimal_proof_is_canonical :
  ...
  simp [ProofWire.Canonical, proofPayloadCodec, PrefixCodec.xmap,
    ...
    openedWitnessCodec, matrixCodec, authPathsCodec, AuthPathsCanonical,
    canonicalMinimalProof, makeProof, zeroPiop, makePcs, makeDecs, zeroLengthAuthPaths,
    zeroMatrix, zeroBytes, openedLeafCount, openedLeafTapeBytes,
    openedLeafTapesBytes, maximumAuthPathDepth, MatrixWire.Canonical,
...
theorem canonical_minimal_proof_within_cap :
    canonicalMinimalProof.encode.length ≤ maximumInnerProofBytes := by
  decide

theorem canonical_minimal_proof_roundtrips :
    decodeProofExact canonicalMinimalProof.encode = some canonicalMinimalProof := by
  exact decodeProofExact_encode canonicalMinimalProof
    canonical_minimal_proof_is_canonical canonical_minimal_proof_within_cap

theorem wrong_magic_rejects : decodeProofExact wrongMagicBytes = none := by
  decide

theorem wrong_path_count_rejects :
    decodeProofExact wrongCountProof.encode = none := by
  decide

theorem excessive_path_depth_rejects :
    decodeProofExact excessiveDepthProof.encode = none := by
  decide

theorem trailing_byte_rejects : decodeProofExact trailingBytes = none := by
  decide

end Examples

end SmallWoodSmz8ProofWire
end HegemonCrypto
