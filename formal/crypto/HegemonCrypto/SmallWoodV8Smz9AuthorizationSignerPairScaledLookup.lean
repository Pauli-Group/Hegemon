...
theorem signer_pair_scaled_exact {left right : Nat} (ordered : left < right)
    (leftBound : left < 6) (rightBound : right < 6) :
    exactNonlinearExpressions[1855 + 9 * signerPairIndexLookup left right]? =
      some (.mul 1234 (1853 + 9 * signerPairIndexLookup left right)) := by
