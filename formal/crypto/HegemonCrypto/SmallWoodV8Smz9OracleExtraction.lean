...
/-!
# Deterministic HGV8RP03/SMZ9 oracle extraction

This file defines the candidate inverse needed by an SMZ9 analogue of
`extracted_production_oracles_satisfied_iff_relation`.  It starts from the exact ideal
`2^23 x 145` committed-oracle shape, interpolates its 140 non-masking rows on the production
disjoint coset, applies the exact inverse index map for `140 x 368` LVCS stacking, selects the
first 686 candidate degree-69 witness polynomials, and materializes their values at the 64
packing points in the row-major order consumed by the HGV8RP03 executable relation program.

For an arbitrary oracle this is only a deterministic candidate inverse.  Proving that verifier
acceptance makes the committed rows a close enough degree-bounded DECS codeword, and therefore
that these candidates are the committed witness, is a separate missing soundness step.

The final equivalence below is deliberately an executable-program statement.  Upgrading
program acceptance to `ExactV8RelationSemanticValid` still requires the separately named
universal semantic-adequacy refinement; this module does not manufacture that receipt,
Fiat--Shamir binding, commitment binding, or production authority.
-/

namespace HegemonCrypto.SmallWood.V8Smz9OracleExtraction
...
def decsOpenedEvaluations : Nat := V8Smz9LogicalOracle.decsOpeningCount
...
theorem exact_extraction_geometry :
    relationRows = 686 ∧ packingFactor = 64 ∧ decsDomainSize = 8388608 ∧
      decsOpenedEvaluations = 20 ∧ unstackedRowCount = 70 ∧
      unstackedColumnCount = 736 ∧ lvcsRowCount = 140 ∧
      lvcsColumnCount = 368 ∧ decsInterpolationPointCount = 388 := by
  decide

/-! ## Exact production disjoint coset -/

/-- The independently certified descriptor has exactly this extraction geometry. -/
theorem extraction_domain_matches_certified_disjoint_coset :
    decsDomainSize = V8Smz9DisjointCoset.domainSize ∧
      decsInterpolationPointCount = V8Smz9DisjointCoset.interpolationPointCount := by
  decide
...
theorem smz9_coset_disjoint_from_interpolation_domain
    ...
    smz9EvaluationPoint index ≠ (point.val : Goldilocks) :=
  V8Smz9DisjointCoset.disjoint_from_interpolation_domain index point
...
/--
The candidate interpolation really recovers a polynomial of degree at most 387 when the committed
...
-/
theorem interpolated_committed_row_eq_of_degree_387_codeword
    (oracle : CommittedOracle)
    (row : Fin lvcsRowCount)
...
/-- The inverse rotation reads the 368 data cells at interpolation points `20 .. 387`. -/
...
end HegemonCrypto.SmallWood.V8Smz9OracleExtraction
