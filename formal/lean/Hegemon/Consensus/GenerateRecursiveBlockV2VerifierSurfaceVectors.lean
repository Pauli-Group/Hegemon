...
def surfaceRejectJson : Option SurfaceReject -> String
  ...
  | some SurfaceReject.versionedCapMismatch => "\"versioned_cap_mismatch\""
...
def surfaceCaseJson (name mutation : String) (input : SurfaceInput) : String :=
  ...
  "    {\n"
    ...
    ++ "      \"versioned_cap_matches\": " ++ boolJson input.versionedCapMatches ++ ",\n"
...
def vectorJson : String :=
  "{\n"
    ...
    ++ surfaceCaseJson "versioned-cap-mismatch" "versioned_cap_mismatch"
      { validSurface with versionedCapMatches := false } ++ ",\n"
    ...
    ++ surfaceCaseJson "proof-aux-limb-count-overflow" "proof_aux_limb_count_overflow"
      { validSurface with proofTraceDecodes := false } ++ ",\n"
