import Hegemon.Transaction.Poseidon2V8RelationProgramHgv8rp04

namespace Hegemon
namespace Transaction
namespace Poseidon2V8ConstraintRefinementHgv8rp04
...
120-word statement, 128 live permutation calls in 128 lanes, two 64-lane groups, 150
...
`HGV8RP04` CSR program and executes the relation-id-bound nonlinear expression DAG in every one
of the 64 packed lanes.  Its 843,715-byte successor transcript, SHA-512 known answer, exact
...
def liveHashCallCount : Nat := 128
def paddedHashCallCount : Nat := 128
def dummyHashCallCount : Nat := 0
...
def nonlinearIdentityCount : Nat := 773
def minimumStatementLinearConstraintCount : Nat := 19838
def maximumStatementLinearConstraintCount : Nat := 20510
...
theorem source_derived_v8_geometry_is_exact :
    publicStatementWordCount = 120
      ...
      ∧ dummyZeroLinearConstraintCount = 0
      ∧ rawRowStart = 0
      ...
      ∧ nonlinearIdentityCount = 773
      ∧ minimumStatementLinearConstraintCount = 19838
      ∧ maximumStatementLinearConstraintCount = 20510
      ∧ enginePiopGammaWidth = 20510
      ∧ maximumSummedIdentityUnionCount = 21283
      ∧ packingFactor = 64
...
def callRoleTable : List CallRoleRange :=
  [ { name := "transaction_prf", start := 0, stop := 1 },
    ...
    { name := "input_0_nullifier", start := 36, stop := 38 },
    { name := "input_1_note", start := 38, stop := 41 },
    { name := "input_1_merkle", start := 41, stop := 73 },
    { name := "input_1_nullifier", start := 73, stop := 75 },
    { name := "output_0_note", start := 75, stop := 78 },
    { name := "output_1_note", start := 78, stop := 81 },
    { name := "action_intent", start := 81, stop := 96 },
    { name := "authorization_policy", start := 96, stop := 100 },
    { name := "authorization_current", start := 100, stop := 103 },
    { name := "authorization_next", start := 103, stop := 106 },
    { name := "authorization_value_lock", start := 106, stop := 107 },
    { name := "authorization_bound_current", start := 107, stop := 108 },
    { name := "authorization_bound_secondary", start := 108, stop := 109 },
    { name := "stable_config_chunks", start := 109, stop := 113 },
    { name := "stable_config_tree", start := 113, stop := 116 },
    { name := "stable_state_leaves", start := 116, stop := 118 },
    { name := "stable_paths", start := 118, stop := 126 },
    { name := "stable_issuer_commitment", start := 126, stop := 127 },
    { name := "stable_issuer_authorization", start := 127, stop := 128 } ]

theorem call_role_table_is_the_exact_gap_free_partition :
    callRoleTable.length = 22
      ∧ callRoleTable.map (fun role => role.start) =
        [0, 1, 4, 36, 38, 41, 73, 75, 78, 81, 96, 100, 103, 106, 107, 108, 109, 113, 116, 118, 126, 127]
      ∧ callRoleTable.map (fun role => role.stop) =
        [1, 4, 36, 38, 41, 73, 75, 78, 81, 96, 100, 103, 106, 107, 108, 109, 113, 116, 118, 126, 127, 128]
...
are fixed by `Poseidon2V8RelationProgram`. `HGV8RP04` is only the shared format magic.
-/
...
structure FullRelationCompilerRefinementReceipt
  ...
  programComponentsCanonical : Poseidon2V8RelationProgramHgv8rp04.candidateCanonical programComponents
  ...
  programBytesExact :
    programBytes = Poseidon2V8RelationProgramHgv8rp04.canonicalProgramTranscript programComponents
  programBytesExactLength :
    programBytes.length = Poseidon2V8RelationProgramHgv8rp04.canonicalProgramTranscriptBytes
  ...
  programSha512Exact :
    programSha512 = Poseidon2V8RelationProgramHgv8rp04.canonicalProgramSha512
  ...
  minimumStatementLinearConstraintCountBound :
    minimumStatementLinearConstraintCount =
      Poseidon2V8ConstraintRefinementHgv8rp04.minimumStatementLinearConstraintCount
  maximumStatementLinearConstraintCount : Nat
  maximumStatementLinearConstraintCountBound :
    maximumStatementLinearConstraintCount =
      Poseidon2V8ConstraintRefinementHgv8rp04.maximumStatementLinearConstraintCount
...
end Poseidon2V8ConstraintRefinementHgv8rp04
end Transaction
end Hegemon
