...
This is real Lean theorem and conformance-vector evidence, but it is deliberately narrow. It does not prove BLAKE3 replay-key derivation, bridge light-client validity, external-chain covenant behavior, target arithmetic, header hashing, full network/finality behavior, full transaction balance conservation, AIR/proof-system soundness, or full native-node equivalence.
