...
Legacy sizing note: the historical `recursive_block_v1` container width is `699,404` bytes, but a current recursive-cap diagnostic shows that width is only validated through the first `StepA` terminal (`2` shielded tx on the current linear recursion chain). Steady-state `v1` recursion already projects above that width, so do not treat the old `699,404`-byte figure as a general packed-block law. The formula below is therefore only the legacy two-step `v1` envelope model; true constant-size recursion studies should use `RecursiveBlockV2` instead. Proofless sidecar transfers still add about `468` public on-chain bytes per tx and about `4,294` raw ciphertext bytes per tx in DA:

- on-chain growth: `G_on(T, k) ~= T * (0.0377 + 56.28 / k) GiB/day`
