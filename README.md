...
Blocks carry a commitment proof that binds the list of transaction proof hashes via a Poseidon sponge (`tx_proofs_commitment`) and exposes the starting/ending state roots, nullifier root, DA root, plus the transaction-ordered and sorted nullifier lists as public inputs. In the Substrate node, the proof bytes are carried on-chain via an unsigned `ShieldedPool::submit_commitment_proof` extrinsic, and validators accept a block by (1) verifying the commitment proof against public inputs derived from the block’s shielded transfers, (2) recomputing `tx_proofs_commitment` and verifying transaction proofs in parallel, and (3) sampling a small set of erasure-coded DA chunks chosen with per-node randomness and rejecting the block if any sample is missing or invalid. Legacy recursion machinery remains under `circuits/epoch` plus `circuits/block::recursive` for dev/test maintenance only.
...
The block circuit now emits `CommitmentBlockProof` payloads, making `tx_proofs_commitment` (and the proof’s `proof_hash`) the canonical validity commitments. Consensus verifies the commitment proof, then verifies transaction proofs in parallel and updates state roots locally.
