...
Blocks carry a commitment proof that binds the list of canonical transaction statement hashes via a Poseidon sponge (`tx_statements_commitment`) and exposes the starting/ending state roots, nullifier root, DA root, plus the transaction-ordered and sorted nullifier lists as public inputs. In the Substrate node, the proof bytes are carried on-chain via an unsigned `ShieldedPool::submit_commitment_proof` extrinsic, and validators accept a block by (1) verifying the commitment proof against public inputs derived from the block’s shielded transfers, (2) recomputing `tx_statements_commitment`, and (3) validating either an aggregation proof (`ShieldedPool::submit_aggregation_proof`) or the per-transaction proofs. In `ProofAvailabilityPolicy::SelfContained` aggregation mode, validators do not fetch proof-DA manifests/entries.
