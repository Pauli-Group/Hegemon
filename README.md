...
The block circuit now emits `RecursiveBlockProof` payloads directly, making `recursive_proof_hash` the canonical validity commitment alongside `da_root` and `da_params` for availability. Consensus no longer depends on a folded digest when a recursive proof is attached, and miners include the recursive proof bytes in the block payload so validators can verify one proof per block.
