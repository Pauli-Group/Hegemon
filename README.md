...
For operator sizing on the shipped lane, the constant on-chain shielded block artifact is `699,404` bytes (`recursive_block_v1`), while proofless sidecar transfers add about `468` public on-chain bytes per tx and about `4,294` raw ciphertext bytes per tx in DA. That gives the practical planning model:
