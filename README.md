...
The current block circuit still records a `RecursiveAggregation` digest as a bridge, but it now also exposes `RecursiveBlockProof` generation/verification so consensus can validate a single proof per block whenever the payload is attached. The scalability plan retires the aggregation digest once mining emits recursive proofs by default, leaving `recursive_proof_hash` and `da_root` in the block header alongside `da_params` as the canonical validity and availability commitments.
