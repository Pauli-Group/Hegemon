...
Blocks carry a commitment proof that binds the list of transaction proof hashes via a Poseidon sponge (`tx_proofs_commitment`) and exposes the starting/ending state roots, nullifier root, and DA root as public inputs. The block header commits to that proof via `commitment_proof_hash` and commits to the block payload via an erasure-coded data-availability root (`da_root`) plus `da_params` that fix chunk size and sample count. Validators accept a block by (1) verifying the commitment proof, (2) recomputing `tx_proofs_commitment` from the transaction proofs and verifying them in parallel, and (3) sampling a small set of chunks chosen with per-node randomness, requesting each chunk and its Merkle proof over P2P, and rejecting the block if any sample is missing or invalid. Legacy recursion machinery remains under `circuits/epoch` plus `circuits/block::recursive` for dev/test maintenance only.
