...
Stablecoin mint/burn commands (`wallet stablecoin-mint`, `wallet stablecoin-burn`) fetch the active `StablecoinPolicy` plus the latest oracle and attestation commitments over Substrate RPC, build the binding required by the circuit, and submit it through the Hegemon shielded RPC. Issuance failures surface early if the policy is inactive, the oracle commitment is stale, or the attestation is disputed.
