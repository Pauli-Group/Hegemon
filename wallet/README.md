...
| `cargo run -p wallet --bin wallet -- node-send --store ~/.synthetic/wallet.db --ws-url http://127.0.0.1:9944 --recipients recipients.json --fee 0` | Prompts for the passphrase, then builds and submits a shielded transaction via native node RPC and stores outgoing disclosure records for on-demand proofs. HTTP and WebSocket endpoints are both accepted. |
...
| `cargo run -p wallet --bin wallet -- payment-proof create --store ~/.synthetic/wallet.db --ws-url ws://127.0.0.1:9944 --tx 0x... --output 0 --out proof.json` | Prompts for the passphrase, then generates a disclosure package (payment proof) for a specific output. |
...
| `cargo run -p wallet --bin wallet -- payment-proof purge --store ~/.synthetic/wallet.db --tx 0x... --output 0` | Prompts for the passphrase, then purges stored outgoing disclosure records after proofs are delivered. |
