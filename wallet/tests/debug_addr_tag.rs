...
fn test_addr_tag_derivation_consistency() {
    ...
    println!(
        "  pk_recipient: {}",
        hex::encode(&addr_from_keys.pk_recipient)
    );
    ...
    println!(
        "  pk_recipient: {}",
        hex::encode(&addr_from_ivk.pk_recipient)
    );
    ...
    println!(
        "\nRe-derived from IVK (index={}):",
        decoded.diversifier_index
    );
    println!("  addr_tag: {}", hex::encode(&re_derived.addr_tag));
    println!("  pk_recipient: {}", hex::encode(&re_derived.pk_recipient));

    println!(
        "\nRe-derived addr_tag == decoded.address_tag: {}",
        re_derived.addr_tag == decoded.address_tag
    );

    assert_eq!(addr_from_keys, decoded, "Encode/decode should be lossless");
    assert_eq!(
        re_derived.addr_tag, decoded.address_tag,
        "Re-derived addr_tag should match encoded"
    );
}
