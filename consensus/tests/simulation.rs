...
use common::{
    assemble_bft_block, assemble_pow_block, dummy_transaction, make_validators, validator_set,
};
...
async fn bft_consensus_liveness_and_slashing() {
    ...
    let (block, _, _) = assemble_bft_block(
        1,
        1,
        [0u8; 32],
        1_000,
    ...
    let (conflict, _, _) = assemble_bft_block(
        1,
        1,
        [0u8; 32],
        1_050,
