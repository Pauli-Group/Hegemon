...
trap 'rm -f "$LEAN_BRIDGE_VECTORS" "$LEAN_SHIELDED_VECTORS" "$LEAN_CONSENSUS_VECTORS" "$LEAN_POW_VECTORS" "$LEAN_PROOF_POLICY_VECTORS" "$LEAN_SUPPLY_VECTORS" "$LEAN_ACTION_ORDER_VECTORS" "$LEAN_TRANSACTION_VECTORS" "$LEAN_MERKLE_VECTORS" "$LEAN_PUBLIC_INPUT_VECTORS"' EXIT
(
  cd "$ROOT/formal/lean"
