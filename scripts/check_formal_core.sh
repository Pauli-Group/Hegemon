...
run_exact_lib_test() {
  ...
  local test_name="$2"
  shift 2
  "$ROOT/scripts/run_exact_cargo_lib_test.sh" "$package" "$test_name" "$@"
}
...
LEAN_NATIVE_BACKEND_ALGEBRA_VECTORS="$(mktemp)"
...
LEAN_ESSENCE_CORE_VECTORS="$(mktemp)"
...
LEAN_CANONICAL_REORG_PERSISTENCE_ADMISSION_VECTORS="$(mktemp)"
...
LEAN_SYNC_BLOCK_CHUNK_ADMISSION_VECTORS="$(mktemp)"
...
LEAN_PRIVATE_MULTISIG_ACCUMULATOR_VECTORS="$(mktemp)"
...
LEAN_SMALLWOOD_PRODUCTION_CONSTRAINT_REFINEMENT_VECTORS="$(mktemp)"
LEAN_ACCEPTED_SMALLWOOD_BLOCK_COMPOSITION_VECTORS="$(mktemp)"
...
(
  ...
  lake exe gen_native_backend_algebra_vectors > "$LEAN_NATIVE_BACKEND_ALGEBRA_VECTORS"
  ...
  lake exe gen_essence_core_vectors > "$LEAN_ESSENCE_CORE_VECTORS"
  ...
  lake exe gen_canonical_reorg_persistence_admission_vectors > "$LEAN_CANONICAL_REORG_PERSISTENCE_ADMISSION_VECTORS"
  ...
  lake exe gen_sync_block_chunk_admission_vectors > "$LEAN_SYNC_BLOCK_CHUNK_ADMISSION_VECTORS"
  ...
  lake exe gen_private_multisig_accumulator_vectors > "$LEAN_PRIVATE_MULTISIG_ACCUMULATOR_VECTORS"
  ...
  lake exe gen_smallwood_production_constraint_refinement_vectors > "$LEAN_SMALLWOOD_PRODUCTION_CONSTRAINT_REFINEMENT_VECTORS"
  lake exe gen_accepted_smallwood_block_composition_vectors > "$LEAN_ACCEPTED_SMALLWOOD_BLOCK_COMPOSITION_VECTORS"
...
HEGEMON_LEAN_ATOMIC_COMMIT_MANIFEST_ADMISSION_VECTORS="$LEAN_ATOMIC_COMMIT_MANIFEST_ADMISSION_VECTORS" \
  run_exact_lib_test \
    hegemon-node \
    native::tests::lean_generated_atomic_commit_manifest_admission_vectors_match_production \
    --no-default-features
...
python3 "$ROOT/scripts/check_essence_core_vectors.py" \
  "$LEAN_ESSENCE_CORE_VECTORS" \
  "$ROOT/config/formal-essence-progress.json"
...
HEGEMON_LEAN_CANONICAL_REORG_PERSISTENCE_ADMISSION_VECTORS="$LEAN_CANONICAL_REORG_PERSISTENCE_ADMISSION_VECTORS" \
  run_exact_lib_test \
    hegemon-node \
    native::tests::lean_generated_canonical_reorg_persistence_admission_vectors_match_production \
    --no-default-features
...
HEGEMON_LEAN_MINED_BLOCK_COMMIT_PUBLICATION_VECTORS="$LEAN_MINED_BLOCK_COMMIT_PUBLICATION_VECTORS" \
  run_exact_lib_test \
    hegemon-node \
    native::tests::lean_generated_mined_block_commit_publication_vectors_match_production \
    --no-default-features
...
HEGEMON_LEAN_SYNC_BLOCK_CHUNK_ADMISSION_VECTORS="$LEAN_SYNC_BLOCK_CHUNK_ADMISSION_VECTORS" \
  run_exact_lib_test \
    hegemon-node \
    native::tests::lean_generated_sync_block_chunk_admission_vectors_match_production \
    --no-default-features
run_exact_lib_test \
  hegemon-node \
  native::tests::oversized_legacy_metadata_chunk_fallback_is_closed_at_v3_import_seam \
  --no-default-features
run_exact_lib_test \
  hegemon-node \
  native::tests::chunk_preflight_rejects_unsolicited_malformed_and_expired_sessions_without_growth \
  --no-default-features
run_exact_lib_test \
  hegemon-node \
  native::tests::chunk_completion_reserve_failure_cools_down_authorized_request \
  --no-default-features
run_exact_lib_test \
  hegemon-node \
  native::tests::announce_tip_is_unverified_request_planning_evidence_and_never_opens_gate \
  --no-default-features
run_exact_lib_test \
  hegemon-node \
  native::tests::native_sync_message_variant_ordinals_are_append_only \
  --no-default-features
run_exact_lib_test \
  hegemon-node \
  native::tests::native_sync_legacy_variant_bytes_remain_frozen_across_append_only_extensions \
  --no-default-features
run_exact_lib_test \
  hegemon-node \
  native::tests::native_sync_chunk_digest_domain_and_record_schema_are_independently_pinned \
  --no-default-features
HEGEMON_LEAN_SYNC_RAW_INGRESS_VECTORS="$LEAN_SYNC_RAW_INGRESS_VECTORS" \
  run_exact_lib_test \
    hegemon-node \
    native::tests::lean_generated_sync_raw_ingress_vectors_match_production \
    --no-default-features
HEGEMON_LEAN_SYNC_RESPONSE_IMPORT_VECTORS="$LEAN_SYNC_RESPONSE_IMPORT_VECTORS" \
  run_exact_lib_test \
    hegemon-node \
    native::tests::lean_generated_sync_response_import_vectors_match_production \
    --no-default-features
run_exact_lib_test \
  hegemon-node \
  native::tests::all_known_disconnected_winning_suffix_backfills_then_reorgs \
  --no-default-features
run_exact_lib_test \
  hegemon-node \
  native::tests::all_known_winning_sync_corrupt_parent_cycle_is_terminal \
  --no-default-features
...
# The production source manifest is intentionally empty until the retained
# proof and release gates authorize a route. Exercise that fail-closed wallet
# boundary here.
cargo test -p wallet fresh_transaction_builder_rejects_direct_artifact_bypass_before_proving --release --lib -- --nocapture
...
# The Poseidon2 production candidate uses the direct canonical SMZ9 carrier.
# SuperNEO receipt wrapping is retired and is not a production-readiness gate.
cargo test -p protocol-shielded-pool poseidon2_production_transport::tests::smz9_profile6_roundtrip_is_additive_exact_and_rejects_smz8_everywhere --lib -- --exact --nocapture
cargo test -p protocol-shielded-pool poseidon2_pending_action_artifact::tests::maximum_shape_codec_is_exact_and_every_frozen_outer_mutation_rejects --lib -- --exact --nocapture
...
HEGEMON_LEAN_PRIVATE_MULTISIG_ACCUMULATOR_VECTORS="$LEAN_PRIVATE_MULTISIG_ACCUMULATOR_VECTORS" \
  cargo test -p transaction-circuit private_multisig_accumulator_matches_lean_vectors -- --nocapture
...
HEGEMON_LEAN_SMALLWOOD_VERIFIER_STATEMENT_PROJECTION_VECTORS="$LEAN_SMALLWOOD_VERIFIER_STATEMENT_PROJECTION_VECTORS" \
  run_exact_lib_test \
    transaction-circuit \
    smallwood_frontend::tests::lean_generated_smallwood_verifier_statement_projection_vectors_match_production
HEGEMON_LEAN_SMALLWOOD_PRODUCTION_CONSTRAINT_REFINEMENT_VECTORS="$LEAN_SMALLWOOD_PRODUCTION_CONSTRAINT_REFINEMENT_VECTORS" \
  run_exact_lib_test \
    transaction-circuit \
    smallwood_frontend::tests::lean_generated_smallwood_production_constraint_maps_match_every_production_row
HEGEMON_LEAN_ACCEPTED_SMALLWOOD_BLOCK_COMPOSITION_VECTORS="$LEAN_ACCEPTED_SMALLWOOD_BLOCK_COMPOSITION_VECTORS" \
  run_exact_lib_test \
    consensus \
    proof::tests::lean_generated_accepted_smallwood_block_identity_vectors_match_production
HEGEMON_LEAN_ACCEPTED_SMALLWOOD_BLOCK_COMPOSITION_VECTORS="$LEAN_ACCEPTED_SMALLWOOD_BLOCK_COMPOSITION_VECTORS" \
  run_exact_lib_test \
    hegemon-node \
    native::tests::lean_generated_accepted_smallwood_block_supply_vectors_match_production \
    --no-default-features
...
cargo test -p tx-proof-manifest manifest_rejects_nested_proof_wrapper_admission_failures --release -- --nocapture
...
printf '\n[10/14] Checking formal security blueprint DAG\n'
