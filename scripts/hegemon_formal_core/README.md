...
The checker validates the formal/security claims ledger, validates the blueprint DAG in `config/formal-security-blueprint.json`, verifies independent bridge message vectors, and checks that the active TLA+ and Lean model files are present. The shell wrapper also builds the pinned Lean proof kernel through `scripts/check_lean_formal.sh`, checks generated Lean-to-Rust bridge conformance vectors through the `protocol-kernel` test target, and runs the existing `native-backend-ref` vector verifier for native tx-leaf and receipt-root proof artifacts.

The independent bridge-vector code must not depend on `protocol-kernel`; it reimplements the small canonical encoding and BLAKE3-XOF-48 derivations so production/reference drift is visible. The separate Lean conformance vector path intentionally runs inside the `protocol-kernel` test target and compares production bridge encoding plus `InboundReplayState` behavior against examples emitted by `formal/lean`.
