...
EXPECTED_BINDING_DESCRIPTORS = (
    Descriptor(
        0x0701,
        (8, 7, 1, 10, 2, 6, 4),
        b"hegemon.smallwood.poseidon2-v8.stablecoin-relation.v2\0SMZ9",
    ),
    Descriptor(
        0x0702,
        (120, FIELD_MODULUS),
        b"HGV8TX02.statement[0,120)->verifier.public[0,120);canonical-goldilocks",
    ),
    Descriptor(
        0x0703,
        (7, 87, 94),
        b"relation.binding[0,7)=expected-action-intent=call[93].final[0,7)",
    ),
    Descriptor(
        0x0704,
        (0, 247, 247, 5, 252, 31, 283, 364, 647, 39, 686),
        b"rows=raw[0,247);dense[247,252);inline[252,283);hash[283,647);stable[647,686)",
    ),
    Descriptor(
        0x0705,
        (125, 128, 16, 48),
        b"calls[125,128).initial[0,16)=0",
    ),
    Descriptor(0x0706, (0,), b"auxiliary-witness-words=0"),
    Descriptor(
        0x0707,
        (64, 8, 830, 19_935, 20_509, 21_339, 5, 6, 2, 23, 20, 5),
        b"DirectPacked64Poseidon2V8Sha512Smz9\0"
        b"Sha512Poseidon2V8Smz9\0rho5-open6-beta2-N23-q20-eta5",
    ),
    Descriptor(
        0x0708,
        (64, 48),
        b"SHA-512\0HGV8RP03-canonical-executable-program-prefix[0,48)",
    ),
)
...
def parse_components(data: bytes, *, verify_identity: bool = True) -> Components:
    ...
    expected_counts = (37, 56, 830, 86, 125, 8, 8271, 830, 565, 20605)
    ...
    if components.binding_descriptors != EXPECTED_BINDING_DESCRIPTORS:
        raise DecodeError("section 7: binding descriptors differ from exact HGV8RP03 contract")
...
def run_self_test(data: bytes) -> None:
    ...
    binding_mutation = bytearray(data)
    exact_binding = encode_descriptor(components.binding_descriptors[6])
    binding_offset = data.find(exact_binding)
    if binding_offset < 0 or data.find(exact_binding, binding_offset + 1) >= 0:
        raise AssertionError("binding descriptor self-test target is not unique")
    binding_mutation[binding_offset + 4 + 3 * 8] ^= 1
    expect_decode_error(
        "binding descriptor geometry mutation",
        lambda: parse_components(bytes(binding_mutation), verify_identity=False),
        "binding descriptors differ",
    )
...
def emit_lean(components: Components, input_path: Path) -> str:
    out = [
        ...
        "853,429-byte HGV8RP03 program artifact. The generator rejects any input whose SHA-512",
    ...
    out.extend(
        [
            ...
            "      exactCsrAttempts.length = 20605 := by",
            "  decide",
            "",
