...
if [[ "$IGNORED_COUNT" -ne 1 ]]; then
  printf 'expected Rust lib test %s in package %s to be ignored, found %s ignored matches\n' \
    "$TEST_NAME" "$PACKAGE" "$IGNORED_COUNT" >&2
  exit 97
fi
...
if ! grep -Eq 'test result: ok\. 1 passed; 0 failed; 0 ignored;' <<<"$OUTPUT"; then
  printf 'required ignored Rust lib test %s in package %s did not execute exactly once\n' \
    "$TEST_NAME" "$PACKAGE" >&2
  exit 97
fi
