...
STRICT_PREPARE_TIMEOUT_SECS="${HEGEMON_SCALE_STRICT_PREPARE_TIMEOUT_SECS:-7200}"
BATCH_JOB_TIMEOUT_MS="${HEGEMON_SCALE_BATCH_JOB_TIMEOUT_MS:-7200000}"
...
for tx_count in $TX_COUNTS; do
  ...
  for prover_workers in $PROVER_COUNTS; do
    ...
    if ! env \
      ...
      HEGEMON_TP_AGG_LEAF_FANIN="$LEAF_FANIN" \
      HEGEMON_TP_RPC_WAIT_SECS="$RPC_WAIT_SECS" \
      HEGEMON_TP_STRICT_PREPARE_TIMEOUT_SECS="$STRICT_PREPARE_TIMEOUT_SECS" \
      HEGEMON_TP_BATCH_JOB_TIMEOUT_MS="$BATCH_JOB_TIMEOUT_MS" \
      HEGEMON_TP_SKIP_BUILD="$SKIP_BUILD" \
