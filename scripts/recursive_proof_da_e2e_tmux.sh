...
PROOF_WAIT_SECS="${HEGEMON_E2E_PROOF_WAIT_SECS:-3600}"
...
tmux new-session -d -s "$SESSION" -n node \
  "cd '$ROOT_DIR' && \
   RUST_LOG=info \
   HEGEMON_MINE=1 \
   HEGEMON_RECURSIVE_BLOCK_PROOFS=1 \
...
echo "Waiting for recursive proof log line..." >&2
START_LINES="$(wc -l < "$LOG_FILE" | tr -d ' ')"
START_LINES="$((START_LINES + 1))"
FOUND_BLOCK=""
for i in $(seq 1 "$PROOF_WAIT_SECS"); do
  LINE="$(tail -n +"$START_LINES" "$LOG_FILE" | rg "Recursive block proof generated" | tail -n 1 || true)"
  if [ -n "$LINE" ]; then
...
if [ -z "$FOUND_BLOCK" ]; then
  echo "Timed out waiting for recursive proof generation; inspect logs: $LOG_FILE" >&2
  echo "Attach: tmux attach -t $SESSION" >&2
  echo "Tip: proof generation can take ~15–20 minutes even in fast mode on this build;" >&2
  echo "     increase the timeout via HEGEMON_E2E_PROOF_WAIT_SECS." >&2
  exit 1
fi

echo "Recursive proof generated for block $FOUND_BLOCK" >&2
