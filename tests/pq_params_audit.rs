...
use crypto::{
    ml_kem::{
        MlKemKeyPair, MlKemCiphertext,
        ML_KEM_PUBLIC_KEY_LEN, ML_KEM_SECRET_KEY_LEN, ML_KEM_CIPHERTEXT_LEN, ML_KEM_SHARED_SECRET_LEN,
    },
    ml_dsa::{
        MlDsaSecretKey, MlDsaSignature,
        ML_DSA_PUBLIC_KEY_LEN, ML_DSA_SECRET_KEY_LEN, ML_DSA_SIGNATURE_LEN,
    },
    traits::{KemKeyPair, KemPublicKey, SigningKey, VerifyKey, Signature},
};
...
fn test_ml_kem_768_different_encapsulations() {
    ...
    assert_ne!(ct1.as_bytes(), ct2.as_bytes(), "Different seeds should produce different ciphertexts");
    assert_ne!(ss1.as_bytes(), ss2.as_bytes(), "Different seeds should produce different shared secrets");
    
    // But each decapsulates correctly
    let dec1 = kp.decapsulate(&ct1).expect("Decapsulation 1 should succeed");
    let dec2 = kp.decapsulate(&ct2).expect("Decapsulation 2 should succeed");
...
fn test_ml_dsa_65_different_messages() {
    ...
    assert_ne!(Signature::as_bytes(&sig1), Signature::as_bytes(&sig2), "Different messages should produce different signatures");
    ...
    assert!(pk.verify(msg1, &sig2).is_err(), "Signature should not verify with wrong message");
    assert!(pk.verify(msg2, &sig1).is_err(), "Signature should not verify with wrong message");
...
fn test_ml_dsa_65_wrong_key() {
    ...
    assert!(result.is_err(), "Signature should not verify with wrong public key");
...
fn test_ml_dsa_65_invalid_signature() {
    ...
    assert!(MlDsaSignature::from_bytes(&short_sig).is_err(), "Short signature should be rejected");
...
fn test_pq_vs_classical_key_sizes() {
    ...
    println!("  ML-KEM-768 public:    {} bytes (quantum-secure)", ML_KEM_PUBLIC_KEY_LEN);
    ...
    println!("  ML-DSA-65 public:     {} bytes (quantum-secure)", ML_DSA_PUBLIC_KEY_LEN);
    println!("  ML-DSA-65 signature:  {} bytes (quantum-secure)", ML_DSA_SIGNATURE_LEN);
...
fn test_summary() {
    ...
    println!("  ✅ Public key:     {} bytes (FIPS 203 compliant)", ML_KEM_PUBLIC_KEY_LEN);
    println!("  ✅ Secret key:     {} bytes (FIPS 203 compliant)", ML_KEM_SECRET_KEY_LEN);
    println!("  ✅ Ciphertext:     {} bytes (FIPS 203 compliant)", ML_KEM_CIPHERTEXT_LEN);
    println!("  ✅ Shared secret:  {} bytes (FIPS 203 compliant)", ML_KEM_SHARED_SECRET_LEN);
    println!();
    
    println!("ML-DSA-65 (Digital Signatures):");
    println!("  ✅ Public key:     {} bytes (FIPS 204 compliant)", ML_DSA_PUBLIC_KEY_LEN);
    println!("  ✅ Secret key:     {} bytes (FIPS 204 compliant)", ML_DSA_SECRET_KEY_LEN);
    println!("  ✅ Signature:      {} bytes (FIPS 204 compliant)", ML_DSA_SIGNATURE_LEN);
    println!();
