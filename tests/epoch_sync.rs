...
use epoch_circuit::{
    compute_proof_root, generate_merkle_proof, verify_merkle_proof,
    types::Epoch,
...
fn test_verify_single_epoch() {
    ...
    let epoch_proof = MockEpochProver::prove(&epoch, &proof_hashes)
        .expect("Mock prover should succeed");
...
fn test_non_sequential_epoch_rejected() {
    ...
    let proof0 = MockEpochProver::prove(&epoch0, &proof_hashes)
        .expect("Mock prover should succeed");
    ...
    let proof2 = MockEpochProver::prove(&epoch2, &proof_hashes2)
        .expect("Mock prover should succeed");
...
fn test_merkle_inclusion_proof_generation() {
    ...
    for (index, hash) in proof_hashes.iter().enumerate() {
        ...
        assert!(
            valid,
            "Inclusion proof for index {} should be valid",
            index
        );
    }
}
...
fn test_light_client_verify_inclusion_wrong_epoch() {
    ...
    assert!(!inclusion_valid, "Inclusion for unverified epoch should fail");
}
...
fn test_proof_root_power_of_two() {
    ...
    for &count in &[2, 4, 8, 16, 32] {
        ...
        let root = compute_proof_root(&proof_hashes);
        assert_eq!(root.len(), 32, "Root should be 32 bytes for {} hashes", count);
    }
}
...
fn test_large_epoch_proof_generation() {
    ...
    let epoch_proof = MockEpochProver::prove(&epoch, &proof_hashes)
        .expect("Large epoch should be provable");
...
fn test_light_client_sync_workflow() {
    ...
    let included = client.verify_inclusion(
        0,
        &proof_hashes[tx_index as usize],
        tx_index,
        &merkle_proof,
    );
    assert!(included);
