...
use transaction_circuit::{
    constants::{MAX_INPUTS, MAX_OUTPUTS, POSEIDON_ROUNDS, CIRCUIT_MERKLE_DEPTH},
...
fn test_fri_security_parameters_fast() {
    ...
    if security_estimate < 80.0 {
        println!("⚠️  WARNING: Fast mode has reduced security (~{:.0} bits)", security_estimate);
...
fn test_poseidon_security_parameters() {
    ...
    println!("✅ Poseidon rounds: {} (minimum: {})", POSEIDON_ROUNDS, MIN_POSEIDON_FULL_ROUNDS);
...
fn test_merkle_tree_security() {
    ...
    println!("✅ Merkle tree depth: {} (capacity: {} notes)", CIRCUIT_MERKLE_DEPTH, max_notes);
    ...
    println!("✅ Transaction limits: {} inputs, {} outputs", MAX_INPUTS, MAX_OUTPUTS);
}
...
fn test_constraint_degree_security() {
    ...
    println!("✅ Constraint degree {} with blowup {} is valid", MAX_CONSTRAINT_DEGREE, blowup);
...
fn test_trace_length_security() {
    ...
    println!("✅ Trace length {} (2^{})", MIN_TRACE_LENGTH, MIN_TRACE_LENGTH.trailing_zeros());
}
...
fn test_end_to_end_proof_security() {
    ...
    println!("✅ Generated STARK proof: {} bytes", tx_proof.stark_proof.len());
...
fn test_security_summary() {
    ...
    println!("  Width: {}", transaction_circuit::constants::POSEIDON_WIDTH);
    ...
    println!("  Note capacity: 2^{} = {}", CIRCUIT_MERKLE_DEPTH, 1u64 << CIRCUIT_MERKLE_DEPTH);
