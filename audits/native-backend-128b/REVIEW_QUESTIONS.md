...
8. The active ring `Z_q[X]/(X^8 + 1)` is fully splitting over the Goldilocks field (`q ≡ 1 mod 16`, so `X^8 + 1` factors into 8 linear terms). The in-repo claim operates at the flattened coefficient-space Euclidean SIS level rather than claiming ring-structured hardness. Does the full splitting enable any algebraic or combinatorial attack on the exact flattened `(n_eq=592, m=4104, q, B_2=16336)` SIS instance that would place it below the stated 865-bit quantum floor?
