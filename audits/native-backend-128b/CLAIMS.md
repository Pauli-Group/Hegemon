...
The adjacent checksum, pre-hash compressed-size cap, pre-parser bound on the complete decompressed tar stream, portable member-path policy, streaming bounded extractor, complete-source parity gate, exact non-source layout, isolated-Python regression, and deterministic CI regeneration protect package transport and provenance. The decompressed-stream limit covers PAX, GNU extension, sparse, header, and padding bytes before `tarfile` can consume them; member paths reject POSIX traversal plus Windows drive, UNC, backslash, and alternate-stream reinterpretation before native path joining. After parity succeeds, verification copies the source snapshot outside the attacker-controlled package directory before invoking Cargo, so extra package-local configuration cannot affect compilation. Complete verification regenerates every claim, manifest, model, reduced-search report, structured-lattice metadata and binary, and verifier report from that source, normalizes generated paths, and requires byte-for-byte equality with the supplied evidence. The generic blueprint review digest deliberately excludes the archive and adjacent checksum bytes because the package contains that blueprint and hashing both creates an unsatisfiable self-reference; this complete verifier and checksum remain the package-specific authority. These controls do not prove the native backend's cryptographic assumptions.
